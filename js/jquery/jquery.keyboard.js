...
$(window).on("orientationchange",function(){
  setTimeout(function () { 
       var landscapeMode = screen.width > screen.height;
       if(landscapeMode && isMobile.any()) enableVirtualKeyboard(); else disableVirtualKeyboard();
          // alert(landscapeMode)
    }, 1)
});

$( document ).ready(function() {
     $(':text:not([readonly])').addClass('kdbInputs');
     var landscapeMode = screen.width > screen.height;
     if(landscapeMode && isMobile.any()) enableVirtualKeyboard(); 
});

// show a fake cursor (.)
function insertAfterCaret(elem) {
   var caretChar = String.fromCharCode(183);

    removeCaret(elem)
    var caretPos = $(elem)[0].selectionStart;
    var textAreaTxt = $(elem).val();
    $(elem).val(textAreaTxt.substring(0, caretPos) + caretChar + textAreaTxt.substring(caretPos));
    $(elem)[0].selectionStart = caretPos;
    $(elem)[0].selectionEnd = caretPos;
}

function removeCaret(elem) {
   var caretChar = String.fromCharCode(183);

    var textAreaTxt = $(elem).val();
    var caretPos = $(elem)[0].selectionStart;
    var pos = textAreaTxt.indexOf(caretChar);

    if (pos == -1) return;
    if (caretPos > pos) caretPos--;
    textAreaTxt = textAreaTxt.replace(caretChar, "");
    $(elem).val(textAreaTxt);
    $(elem)[0].selectionStart = caretPos;
    $(elem)[0].selectionEnd = caretPos;
    $(elem).data('oldCaretPos', -1)
}
...
function enableVirtualKeyboard() {
    $('.kdbInputs').keyboard({
        ...
        hidden: function(e, keyboard, elem) {
            $(elem).removeProp('disable')
            $(elem).data('oldCaretPos', -1);
            removeCaret(elem);
            elem.blur()
           }
    ...
    // Show virtual caret
    $('.kdbInputs').bind("click keyup", caretHandler);
    
}

function caretHandler(event) {
    var elem = $(this);
    var caretPos = $(elem)[0].selectionStart;
    var oldCaretPos = elem.data('oldCaretPos')

    if (elem.data('oldVal') != elem.val() || oldCaretPos != caretPos) {
        
        elem.data('oldVal', elem.val());
        elem.data('oldCaretPos', caretPos)
        
        var caretPos = $(elem)[0].selectionStart;
        
        $(this).prop('readonly', true); // stops android keyboard
        setTimeout(function () { 
           $(elem)[0].selectionStart = caretPos 
           $(elem)[0].selectionEnd = caretPos 
        }, 1)
        
        insertAfterCaret(elem)
    }
}

function disableVirtualKeyboard() {
    $('.kdbInputs').keyboard().getkeyboard().destroy();
    $('.kdbInputs').prop('readonly', false);
    
    // remove virtual caret
    $('.kdbInputs').each(function () {
        removeCaret($(this))
        $(this)[0].blur()
    }); 
    
    $('.kdbInputs').unbind("click keyup", caretHandler)
}
