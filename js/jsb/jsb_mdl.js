...
async function JSB_MDL_ADDCLASSNAMES_Sub(ByRef_Dbgroups, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "addclassnames", "JSB_MDL_ADDCLASSNAMES_Sub");
    // local variables
    var Tagname, V;
...
async function JSB_MDL_ADDCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "addcolumn", "JSB_MDL_ADDCOLUMN_Sub");
    // local variables
    ...
    Newcolumnid = Trim(await JSB_BF_INPUTBOX('Add Column', 'New column name', undefined, undefined, undefined, function (_P1) {  })); 
    ...
    if (await JSB_ODB_WRITEJSON(Dataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname))); else return Stop(At(-1), 'edc-', System(28, me), ': ', me._activeProcess.At_Errors);
...
async function JSB_MDL_ADDEXTRACOLUMNS2VIEWMODEL_Sub(ByRef_Model, ByRef_Templatename, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "ADDEXTRACOLUMNS2VIEWMODEL", "JSB_MDL_ADDEXTRACOLUMNS2VIEWMODEL_Sub");
    // local variables
...
async function JSB_MDL_ADDFROMANOTHERVIEW(ByRef_Projectname, ByRef_Viewname, ByRef_Dataset, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "addfromanotherview", "JSB_MDL_ADDFROMANOTHERVIEW");
    // local variables
    ...
    if (Left(Ans,Len(Tprefix))==Tprefix) {
       ...
       Possiblecolumns = await JSB_BF_GETTABLECOLUMNDEFS(Ans, CStr(true), true, function (_Ans) { Ans = _Ans });
    } else {
...
async function JSB_MDL_ADDNESTEDVIEWS(ByRef_Projectname, ByRef_Views, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "addnestedviews", "JSB_MDL_ADDNESTEDVIEWS");
    // local variables
...
async function JSB_MDL_ADDPAGETOMENU(ByRef_Projectname, ByRef_Pagename, ByRef_Whichmenupage, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "addpagetomenu", "JSB_MDL_ADDPAGETOMENU");
    // local variables
    ...
    if (isEmpty(ByRef_Projectname)) { me._activeProcess.At_Errors = 'Missing URL Parameters'; return exit(false); }
    ...
    if (await JSB_ODB_READJSON(Pagedataset, await JSB_BF_FHANDLE('dict', ByRef_Projectname), CStr(ByRef_Whichmenupage) + '.page', function (_Pagedataset) { Pagedataset = _Pagedataset })); else {
       ...
       Pagedataset = await JSB_MDL_NEWROW('Menu', Objectmodel.columns, function (_P1, _P2) {  }); 
       Pagedataset.templateName = Pagetype; 
       Pagedataset.menuitem = 'Admin'; 
       Pagedataset.centerview = CStr(ByRef_Whichmenupage) + '.view'; 
       if (await JSB_ODB_WRITEJSON(Pagedataset, await JSB_BF_FHANDLE('dict', ByRef_Projectname), CStr(ByRef_Whichmenupage) + '.page')); else return Stop(At(-1), 'Edp-', System(28, me), ': ', me._activeProcess.At_Errors);
    }
    ...
    for (C of iterateOver(Viewdataset.columns)) {
       if (Null0(C.name)==ByRef_Pagename) Alreadythere = true;
    } 
    ...
    if (Not(Alreadythere)) {
       ...
       if (await JSB_ODB_WRITEJSON(Viewdataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(Viewname))); else return Stop(At(-1), 'Edp-', System(28, me), ': ', me._activeProcess.At_Errors);
    }
    ...
    if (await JSB_ODB_READJSON(Viewdataset, await JSB_BF_FHANDLE('dict', ByRef_Projectname), CStr(ByRef_Pagename) + '.view', function (_Viewdataset) { Viewdataset = _Viewdataset })) {
       ...
       if (await JSB_ODB_WRITEJSON(Viewdataset, await JSB_BF_FHANDLE('dict', ByRef_Projectname), CStr(ByRef_Pagename) + '.view')); else Alert(me._activeProcess.At_Errors);
    }
...
async function JSB_MDL_ADDPAGETOMENU_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "ADDPAGETOMENU", "JSB_MDL_ADDPAGETOMENU_Pgm");
    // local variables
    var Projectname, Pagename, Frompage, Closewindow;
    ...
    Closewindow = paramVar('closeWindow'); 

    if (Not(await JSB_MDL_ADDPAGETOMENU(Projectname, Pagename, 'Menu', function (_Projectname, _Pagename, _P3) { Projectname = _Projectname; Pagename = _Pagename }))) Alert(me._activeProcess.At_Errors);

    if (CBool(Frompage)) return At_Response.Redirect(Frompage);
    if (CBool(Closewindow) || !JSB_BF_HASPARENTPROCESS()) At_Server.End();

    return At_Response.Redirect(JSB_BF_JSBROOTEXECUTE(CStr(Pagename)));
}
// </ADDPAGETOMENU_Pgm>

// <ADDPAGETOREFERENCEMENU_Pgm>
async function JSB_MDL_ADDPAGETOREFERENCEMENU_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "addpagetoreferencemenu", "JSB_MDL_ADDPAGETOREFERENCEMENU_Pgm");
    // local variables
    var Projectname, Pagename, Frompage, Closewindow;
    ...
    if (Not(await JSB_MDL_ADDPAGETOMENU(Projectname, Pagename, 'ReferenceMenu', function (_Projectname, _Pagename, _P3) { Projectname = _Projectname; Pagename = _Pagename }))) Alert(me._activeProcess.At_Errors);
...
async function JSB_MDL_ADDVIEWBTN(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, Modelafterbtn, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "addviewbtn", "JSB_MDL_ADDVIEWBTN");
    // local variables
    ...
    Defaultrow = await JSB_MDL_GETJSONDEFDEFAULTS('customBtns', function (_P1) {  }); 
    ...
    if (await JSB_ODB_WRITEJSON(Objectmodel, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname))); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
    await JSB_MDL_REGENPAGEMODEL_Sub(ByRef_Projectname, ByRef_Pagename, false); 
    return exit(true); 
}
// </ADDVIEWBTN>
...
async function JSB_MDL_BUILDSQLSELECT(ByRef_Viewmodel, Haslocalformvars, Orderallcolumns, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "buildsqlselect", "JSB_MDL_BUILDSQLSELECT");
    // local variables
    ...
    var Hasparam, Dftval, Cj, Html, Possibleerrors, Columns, Cnames;
    ...
    if (CBool(Inputs)) {
       ...
       for (Row of iterateOver(Inputs)) {
          ...
          Varname = nicename(CStr(Row.name)); 
       ...
       if (CBool(Checkrequired)) {
          Html[Html.length] = '   @Errors = \'\''; 
          Html[Html.length] = Join(Checkrequired,crlf); 
          Html[Html.length] = '   If @Errors Then'; 
          Html[Html.length] = '      rtnErrors = @Errors'; 
          Html[Html.length] = '      sqlFilter = \'1=0\''; 
          Html[Html.length] = '   Else'; 
          Html[Html.length] = Join(Buildselect,crlf); 
          Html[Html.length] = '   End If';
       } else {
          Html[Html.length] = Join(Buildselect,crlf);
       }

       if (CBool(ByRef_Viewmodel.orderby)) Html[Html.length] = '    sqlFilter := ` ' + CStr(ByRef_Viewmodel.orderby) + '`';
    } else {
       Html = Buildselect; 
       if (CBool(ByRef_Viewmodel.orderby)) Html[Html.length] = [undefined, '    sqlFilter = `' + CStr(ByRef_Viewmodel.orderby) + '`'];
    }
    ...
    Html[Html.length] = '   SqlColumns = `' + Join(Cnames,',') + '`'; 

    return exit(Join(Html,crlf)); 
}
// </BUILDSQLSELECT>

// <CHOOSECOLUMNS_Sub>
async function JSB_MDL_CHOOSECOLUMNS_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "choosecolumns", "JSB_MDL_CHOOSECOLUMNS_Sub");
    // local variables
    ...
    Possiblecolumns = await JSB_BF_GETTABLECOLUMNDEFS(Dataset.tableName, CStr(true), true, function (_P1) {  }); 

    if (await JSB_MDL_CHOOSECOLUMNS_SS(CStr(ByRef_Projectname), CStr(ByRef_Viewname), Dataset, Possiblecolumns)) {
       if (await JSB_ODB_WRITEJSON(Dataset, Fdict, CStr(ByRef_Viewname))); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
       await JSB_MDL_REGENPAGEMODEL_Sub(ByRef_Projectname, ByRef_Pagename, false);
    }
    return exit(); 
}
...
async function JSB_MDL_CHOOSECOLUMNS_SS(Projectname, Viewname, Dataset, Possiblecolumns, Question) { 
    var me = new jsbRoutine("JSB_MDL", "choosecolumns_ss", "JSB_MDL_CHOOSECOLUMNS_SS");
    // local variables
    ...
    if (Len(Possiblecolumns)) {
       if (HasTag(Possiblecolumns[LBound(Possiblecolumns)],'Oridinal')) {
          Possiblecolumns = Sort(CStr(Possiblecolumns), 'Oridinal'); 
          Needsort = false;
       }
    }
    ...
    var Innerhtmldialog = JSB_HTML_MULTISELECTLISTBOX('ddbox', Allcolumns, CStr(Selcolumns), [undefined, ]) + JSB_HTML_BR() + JSB_HTML_BR(); 
    Innerhtmldialog = CStr(Innerhtmldialog) + CStr(htmlEncode('Add detached columns (comma seperated):')) + JSB_HTML_BR() + JSB_HTML_BR(); 
    Innerhtmldialog = CStr(Innerhtmldialog) + JSB_HTML_TEXTBOX('addcols'); 

    Innerhtmldialog = CStr(Innerhtmldialog) + Chr(28) + '\<input type="button" value=\'or Select another table/view\' class=\'Button\''; 
    Innerhtmldialog = CStr(Innerhtmldialog) + '   onclick="$(\'#msgboxResult\').val(Chr(27)+\'SA\'); $(\'#dialog\').dialog(\'close\'); "'; 
    Innerhtmldialog = (CStr(Innerhtmldialog) + '/\>&nbsp;\<br /\>\<br /\>'); 

    Innerhtmldialog = CStr(Innerhtmldialog) + '\<input type="submit" value=\'OK\' class=\'Button\''; 
    Innerhtmldialog = (CStr(Innerhtmldialog) + 'onclick="\r\n\
...
\r\n\
    ...
    Innerhtmldialog = (CStr(Innerhtmldialog) + '/\>&nbsp;'); 
    Innerhtmldialog = CStr(Innerhtmldialog) + '\<input type="button" value=\'All\' class=\'Button\''; 
    Innerhtmldialog = CStr(Innerhtmldialog) + '   onclick="multiSelectAll($(\'#ddbox\'));"'; 
    Innerhtmldialog = (CStr(Innerhtmldialog) + '/\>&nbsp;'); 

    Innerhtmldialog = CStr(Innerhtmldialog) + '\<input type="button" value=\'None\' class=\'Button\''; 
    Innerhtmldialog = CStr(Innerhtmldialog) + '   onclick="multiSelectNone($(\'#ddbox\'));"'; 
    Innerhtmldialog = (CStr(Innerhtmldialog) + '/\>&nbsp;'); 

    Innerhtmldialog = CStr(Innerhtmldialog) + '\<input type="submit" value=\'Cancel\' class=\'Button\''; 
    Innerhtmldialog = CStr(Innerhtmldialog) + '   onclick="$(\'#msgboxResult\').val(Chr(27)); $(\'#dialog\').dialog(\'close\'); "'; 
    Innerhtmldialog = (CStr(Innerhtmldialog) + '/\>&nbsp;' + Chr(29)); 
    ...
    for (Selectedname of iterateOver(Selectedcols)) {
       ...
       for (Oldcolumn of iterateOver(Dataset.columns)) {
          if (Null0(Oldcolumn.name)==Selectedname) {
             Previousdef = true; 
       ...
       if (!Previousdef) {
          ...
          for (Newcolumn of iterateOver(Possiblecolumns)) {
             if (Null0(Newcolumn.name)==Selectedname) {
          ...
          if (!Previousdef) {
             ...
             if (CBool(Dataset.tableName)) { if (await JSB_ODB_WRITE(CStr(Newcolumn), await JSB_BF_FHANDLE('dict', Dataset.tableName), '!' + CStr(Selectedname))); else return Stop(me._activeProcess.At_Errors); }
          }
       }
    } 

    var _ForEndI_24 = UBound(Dataset.columns); 
    for (I = 1; (I<=_ForEndI_24) && I<=UBound(Dataset.columns); I++) {
...
async function JSB_MDL_CLONEPAGEMODEL(Projectname, Oldpagename, Clonepagename) { 
    var me = new jsbRoutine("JSB_MDL", "clonepagemodel", "JSB_MDL_CLONEPAGEMODEL");
    // local variables
    ...
    if (Not(Clonepagename)) {
       Clonepagename = Trim(await JSB_BF_INPUTBOX('Save a copy As', 'Enter a clone pageName or \'(\' projectname clonePageName', '', '80%', '', function (_P1) {  })); 
    ...
    if (await JSB_ODB_READJSON(Pagedataset, await JSB_BF_FHANDLE('dict', Projectname), CStr(Oldpagename) + '.page', function (_Pagedataset) { Pagedataset = _Pagedataset })); else { me._activeProcess.At_Errors = 'Unable to find page ' + CStr(Oldpagename); return ''; }
    ...
    if (await JSB_ODB_READ(Src, await JSB_BF_FHANDLE(CStr(Cprojectname)), CStr(Oldpagename), function (_Src) { Src = _Src })) {
       if (await JSB_ODB_WRITE(Change(Src,Oldpagename,Clonepagename), await JSB_BF_FHANDLE(CStr(Cprojectname)), CStr(Clonepagename))); else return Stop(me._activeProcess.At_Errors);
    }
...
async function JSB_MDL_CLONEPAGEMODEL_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "CLONEPAGEMODEL", "JSB_MDL_CLONEPAGEMODEL_Pgm");
    // local variables
    ...
    if (isEmpty(Projectname) && InStr1(1,me._activeProcess.At_Sentence,'=')==0) {
       Sentence = Field(me._activeProcess.At_Sentence,'(',1); 
    ...
    if (Clonepagename==Chr(27) || isEmpty(Clonepagename)) {
       ...
       if (JSB_BF_HASPARENTPROCESS()) return Stop();
       await JSB_MDL_RESPONSEREDIRECT(JSB_BF_JSBROOTEXECUTE(CStr(Pagename)), function (_P1) {  });
    }
    ...
    await JSB_MDL_RESPONSEREDIRECT(JSB_BF_JSBROOTEXECUTE(CStr(Clonepagename)), function (_P1) {  });
    return; 
}
// </CLONEPAGEMODEL_Pgm>
...
async function JSB_MDL_CLONEVIEWTO(Fromprojectname, Toprojectname, Fromviewname, Opttoviewname) { 
    var me = new jsbRoutine("JSB_MDL", "cloneviewto", "JSB_MDL_CLONEVIEWTO");
    // local variables
    ...
    if (Not(Opttoviewname)) {
       Opttoviewname = Trim(await JSB_BF_INPUTBOX('Save a copy As', 'Enter a clone name (may space-prefix with another project)', '', '80%', '', function (_P1) {  })); 
    ...
    if (await JSB_ODB_READJSON(Dataset, Ftodict, Fromviewname, function (_Dataset) { Dataset = _Dataset })); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);

    for (Column of iterateOver(Dataset.columns)) {
       var Ctlname = LCase(Column.control); 
       if (Ctlname=='json_inline' || Ctlname=='json_popup') {
          Newviewname = await JSB_MDL_MAKECLONENAME(Toprojectname, dropIfRight(Fromviewname, '.view', true), Column.useview, dropIfRight(Opttoviewname, '.view', true)); 
          Column.useview = await JSB_MDL_CLONEVIEWTO(Fromprojectname, Toprojectname, CStr(Column.useview), CStr(Newviewname));
       }
    } 

    if (await JSB_ODB_READ(Htm, Ftodict, Fromviewname + '.htm', function (_Htm) { Htm = _Htm })) {
       if (await JSB_ODB_WRITE(CStr(Htm), Ftodict, Opttoviewname + '.htm')); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
    }

    if (await JSB_ODB_WRITEJSON(Dataset, Ftodict, Opttoviewname + '.view')); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
...
async function JSB_MDL_CLONEWEBSITE_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "clonewebsite", "JSB_MDL_CLONEWEBSITE_Pgm");
    // local variables
    ...
    while (true) {
       Newactname = await JSB_BF_INPUTBOX('Clone ' + CStr(Projectname), 'New Web Site Name', '', '80%', '', function (_P1) {  }); 
       ...
       if (CBool(Dft)) {
          ...
          if (await JSB_ODB_WRITE(CStr(Dft), Fmd, CStr(Newactname))); else await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
          Didclone = await JSB_BF_CLONEACCOUNT(Newactname, undefined, function (_Newactname) { Newactname = _Newactname }); 
          if (CBool(Hold)) {
             if (await JSB_ODB_WRITE(CStr(Hold), Fmd, CStr(Newactname))); else await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
          } else {
             if (await JSB_ODB_DELETEITEM(Fmd, CStr(Newactname))); else await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
          }
       } else {
          Didclone = await JSB_BF_CLONEACCOUNT(Newactname, undefined, function (_Newactname) { Newactname = _Newactname });
       }
       ...
       await JSB_BF_MSGBOX(me._activeProcess.At_Errors)
    } 

    return At_Response.Redirect(JSB_BF_HTMLROOT() + CStr(Newactname));
}
// </CLONEWEBSITE_Pgm>

// <COPYVIEWFROM>
async function JSB_MDL_COPYVIEWFROM(ByRef_Projectname, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "copyviewfrom", "JSB_MDL_COPYVIEWFROM");
    // local variables
    ...
    Fdict = await JSB_BF_FHANDLE('DICT', ByRef_Projectname); 
    if (await JSB_ODB_SELECTTO('', Fdict, 'ItemId Like \'%.view\'', Sl, function (_Sl) { Sl = _Sl })); else { Println(Alert(me._activeProcess.At_Errors)); debugger; }
...
async function JSB_MDL_CREATENEWVIEW(ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "createnewview", "JSB_MDL_CREATENEWVIEW");
    // local variables
    ...
    Dataset = await JSB_MDL_NEWROW(dropIfRight(CStr(ByRef_Viewname), '.view', true), Objectmodel.columns, function (_P1, _P2) {  }); 
    Dataset.attachdb = JSB_BF_ACCOUNT(); 
...
async function JSB_MDL_CRLFCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "crlfcolumn", "JSB_MDL_CRLFCOLUMN_Sub");
    // local variables
    ...
    if (await JSB_ODB_READJSON(Dataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname), function (_Dataset) { Dataset = _Dataset })); else return exit(undefined);
    Columnidx = await JSB_MDL_FINDVIEWCOLUMN(Dataset, ByRef_Columnid, function (_Dataset) { Dataset = _Dataset }); 
    if (Columnidx=='0') return exit(undefined);
...
async function JSB_MDL_CUSTOMMERGE(Newgeneratedcode, Priorgeneratedcode) { 
    var me = new jsbRoutine("JSB_MDL", "custommerge", "JSB_MDL_CUSTOMMERGE");
    // local variables
    ...
    while (true) {
       ...
       if (Left(Ptline,10)=='* \<insert\>' || Left(Ptline,9)=='*\<insert\>') {
          ...
          Ptline = Trim(Pline); 
          if (Ptline==Ntline) Pci++;
          Mergedcode[Mergedcode.length] = Pline; 
          Pci++; ;
       } else if (Ptline==Ntline) {
          ...
          for (Nci2 =  +Nci; Nci2<=_ForEndI_13; Nci2++) {
             ...
             if (Ptline==Ntline2 && Len(Ptline)>3) {
    ...
    if (Z>19999) Alert('mdl updatecode fail safe fallout');
...
async function JSB_MDL_DB_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "db", "JSB_MDL_DB_Pgm");
    // local variables
    var Rolemsg, Tabid, Treeid, Centerhtml, Westoptions, Theme;
    var Northhtml, Northoptions, Southoptions, Eastoptions, Fromroot;
    var Tree, Westhtml, Html, Cmd, Itemid, Menucmd, Menutext, Menuval;
    var Targetid, Targettext;
    ...
    Rolemsg = (JSB_BF_ISAUTHENTICATED()?'':'You must be logged in to view this page. ' + JSB_HTML_ANCHOR(JSB_BF_LOGINURL(), 'Click here to login')); 
    ...
    do {
       ...
       Westhtml = JSB_HTML_DIV('', JSB_HTML_JSONTREE(CStr(Treeid), Tree), {"style":'height: 98%; width: 100%; padding-top: 5px'}); 
       ...
       Html = CStr(Html) + JSB_HTML_LAYOUT(CStr(Centerhtml), CStr(Northhtml), CStr(Northoptions), 'South', CStr(Southoptions), 'East', CStr(Eastoptions), CStr(Westhtml), CStr(Westoptions)); 

       Html = CStr(Html) + JSB_HTML_SCRIPT('function treeShow() { $("#mylayout").layout().open("west");}'); 
       Html = CStr(Html) + JSB_HTML_SCRIPT('function treeToggle() {$("#mylayout").layout().toggle("west");}'); 
       Html = CStr(Html) + JSB_HTML_SCRIPT('function treeHide() { if (innerWidth \< 1024) $("#mylayout").layout().close("west");}'); 

       // Add javascript to allow tree to be loaded via API
       Html = CStr(Html) + await JSB_MDL_DB_LOADDBTREEGROUPS(Tabid, Treeid, function (_Tabid, _Treeid) { Tabid = _Tabid; Treeid = _Treeid }); 

       // Add jQuery drag & drop to allow .databases to be dropped on .databaseLists
       if (await JSB_BF_ISMANAGER()) {
          Html = CStr(Html) + await JSB_MDL_DB_SETUPDRAGANDDROP(Fromroot, function (_Fromroot) { Fromroot = _Fromroot }); 
          Html = CStr(Html) + await JSB_MDL_DB_SETUPCONTEXTMENUS(Fromroot, function (_Fromroot) { Fromroot = _Fromroot });
       }

       // Theme it
       if (CBool(Theme)) Html = await asyncCallByName(Theme, me, 0/*ignore if missing */, System(27, me), System(28, me), Html);

       // Display it
       Print(At(-1), Html); 

       if (!JSB_BF_HASTCLPARENT()) break;
...
async function JSB_MDL_DB_SETUPDRAGANDDROP(ByRef_Fromroot, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "DB", "JSB_MDL_DB_SETUPDRAGANDDROP");
    function exit(v) {
...
async function JSB_MDL_DB_SETUPCONTEXTMENUS(ByRef_Fromroot, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "DB", "JSB_MDL_DB_SETUPCONTEXTMENUS");
    // local variables
    var Javascriptsharecode, Javascriptrenamecode, Javascriptdeletecode;
    var Javascriptnewsubgrpcode, S, Cmgroupmenu, Html, Cmdatabasemenu;
...
\r\n\
    ...
    Html = CStr(Html) + JSB_HTML_CONTEXTMENU('#doNothingYet', {
...
"Delete Report":{"onclick":'deleteRPT(contextMenuTarget, this);'}
    ...
    Html = CStr(Html) + JSB_HTML_CONTEXTMENU('#doNothingYet', {"New Sub-Group":{"onclick":Javascriptnewsubgrpcode}}, false, CStr(Cmrootmenu)); 
    S[S.length] = '$(document).on("contextmenu", ".css-treeblankspace", function (e) { return ' + CStr(Cmrootmenu) + '(e, this) });'; 

    Html = CStr(Html) + JSB_HTML_SCRIPT('\r\n\
...
async function JSB_MDL_DB_BUILDJSON4TREEVIEW(Tabid, Root) { 
    var me = new jsbRoutine("JSB_MDL", "DB", "JSB_MDL_DB_BUILDJSON4TREEVIEW");
    // local variables
    var Dbgroups, Nopermissions, Onlinedbs, Cnt, Tree;

    // %options aspxC-
    Dbgroups = await asyncRpcRequest('DB_GETGROUPSFROMROOT', Root, function (_Root) {  }); 
...
async function JSB_MDL_DB_BUILDTREEVIEWSUBLEVEL(Tabid, Nopermissions, Onlinedbs, Dbsubgroup, ByRef_Tree, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "DB", "JSB_MDL_DB_BUILDTREEVIEWSUBLEVEL");
    // local variables
       ...
       if (isJSON(V)) {
          ...
          if (Len(V)) {
             ...
             if (CBool(Subcnt)) {
                ...
                Subtree.caption = await JSB_MDL_DISABLEDLOOK(CStr(Tag) + ' (not online)', function (_P1) {  });
             }
          } else {
             // empty group
             ByRef_Tree[Tag] = {"url":''}
          ...
          ByRef_Tree[Tag] = ({
"url":JSB_BF_JSBRESTCALL('db_getSubTree') + '?tabID=' + urlEncode(CStr(Tabid)) + '&databaseName=' + urlEncode(CStr(V)) + '&noCache=' + CStr(Timer()),
"data":V,
"class":'database',
"id":Tag
}); 
          ...
          if (Locate(LCase(V), Onlinedbs, 0, 0, 0, "", position => { })) {
             if (Locate(LCase(V), Nopermissions, 0, 0, 0, "", position => { })) {
                ByRef_Tree[Tag].caption = await JSB_MDL_DISABLEDLOOK(CStr(Tag) + ' (no permissions)', function (_P1) {  }); 
                ByRef_Tree[Tag].url = '';
             }
             Dbcnt++;
          } else {
             ByRef_Tree[Tag].caption = await JSB_MDL_DISABLEDLOOK(CStr(Tag) + ' (not online)', function (_P1) {  }); 
             ByRef_Tree[Tag].url = '';
          }
       }
    } 
...
async function JSB_MDL_DISABLEDLOOK(ByRef_Txt, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "DB", "JSB_MDL_DISABLEDLOOK");
    function exit(v) {
...
async function JSB_MDL_DB_LOADDBTREEGROUPS(ByRef_Tabid, ByRef_Treeid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "DB", "JSB_MDL_DB_LOADDBTREEGROUPS");
    function exit(v) {
...
async function JSB_MDL_DBCLEANJSON(ByRef_Dbsubgroup, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "dbcleanjson", "JSB_MDL_DBCLEANJSON");
    // local variables
    var Tag, V, Ary, Db;
...
async function JSB_MDL_DBUPLOADDATABASE_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "dbuploaddatabase", "JSB_MDL_DBUPLOADDATABASE_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                ...
                Ctlhtml = JSB_BF_LBLCTLSET('Database', CStr(Ctlhtml), CStr(Ctlid), true, 0, ''); 
            
                JSB_HTML_SUBMITBTN('Btn', 'OK', 'OK Display', {}, 'danger', 'lg', 'alert(\'you clicked it\')');
            
                Btns = JSB_HTML_DIV('', JSB_HTML_SAVEBTN('cmd', 'Upload', 'Upload', {}, '', '', 'if ($(\'#uploadBoxID\').val()) startSpinner(\'Uploading\')') + JSB_HTML_SAVEBTN('cmd', 'Cancel', 'Cancel'), {"class":'col-xs-12 col-sm-10 col-md-10 col-lg-11',"style":'xmargin-top:5px'}); 
                ...
                if (Cmd!='Upload') { me._state = "DOEXIT"; continue atgoto; }
            
                Uploadddbname = At_Request.FileName(Ctlid); 
                if (Not(Uploadddbname)) { me._state = "ERROUT"; continue atgoto; }
                ...
                if (Locate(Ext, [undefined, 'accdb', 'mdb', 'db3', 'sl3', 'xls', 'xlsx', 'xlsm', 'xlsb'], 0, 0, 0, "", position => { })); else {
                   ...
                   me._state = "DOEXIT"; continue atgoto;
                }
                ...
                if (Locate(Ext, [undefined, 'xls', 'xlsx', 'xlsm', 'xlsb'], 0, 0, 0, "", position => { })) {
                   ...
                   if (await JSB_ODB_WRITE(CStr(Uploadedfilecontents), Ftmp, CStr(Srcdb))); else { me._state = "ERROUT"; continue atgoto; }
                   if (Not(await JSB_MDL_SHOWEXCELFORM(undefined, Ftmp.filePath(), Srcdb, 'scp_history', function (_P1, _P2, _Srcdb, _P4) { Srcdb = _Srcdb }))) await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
                   return Stop();
                }
                ...
                while (await JSB_BF_DBDATABASEEXISTS(CStr(Dstdb))) {
                   ...
                   if (isEmpty(Ans) || Ans==Chr(27) || Ans=='Cancel') { me._state = "ERROUT"; continue atgoto; }
                   ...
                   switch (Ans) {
                      case 'Delete Existing SQL DB': 
                         ...
                         if (!(await JSB_BF_DBDROPSQLDATABASE(CStr(Dstdb)))) await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
                   ...
                   break; 
                      ...
                      case 'Choose a New Name': case 'Rename the existing SQL DB': 
                         ...
                         while (true) {
                            for (I = 1; I<=50; I++) {
                               if (I>1) Newname = CStr(Dbprefix) + CStr(Uploadddbname) + '_' + CStr(I);
                               if (!(await JSB_BF_DBDATABASEEXISTS(CStr(Newname)))) break;
                            } 
                            if (I>=50) Newname = '';
            
                            Newname = await JSB_BF_INPUTBOX('New Name', CStr(Msg), CStr(Newname), undefined, undefined, function (_P1) {  }); 
                            if (isEmpty(Newname) || Newname==Chr(27)) { me._state = "DOEXIT"; continue atgoto; }
                         ...
                         if (Ans=='Choose a New Name') {
                            ...
                            if (!(await JSB_BF_DBRENAMESQLDATABASE(CStr(Dstdb), CStr(Dbprefix) + CStr(Newname)))) await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
                         }
                ...
                if (await JSB_ODB_WRITE('D\xFE' + CStr(Ds), Fsystem, CStr(Srcdb))); else { me._state = "ERROUT"; continue atgoto; }
            
                if (await JSB_BF_DBCOPYDATABASE(Srcdb, Dstdb, Appendrecords, function (_Srcdb, _Dstdb, _Appendrecords) { Srcdb = _Srcdb; Dstdb = _Dstdb; Appendrecords = _Appendrecords })) { me._state = "DOEXIT"; continue atgoto; }
            
                
            case "ERROUT": 
                
                if (me._activeProcess.At_Errors) await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
    ...
    ')); 
            ...
            default:
                throw "we entered " + me._functionName + " with an invalid state: " + me._state; 
        } // switch
        } // agoto while
}
// </DBUPLOADDATABASE_Pgm>
...
async function JSB_MDL_DB_CUSTOMMENU_Sub(ByRef_Jsonmenu, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "db_custommenu", "JSB_MDL_DB_CUSTOMMENU_Sub");
    // local variables
    ...
    // ******************* upload     // ******************* 
    if (await JSB_BF_ISMANAGER()) {
       Url = (JSB_BF_JSBRESTCALL('dbUploadDatabase') + '?tabID=' + urlEncode(CStr(Tabid)) + '&noCache=' + CStr(Timer())); 
       Onclick = 'openInTab(' + JSB_BF_JSESCAPESTRING(CStr(Tabid)) + ', "Upload_Database", "Upload database", ' + JSB_BF_JSESCAPESTRING(CStr(Url)) + ');'; 
       ByRef_Jsonmenu.File = {}; 
       ByRef_Jsonmenu.File.Upload = {"id":'upload',"caption":'Upload MS Access Or Excel DB',"onclick":Onclick}; 
       ByRef_Jsonmenu.collaspable = true;
    }

    if (JSB_BF_HASTCLPARENT()) {
...
async function JSB_MDL_DB_SERVER() { 
    var me = new jsbRoutine("JSB_MDL", "db_server", "JSB_MDL_DB_SERVER");
}
// </DB_SERVER>

// <MOVEDBTOLIST>
async function JSB_MDL_MOVEDBTOLIST(Dbsubgroup, Dbname, Togroup) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_MOVEDBTOLIST");
    // local variables
    var Tag;
    ...
    if (Togroup) {
       for (Tag of iterateOver(Dbsubgroup)) {
          ...
          if (isJSON(V) && L) {
             if (Tag==Null0(Togroup)) {
                V[Dbname] = Dbname; 
                return true;
             } else {
                // a High level group
                if (await JSB_MDL_MOVEDBTOLIST(V, Dbname, Togroup)) return true;
             }
          } else if (Tag==Null0(Togroup)) {
             // *** a single database
             V = [undefined, Dbname]; 
             Dbsubgroup[Tag] = V; 
             return true;
...
async function JSB_MDL_HASGROUPNAME(Dbsubgroup, Groupname) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_HASGROUPNAME");
    // local variables
    var Tag;

    for (Tag of iterateOver(Dbsubgroup)) {
       if (Tag==Null0(Groupname)) return true;
       var V = Dbsubgroup[Tag]; 

       if (isJSON(V)) {
          if (await JSB_MDL_HASGROUPNAME(V, Groupname)) return true;
       }
    } 
...
async function JSB_MDL_REMOVETREEVIEWDB(Dbsubgroup, Dbname) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_REMOVETREEVIEWDB");
    // local variables
    var Tag, Spot;
    ...
    for (Tag of iterateOver(Dbsubgroup)) {
       ...
       if (isJSON(V)) {
          // a High level group
          if (await JSB_MDL_REMOVETREEVIEWDB(V, Dbname)) Changed = true;
       } else if (CBool(isArray(V))) {
...
async function JSB_MDL_DBFINDANDRENAMEGROUP(Dbsubgroup, Path, Newname) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DBFINDANDRENAMEGROUP");
    // local variables
    var Tag;

    var Groupname = (fieldLeft(Path, '|')); 
    var Subpath = (dropLeft(Path, '|')); 

    for (Tag of iterateOver(Dbsubgroup)) {
       if (Tag==Null0(Groupname)) {
          var V = Dbsubgroup[Tag]; 
          if (Subpath) {
             if (isJSON(V)) {
                if (await JSB_MDL_DBFINDANDRENAMEGROUP(V, Subpath, Newname)) return true;
             }
          } else {
...
async function JSB_MDL_DBFINDANDDELETEGROUP(Dbsubgroup, Path) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DBFINDANDDELETEGROUP");
    // local variables
    var Tag;
...
async function JSB_MDL_NEWSUBGROUPAPI_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_NEWSUBGROUPAPI_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                Tabid = paramVar('TABID', 1); Root = paramVar('ROOT', 2); Prmdb = paramVar('PRMDB', 3); Prmgrp = paramVar('PRMGRP', 4); 
                await JSB_MDL_DBNEWSUBGROUP_Sub(Root, Prmdb, Prmgrp, function (_Prmgrp) { Prmgrp = _Prmgrp }); 
                me.Restful_Result = ''; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; 
...
async function JSB_MDL_DBNEWSUBGROUP_Sub(Root, Path, ByRef_Newsubgroupname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DBNEWSUBGROUP_Sub");
    // local variables
    var Dbgroups;
...
async function JSB_MDL_DELETEGROUPAPI_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DELETEGROUPAPI_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                Tabid = paramVar('TABID', 1); Root = paramVar('ROOT', 2); Path = paramVar('PATH', 3); 
                if (Not(await JSB_MDL_DBDELETEGROUP(Root, Path))) { me.Restful_Result = me._activeProcess.At_Errors; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
                me.Restful_Result = ''; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; 
...
async function JSB_MDL_DBDELETEGROUP(Root, Path) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DBDELETEGROUP");
    var Dbgroups = await JSB_MDL_GETDBGROUPSFROMROOT(Root, function (_Root) { Root = _Root }); 
    if (isNothing(Dbgroups)) return false;

    if (await JSB_MDL_DBFINDANDDELETEGROUP(Dbgroups, Path)) {
...
async function JSB_MDL_DBMOVEDATABASEAPI_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DBMOVEDATABASEAPI_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                Tabid = paramVar('TABID', 1); Root = paramVar('ROOT', 2); Frompath = paramVar('FROMPATH', 3); Topath = paramVar('TOPATH', 4); 
                if (Not(await JSB_MDL_DBMOVEDBTOGROUP(Root, Frompath, Topath))) { me.Restful_Result = me._activeProcess.At_Errors; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
                me.Restful_Result = ''; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; 
...
async function JSB_MDL_DBMOVEDBTOGROUP(Root, Dbname, Togroup) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DBMOVEDBTOGROUP");
    var Dbgroups = await JSB_MDL_GETDBGROUPSFROMROOT(Root, function (_Root) { Root = _Root }); 
    if (isNothing(Dbgroups)) return false;

    await JSB_MDL_REMOVETREEVIEWDB(Dbgroups, Dbname);
    if (await JSB_MDL_MOVEDBTOLIST(Dbgroups, Dbname, Togroup)) { await JSB_MDL_UPDATEGROUPFROMPATH(Root, Dbgroups, function (_Root, _Dbgroups) { Root = _Root; Dbgroups = _Dbgroups }); }
...
async function JSB_MDL_RENAMEGROUPAPI_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_RENAMEGROUPAPI_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                Tabid = paramVar('TABID', 1); Root = paramVar('ROOT', 2); Fromname = paramVar('FROMNAME', 3); Toname = paramVar('TONAME', 4); 
                if (Not(await JSB_MDL_DBRENAMEGROUP(Root, Fromname, Toname, function (_Root, _Fromname, _Toname) { Root = _Root; Fromname = _Fromname; Toname = _Toname }))) { me.Restful_Result = me._activeProcess.At_Errors; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
                me.Restful_Result = ''; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; 
...
async function JSB_MDL_DB_RENAMEGROUP(Root, Path, Newname) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_RENAMEGROUP");
    // local variables
    var Dbgroups;
    ...
    if (await JSB_MDL_HASGROUPNAME(Dbgroups, Newname)) { me._activeProcess.At_Errors = '\'' + Newname + '\' already exists'; return false; }

    if (await JSB_MDL_DBFINDANDRENAMEGROUP(Dbgroups, Path, Newname)) {
       await JSB_MDL_UPDATEGROUPFROMPATH(Root, Dbgroups, function (_Root, _Dbgroups) { Root = _Root; Dbgroups = _Dbgroups });
    }
...
async function JSB_MDL_RUNREPORTURL(Tabid, Reportid, Databasename) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_RUNREPORTURL");
    return JSB_BF_JSBROOTACCOUNT() + urlEncode(Reportid) + '?tabID=' + urlEncode(Tabid) + '&databaseName=' + urlEncode(Databasename); 
}
// </RUNREPORTURL>

// <NEWREPORTURL>
async function JSB_MDL_NEWREPORTURL(Tabid, Databasename) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_NEWREPORTURL");
    return JSB_BF_JSBROOTACCOUNT() + 'makenewreport?tabID=' + urlEncode(Tabid) + '&databaseName=' + urlEncode(Databasename); 
}
// </NEWREPORTURL>

// <DB_SHOWTOP1000URL>
async function JSB_MDL_DB_SHOWTOP1000URL(Tabid, Tableorviewname, Databasename) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_SHOWTOP1000URL");
    return JSB_BF_JSBROOTACCOUNT() + 'showTop1000?tabID=' + urlEncode(Tabid) + '&tableID=' + urlEncode(Tableorviewname) + '&databaseName=' + urlEncode(Databasename); 
}
// </DB_SHOWTOP1000URL>

// <DB_GETSUBTREE_Pgm>
async function JSB_MDL_DB_GETSUBTREE_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_GETSUBTREE_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                Tabid = paramVar('TABID', 1); Databasename = paramVar('DATABASENAME', 2); 
                me.Restful_Result = await JSB_MDL_DBGETSUBTREE(Tabid, Databasename); me._state = "RESTFUL_SERVEREXIT"; continue atgoto; 
...
async function JSB_MDL_DBGETSUBTREE(Tabid, Databasename) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DBGETSUBTREE");
    var Preventtimeout = System(56, me); 
    Databasename = Change(Change(Databasename,'[',''),']',''); 

    var Tables = await JSB_MDL_DB_GETLISTOFTABLES(Tabid, Databasename); 
    var Views = await JSB_MDL_DB_GETLISTOFVIEWS(Tabid, Databasename); 
    var Dbbuilderreports = await JSB_MDL_DB_GETLISTOFREPORTS(Tabid, Databasename); 
...
async function JSB_MDL_DB_GETLISTOFCUSTOMSQL(Tabid, Databasename) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_GETLISTOFCUSTOMSQL");
    var Customsqls = {}; 
    ...
    if (CBool(await JSB_ODB_ATTACHDBEXTENDED(Databasename, function (_Databasename) { Databasename = _Databasename }))) {
       if (UCase(await JSB_BF_TYPEOFCONNECTION())!='FILESYSTEM') {
          ...
          if (await JSB_ODB_OPEN('', 'jsbCustomSQL', Fsql, function (_Fsql) { Fsql = _Fsql })); else {
             if (await JSB_ODB_ATTACHDB('')); else return Stop(me._activeProcess.At_Errors);
             Fsql = await JSB_BF_FHANDLE('', 'jsbCustomSQL', true);
          }

          var Prefix = nicename(Databasename) + '_'; 
          var Sl = undefined; 
          ...
          for (Sqlid of iterateOver(Sqllist)) {
             if (LCase(Right(Sqlid,4))=='.sql') {
                ...
                Ontableclick = 'openInTab(' + JSB_BF_JSESCAPESTRING(Tabid) + ', ' + JSB_BF_JSESCAPESTRING(Sqlid) + ', ' + JSB_BF_JSESCAPESTRING(Tabcaption) + ', ' + JSB_BF_JSESCAPESTRING(Url) + '); treeHide();'; 
          ...
          Url = await JSB_MDL_DB_BUILDQRYURL(Tabid, Sqlid, Databasename, function (_Tabid, _Sqlid, _Databasename) { Tabid = _Tabid; Sqlid = _Sqlid; Databasename = _Databasename }); 
          Tabcaption = 'SQL: *NEW*'; 
          Ontableclick = 'openInTab(' + JSB_BF_JSESCAPESTRING(Tabid) + ', ' + JSB_BF_JSESCAPESTRING(Databasename + '_' + Sqlid) + ', ' + JSB_BF_JSESCAPESTRING(Tabcaption) + ', ' + JSB_BF_JSESCAPESTRING(Url) + '); treeHide();'; 
          Customsqls[Sqlid] = {"id":Sqlid,"class":'usrSQL',"onclick":Ontableclick,"data":{"url":Url,"sqlID":Sqlid,"databaseName":Databasename,"tabID":Tabid}}
       }
    }
...
async function JSB_MDL_DB_FDICT_DBBUILDER() { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_FDICT_DBBUILDER");
...
async function JSB_MDL_DB_BUILDQRYURL(ByRef_Tabid, ByRef_Sqlid, ByRef_Databasename, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_BUILDQRYURL");
    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Tabid, ByRef_Sqlid, ByRef_Databasename)
        return v
    }
    return exit(JSB_BF_JSBROOTACCOUNT() + 'sqlQueryBuilder?sqlID=' + urlEncode(CStr(ByRef_Sqlid)) + '&tabID=' + urlEncode(CStr(ByRef_Tabid)) + '&databaseName=' + urlEncode(CStr(ByRef_Databasename))); 
}
// </DB_BUILDQRYURL>

// <DB_GETLISTOFREPORTS>
async function JSB_MDL_DB_GETLISTOFREPORTS(Tabid, Databasename) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_GETLISTOFREPORTS");
    // local variables
    var Reportname, Reportid, Ontableclick;

    var Freports = await JSB_MDL_DB_FDICT_DBBUILDER(); 
    var Rptprefix = nicename(Databasename) + '_'; 
    ...
    Reportlist = Sort(CStr(Reportlist)); 
    var Dbbuilderreports = {}; 
    for (Reportid of iterateOver(Reportlist)) {
       var Url = '', Tabcaption = ''; 
       if (Reportid=='*New Report*') {
          Url = await JSB_MDL_NEWREPORTURL(Tabid, Databasename); 
          Tabcaption = Reportid;
       } else {
          Url = await JSB_MDL_RUNREPORTURL(Tabid, CStr(Reportid), Databasename); 
          Tabcaption = Change(Mid1(Reportid,Prefixlen+1),'_',' ');
       }

       Ontableclick = 'openInTab(' + JSB_BF_JSESCAPESTRING(Tabid) + ', ' + JSB_BF_JSESCAPESTRING(CStr(Reportid)) + ', ' + JSB_BF_JSESCAPESTRING(Tabcaption) + ', ' + JSB_BF_JSESCAPESTRING(Url) + '); treeHide();'; 
...
async function JSB_MDL_DB_GETLISTOFVIEWS(Tabid, Databasename) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_GETLISTOFVIEWS");
    // local variables
    var Row, Viewname, Url, Tabcaption, Ontableclick, Err;

    var Views = {}; 
    try {
       var Systemdbtemplatename = '', Connectionstring = '', Username = '', Password = '', Initialcatalog = ''; 
       if (CBool(await JSB_ODB_ATTACHDBEXTENDED(Databasename, Systemdbtemplatename, Connectionstring, Username, Password, Initialcatalog, function (_Databasename, _Systemdbtemplatename, _Connectionstring, _Username, _Password, _Initialcatalog) { Databasename = _Databasename; Systemdbtemplatename = _Systemdbtemplatename; Connectionstring = _Connectionstring; Username = _Username; Password = _Password; Initialcatalog = _Initialcatalog }))) {
          var Sql = 'USE [' + Initialcatalog + ']; SELECT Name FROM sys.objects WHERE type = \'V\' Order By Name'; 
          var Sl = undefined; 
          ...
          for (Row of iterateOver(Rows)) {
             Viewname = Row.Name; 
             Url = await JSB_MDL_DB_SHOWTOP1000URL(Tabid, CStr(Viewname), Databasename); 
             Tabcaption = 'view: ' + CStr(Viewname); 
             Ontableclick = 'openInTab(' + JSB_BF_JSESCAPESTRING(Tabid) + ', ' + JSB_BF_JSESCAPESTRING(Databasename + '_' + CStr(Viewname)) + ', ' + JSB_BF_JSESCAPESTRING(CStr(Tabcaption)) + ', ' + JSB_BF_JSESCAPESTRING(CStr(Url)) + '); treeHide();'; 
...
async function JSB_MDL_DB_GETLISTOFTABLES(Tabid, Databasename) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_GETLISTOFTABLES");
    // local variables
    var Rec;
    ...
    var Sql = 'use [' + Databasename + ']; SELECT table_name FROM information_schema.tables WHERE table_type = \'base table\'  Order By table_name'; 
    var Sl = undefined; 
    if (await asyncDNOSqlSelect(Sql, _selectList => Sl = _selectList)); else {
       if (await JSB_ODB_ATTACHDB('')); else null;
       return {"Errors":me._activeProcess.At_Errors}
    }
    ...
    for (Rec of iterateOver(Recs)) {
       ...
       if (Locate(Tablename, [undefined, 'master', 'tempdb', 'model', 'msdb', 'MSys', 'sysdiagrams'], 0, 0, 0, "", position => { })); else {
          var Url = await JSB_MDL_DB_SHOWTOP1000URL(Tabid, Tablename, Databasename); 
          var Tabcaption = 'tbl: ' + Tablename; 
          var Ontableclick = 'openInTab(' + JSB_BF_JSESCAPESTRING(Tabid) + ', ' + JSB_BF_JSESCAPESTRING(Databasename + '_' + Tablename) + ', ' + JSB_BF_JSESCAPESTRING(Tabcaption) + ', ' + JSB_BF_JSESCAPESTRING(Url) + '); treeHide();'; 
...
async function JSB_MDL_DB_GETDATABASENAMESWITHOUTPERMISSONS() { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_GETDATABASENAMESWITHOUTPERMISSONS");
    // local variables
    var Databasename;
    ...
    if (await JSB_ODB_ATTACHDB('databaseBuilder_ro')); else return Stop(me._activeProcess.At_Errors);
...
async function JSB_MDL_DB_CLEANJSON(Dbsubgroup) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_CLEANJSON");
    // local variables
    var Tag, Db;
...
async function JSB_MDL_DB_GETDEFINEDDBS(Databasenames, Dbgroups, ByRef_Defineddbs, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_GETDEFINEDDBS");
    // local variables
    ...
    var GROUPNAME_LastI67 = UBound(Dbgroups); 
    for (Grpi = LBound(Dbgroups); Grpi<=GROUPNAME_LastI67; Grpi++) {
       Groupname = Extract(Dbgroups, Grpi, 0, 0); 
       ...
       if (isJSON(Subgroups)) {
          ...
          var DBNAME_LastI70 = UBound(Subgroups); 
          for (Dbi = LBound(Subgroups); Dbi<=DBNAME_LastI70; Dbi++) {
             Dbname = Extract(Subgroups, Dbi, 0, 0); 
...
async function JSB_MDL_DB_GETDATABASENAMES() { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_GETDATABASENAMES");
    // local variables
    var Sl, Rec;
    ...
    if (await asyncDNOSqlSelect(Cmd, _selectList => Sl = _selectList)); else return Stop(me._activeProcess.At_Errors);
...
async function JSB_MDL_DB_UPDATEGROUPFROMPATH(Root, Dbgroupreplacement) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_UPDATEGROUPFROMPATH");
    var Allgroups = undefined; 

    if (Root) {
       Allgroups = await JSB_MDL_DB_GETGROUPSFROMROOT(''); 
       await JSB_MDL_DB_REPLACEGROUP(Root, Allgroups, Dbgroupreplacement);
    } else {
...
async function JSB_MDL_DB_REPLACEGROUP(Path, Dbgroups, Replacement) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_REPLACEGROUP");
    // local variables
    var Tag;

    var Groupname = (LCase(fieldLeft(Path, '|'))); 
    var Subpath = (dropLeft(Path, '|')); 
...
async function JSB_MDL_DB_REMOVEVACANTDATABASESSUBLEVEL(Level, Dbsubgroup, Databasenames, ByRef_Changed, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_REMOVEVACANTDATABASESSUBLEVEL");
    // local variables
    var Tag, Tagi;
    ...
    var TAG_LastI82 = UBound(Dbsubgroup); 
    for (Tagi = LBound(Dbsubgroup); Tagi<=TAG_LastI82; Tagi++) {
       Tag = Extract(Dbsubgroup, Tagi, 0, 0); 
       var V = Dbsubgroup[Tag]; 
...
async function JSB_MDL_DB_GETGROUPSFROMROOT(Root) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_GETGROUPSFROMROOT");
    // local variables
    var Dbgroups;
    ...
    if (HasTag(Dbgroups,'ItemID')) { me._activeProcess.At_Errors = 'Corrupt jsb_config dbBuilderDBGroups'; return undefined; }

    await JSB_MDL_DB_CLEANJSON(Dbgroups);

    if (Root) Dbgroups = await JSB_MDL_DB_FINDROOT(Dbgroups, Root);
...
async function JSB_MDL_DB_FINDROOT(Dbgroups, Path) { 
    var me = new jsbRoutine("JSB_MDL", "DB_SERVER", "JSB_MDL_DB_FINDROOT");
    // local variables
    var Tag;
...
async function JSB_MDL_DEFAULTCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "defaultcolumn", "JSB_MDL_DEFAULTCOLUMN_Sub");
    // local variables
    ...
    Columnidx = await JSB_MDL_FINDVIEWCOLUMN(Dataset, ByRef_Columnid, function (_Dataset) { Dataset = _Dataset }); 
    if (Columnidx=='0') return exit(undefined);
    Newvalue = Trim(await JSB_BF_INPUTBOX('Column Property ' + CStr(ByRef_Columnid), 'Default Value', CStr(Dataset.columns[Columnidx].defaultvalue), '80%', 'auto', function (_P1) {  })); 
...
async function JSB_MDL_DELETENONESSENTIALFILES_Sub(ByRef_Projectname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "deletenonessentialfiles", "JSB_MDL_DELETENONESSENTIALFILES_Sub");
    // local variables
    ...
    if (await JSB_ODB_SELECTTO('', Scp, '', Sl, function (_Sl) { Sl = _Sl })); else return Stop(me._activeProcess.At_Errors);
    
    while (true) {
       Id = readNext(Sl).itemid; 
       if (CBool(Id)); else break
       if (Not(true)) break; 
       if (InStr1(1,LCase(Id),'_custom')==0) { if (await JSB_ODB_DELETEITEM(Scp, CStr(Id))); else return Stop(me._activeProcess.At_Errors); }
    } 
    return exit(); 
}
// </DELETENONESSENTIALFILES_Sub>
...
async function JSB_MDL_DELETEPAGE_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "deletepage", "JSB_MDL_DELETEPAGE_Pgm");
    // local variables
    var Top, Columns, Dict, Projectname, Filterby, Orderby, Options;
    var Allitems, F, Itemlist, Ans, Pagename;

    // %options aspxC-
    if (!(await JSB_BF_ISMANAGER())) return Stop('You must be at least a data manager to run program');

    if (!(await JSB_BF_PARSESELECT(me._activeProcess.At_Sentence, '', Top, Columns, Dict, Projectname, Filterby, Orderby, Options, Allitems, F, undefined, undefined, undefined, function (_Top, _Columns, _Dict, _Projectname, _Filterby, _Orderby, _Options, _Allitems, _F) { Top = _Top; Columns = _Columns; Dict = _Dict; Projectname = _Projectname; Filterby = _Filterby; Orderby = _Orderby; Options = _Options; Allitems = _Allitems; F = _F }))) return Stop();
    Itemlist = getList(0); 
...
async function JSB_MDL_DELETEPAGEMODEL(ByRef_Projectname, ByRef_Ipagename, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "deletepagemodel", "JSB_MDL_DELETEPAGEMODEL");
    // local variables
    ...
    if (await JSB_ODB_DELETEITEM(await JSB_BF_FHANDLE('', ByRef_Projectname), CStr(Pagename))); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;

    Fdict = await JSB_BF_FHANDLE('DICT', ByRef_Projectname); 
    await JSB_BF_TRASHIT('dict ' + CStr(ByRef_Projectname), CStr(Pagename) + '.page');
    if (await JSB_ODB_DELETEITEM(Fdict, CStr(Pagename) + '.page')); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;
    if (await JSB_ODB_DELETEITEM(Fdict, CStr(Pagename) + '.pcd')); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;

    if (await JSB_ODB_DELETEITEM(await JSB_BF_FHANDLE('', 'md'), CStr(Pagename))); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;
...
async function JSB_MDL_DELETEPAGEMODEL_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "DELETEPAGEMODEL", "JSB_MDL_DELETEPAGEMODEL_Pgm");
    // local variables
    ...
    if (Not(Frompage) && !JSB_BF_HASPARENTPROCESS()) At_Server.End();
    return At_Response.Redirect(JSB_BF_JSBROOTEXECUTE('menu'));
}
// </DELETEPAGEMODEL_Pgm>

// <DELETERPTAPI_Pgm>
async function JSB_MDL_DELETERPTAPI_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "deleterptapi", "JSB_MDL_DELETERPTAPI_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                Tabid = paramVar('TABID', 1); Databasename = paramVar('DATABASENAME', 2); Reportname = paramVar('REPORTNAME', 3); Path = paramVar('PATH', 4); 
                // %options aspxC-
            
                if (Left(Reportname,Len(Databasename)+1)!=CStr(Databasename) + '_') { me.Restful_Result = {"AtErrors":'bad report name prefix'}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
                if (LCase(Right(Reportname,7))!='_report') { me.Restful_Result = {"AtErrors":'bad report name suffix'}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
            
                Success_Editrecord = await JSB_MDL_DELETEPAGEMODEL('dbBuilderReports', CStr(Reportname) + '_editRecord', function (_P1, _P2) {  }); 
                Success_Grid = await JSB_MDL_DELETEPAGEMODEL('dbBuilderReports', CStr(Reportname) + '_Grid', function (_P1, _P2) {  }); 
                Success_Pivot = await JSB_MDL_DELETEPAGEMODEL('dbBuilderReports', CStr(Reportname) + '_Pivot', function (_P1, _P2) {  }); 
                Success_Pivotsetup = await JSB_MDL_DELETEPAGEMODEL('dbBuilderReports', CStr(Reportname) + '_PivotSetup', function (_P1, _P2) {  }); 
            
                // Create file if necessary
                Freports = await JSB_MDL_FDICTDBBUILDER(); 
                Rptprefix = nicename(CStr(Reportname)); 
            
                if (await JSB_ODB_SELECTTO('', Freports, 'ItemID like \'' + CStr(Rptprefix) + ']\'', Sl, function (_Sl) { Sl = _Sl })); else { me.Restful_Result = {"AtErrors":me._activeProcess.At_Errors}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
                Rptitems = getList(Sl); 
            
                for (Rptname of iterateOver(Rptitems)) {
                   if (await JSB_ODB_DELETEITEM(Freports, CStr(Rptname))); else return Stop(me._activeProcess.At_Errors);
                } 
            
                me.Restful_Result = {"success":true}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; 
...
async function JSB_MDL_DELETEVIEW(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Confirm, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "deleteview", "JSB_MDL_DELETEVIEW");
    // local variables
    var Fdict, Id, Ext;
    ...
    if (await JSB_ODB_DELETEITEM(Fdict, CStr(ByRef_Viewname) + '.view')); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;

    await JSB_BF_TRASHIT('dict ' + CStr(ByRef_Projectname), CStr(ByRef_Viewname) + '.htm');
    if (await JSB_ODB_DELETEITEM(Fdict, CStr(ByRef_Viewname) + '.htm')); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;

    if (await JSB_ODB_DELETEITEM(Fdict, CStr(ByRef_Viewname) + '.pcd')); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;
    if (await JSB_ODB_DELETEITEM(await JSB_BF_FHANDLE('MD'), CStr(ByRef_Viewname))); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;

    // cleaup .pcd, .pcs, .pcf files:  (Anything the starts, contains, or endswith viewname
    for (Id of iterateOver(await JSB_BF_FSELECT('dict ' + CStr(ByRef_Projectname), 'ItemID like \'' + CStr(ByRef_Viewname) + '_%\' Or ItemID like \'%_' + CStr(ByRef_Viewname) + '_%\' Or ItemID like \'%_' + CStr(ByRef_Viewname) + '\'', undefined, undefined, function (_P1) {  }))) {
       Ext = LCase(Right(Id,4)); 
       if (Ext=='.pcd' || Ext=='.pcs' || Ext=='.pcf') {
          if (await JSB_ODB_DELETEITEM(Fdict, CStr(Id))); else Alert(me._activeProcess.At_Errors);
       }
    } 
...
async function JSB_MDL_DELETEVIEWBTN(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Btntext, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "deleteviewbtn", "JSB_MDL_DELETEVIEWBTN");
    // local variables
...
async function JSB_MDL_DELETEVIEWCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, ByRef_Includedb, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "deleteviewcolumn", "JSB_MDL_DELETEVIEWCOLUMN_Sub");
    // local variables
          ...
          if (await JSB_ODB_SELECTTO('ItemID,*', X, '', Sl, function (_Sl) { Sl = _Sl })) {
             ...
             for (Item of iterateOver(Items)) {
                if (HasTag(Item,ByRef_Columnid)) {
                   ...
                   delete Item['ItemID']
                   if (await JSB_ODB_WRITEJSON(Item, X, CStr(Itemid))); else Alert(me._activeProcess.At_Errors);
                }
             }
          }
       }
...
async function JSB_MDL_DELETEWEBSITEMODEL_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "deletewebsitemodel", "JSB_MDL_DELETEWEBSITEMODEL_Pgm");
    // local variables
    var Ans;
    ...
    Ans = await JSB_BF_MSGBOX('DELETE ACCOUNT', 'Are you sure you want to completely DELETE the website: ' + JSB_BF_JSBACCOUNT(), 'Yes,No'); 
    if (Ans=='Yes') {
       if (await JSB_BF_DELETEACCOUNT(JSB_BF_JSBACCOUNT(), function (_P1) {  })) return At_Response.Redirect(JSB_BF_LOGOUTURL());
       await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
    }
    return; 
}
...
async function JSB_MDL_DEVXPIVOTSETUP_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "devxpivotsetup", "JSB_MDL_DEVXPIVOTSETUP_Pgm");
    // local variables
    var Rolemsg, Databasename, Tableid, Tabid, Pivotlayout, S;
    var Cmd, Issqlselect, Reportname;

    // %options aspxC-
    if (!JSB_BF_ISLOCALHOST()) {
       Rolemsg = (JSB_BF_ISAUTHENTICATED()?'':'You must be logged in to view this page. ' + JSB_HTML_ANCHOR(JSB_BF_LOGINURL(), 'Click here to login')); 
    ...
    if (Not(Tableid)) {
       S = RTrim(Field(me._activeProcess.At_Sentence,'(',1)); 
       Cmd = Field(S,' ',1); 
    ...
    if (await JSB_ODB_WRITEJSON(Pivotlayout, await JSB_BF_FHANDLE('tmp'), CStr(Reportname))); else return Stop(me._activeProcess.At_Errors);

    if (JSB_BF_HASTCLPARENT() || JSB_BF_HASPARENTPROCESS()) return Stop(); else At_Server.End();
    return; 
}
// </DEVXPIVOTSETUP_Pgm>
...
async function JSB_MDL_DISPLAY_EXCELUPLOAD(Viewvars) { 
    var me = new jsbRoutine("JSB_MDL", "DISPLAY_EXCELUPLOAD", "JSB_MDL_DISPLAY_EXCELUPLOAD");
    var Aryhtml = [undefined, JSB_BF_HTML('\<div class="container"\>')]; var Row = Viewvars.Row; var Ctlhtml = ''; 

    Aryhtml[Aryhtml.length] = JSB_BF_HTML('\<div class="form-group row"\>'); 

    Ctlhtml = JSB_HTML_CHECKBOX('DataMustHaveOneNumericRow', CStr(true), '', CNum((Row['DataMustHaveOneNumericRow']?Row['DataMustHaveOneNumericRow']:1)), ''); 
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('Rows have some numeric data', Ctlhtml, 'DataMustHaveOneNumericRow', 0, 0, ''); 

    Ctlhtml = JSB_HTML_CHECKBOX('SideBySideTables', CStr(true), '', CNum(Row['SideBySideTables']), ''); 
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('Allow Side By Side Tables', Ctlhtml, 'SideBySideTables', 0, 0, ''); 

    Ctlhtml = JSB_HTML_CHECKBOX('ignoreTablesWithNoRows', CStr(true), '', CNum((Row['ignoreTablesWithNoRows']?Row['ignoreTablesWithNoRows']:1)), ''); 
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('ignore Tables With No Rows', Ctlhtml, 'ignoreTablesWithNoRows', 0, 0, ''); 

    Ctlhtml = JSB_HTML_INTEGERBOX('ignoreTablesWithLessThanNColumns', CStr(Row['ignoreTablesWithLessThanNColumns']), false, [undefined, 'onchange=\'parsleyReset(this)\'', 'placeholder="ignore Tables With Less Than N-Columns"', 'parsley-trigger="change focusout"', 'min="0"', 'max="10"', 'data-parsley-type="integer"', 'type=\'number;number\''], '0', '10'); 
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('ignore Tables With Less Than N-Columns', Ctlhtml, 'ignoreTablesWithLessThanNColumns', 0, 0, ''); 

    Ctlhtml = JSB_HTML_CHECKBOX('OneTablePerSheet', CStr(true), '', CNum((Row['OneTablePerSheet']?Row['OneTablePerSheet']:1)), ''); 
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('OneTablePerSheet', Ctlhtml, 'OneTablePerSheet', 0, 0, ''); 

    Ctlhtml = JSB_HTML_CHECKBOX('AllowBlankRows', CStr(true), '', CNum((Row['AllowBlankRows']?Row['AllowBlankRows']:1)), ''); 
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('Allow Blank Rows', Ctlhtml, 'AllowBlankRows', 0, 0, ''); 

    Ctlhtml = JSB_HTML_CHECKBOX('DefaultEmptyValuesFromPreviousRow', CStr(true), '', CNum(Row['DefaultEmptyValuesFromPreviousRow']), ''); 
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('Default Empty Values From Previous Row', Ctlhtml, 'DefaultEmptyValuesFromPreviousRow', 0, 0, ''); 

    Ctlhtml = JSB_HTML_CHECKBOX('deleteAndRecreateTables', CStr(true), '', CNum(Row['deleteAndRecreateTables']), ''); 
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('delete And Recreate Sql Table', Ctlhtml, 'deleteAndRecreateTables', 0, 0, ''); 

    Ctlhtml = JSB_HTML_CHECKBOX('showTables', CStr(true), '', CNum((Row['showTables']?Row['showTables']:1)), ''); 
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('show Table', Ctlhtml, 'showTables', 0, 0, ''); 

    Aryhtml[Aryhtml.length] = JSB_BF_HTML('\</div\>'); 
    Aryhtml[Aryhtml.length] = JSB_BF_HTML('\<div class="form-group row"\>'); 
    ...
    Aryhtml[Aryhtml.length] = JSB_BF_LBLCTLSET('Put tables into the Database', Ctlhtml, 'commonDatabaseName', 1, 0, ''); 
...
async function JSB_MDL_DISPLAY_EXCELUPLOAD_EXTRAS(Viewvars, Html) { 
    var me = new jsbRoutine("JSB_MDL", "DISPLAY_EXCELUPLOAD_EXTRAS", "JSB_MDL_DISPLAY_EXCELUPLOAD_EXTRAS");
    ...
    if (Not(Viewvars.currentID)) {
       ...
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINBUTTONCONTEXTMENU('ExcelUpload', 'formBtn_Save'); 
       Toolbar = CStr(Toolbar) + Btnhtml;
    }
    ...
    if (CBool(Viewvars.currentID)) {
       ...
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINBUTTONCONTEXTMENU('ExcelUpload', 'formBtn_Update'); 
       Toolbar = CStr(Toolbar) + Btnhtml;
    }
    ...
    Jscode = JSB_BF_GENEVENTHANDLER('Cancel', 'ExcelUpload' + Urlparams, 10, '', 0, false); 
    Jscall = 'window.eventHandler_Cancel(jsbFormVars(), "ItemID" /* primary Key */);'; 
    Jscall = '$("form").parsley().destroy(); ' + Jscall; 
    Btnhtml = JSB_HTML_BUTTON('formBtn_Cancel', 'Cancel', {"onclick":Jscall,"style":''}) + Jscode; 
    Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINBUTTONCONTEXTMENU('ExcelUpload', 'formBtn_Cancel'); 
    Toolbar = CStr(Toolbar) + Btnhtml; 

    Toolbar = CStr(Toolbar) + CStr(Viewvars.extraToolBarHtml); 

    var Header = JSB_BF_HTML('\<div id=\'header_ExcelUpload\' class=\'HeaderStyle\'\>') + 'ExcelUpload' + JSB_BF_HTML('\</div\>'); 
    Html = JSB_HTML_ROWS2('46px', Header, '%', Html, 'overflow: hidden', 'overflow: auto'); 

    if (await JSB_BF_ISMANAGER()) {
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'DataMustHaveOneNumericRow', 'ctl_checkbox', 0, -1, 0, 0, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'SideBySideTables', 'ctl_checkbox', 0, -1, 0, 0, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'ignoreTablesWithNoRows', 'ctl_checkbox', 0, -1, 0, 0, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'ignoreTablesWithLessThanNColumns', 'textbox', 0, -1, 0, 0, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'OneTablePerSheet', 'ctl_checkbox', 0, -1, 0, 0, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'AllowBlankRows', 'ctl_checkbox', 0, -1, 0, 0, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'DefaultEmptyValuesFromPreviousRow', 'ctl_checkbox', 0, -1, 0, 0, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'deleteAndRecreateTables', 'ctl_checkbox', 0, -1, 0, 0, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'showTables', 'ctl_checkbox', 0, -1, 0, 0, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINCONTROLCONTEXTMENU('ExcelUpload', 'commonDatabaseName', 'textbox', 0, -1, 0, 1, 0); 
       Contextmenus = CStr(Contextmenus) + JSB_BF_ADMINVIEWCONTEXTMENU('ExcelUpload', CNum(Viewvars.showAddRmvViews)+0); 
       Html = CStr(Html) + JSB_HTML_HIDDEN('contextMenuText') + JSB_HTML_HIDDEN('contextMenuID');
    }
    Html = CStr(Html) + JSB_HTML_SCRIPT(Contextmenus); 

    if (window.JSB_MDL_VIEW_EXCELUPLOAD_DISPLAY_CC_Sub) await JSB_MDL_VIEW_EXCELUPLOAD_DISPLAY_CC_Sub(Viewvars, Html, Toolbar, function (_Html, _Toolbar) { Html = _Html; Toolbar = _Toolbar }); 

    if (Toolbar) {
       Toolbar = JSB_BF_HTML('\<div class=\'BOBlock_ToolBar\'\>') + Toolbar + JSB_BF_HTML('\</div\>'); 
       if (CBool(Viewvars.parentMultiView)) {
          Html = JSB_BF_HTML('\<div class="BOBlock ExcelUpload"\>' + crlf) + Html + Toolbar + JSB_BF_HTML('\</div\>' + crlf);
       } else {
          Html = JSB_BF_HTML('\<div class="BOBlock FullHeight ExcelUpload"\>' + crlf) + JSB_HTML_ROWS2('%', Html, '66px', Toolbar, 'overflow-y:auto', 'overflow:hidden') + JSB_BF_HTML('\</div\>' + crlf);
       }
    } else {
       Html = JSB_BF_HTML('\<div class="BOBlock ExcelUpload"\>' + crlf) + Html + JSB_BF_HTML('\</div\>' + crlf);
    }

    // Output Refresh if needed
    if (CBool(Viewvars.masterview)) {
       // Output Code that will enable us to refresh() if needed by our childer
       Html = CStr(Html) + JSB_HTML_SCRIPT('function refreshData() { if (window.ExcelUpload_refresh) window.ExcelUpload_refresh() }'); ;
    } else if (CBool(Viewvars.ParentRefresh)) {
       Viewvars.ParentRefresh = false; 
       Html = CStr(Html) + JSB_HTML_SCRIPT('if (parent.refreshData) parent.refreshData();');
    }
...
async function JSB_MDL_DOWIZARD_Sub(ByRef_Viewname, ByRef_Dataset, ByRef_Totype, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "dowizard", "JSB_MDL_DOWIZARD_Sub");
    // local variables
    ...
    Dictcolumns = await JSB_BF_GETTABLECOLUMNDEFS(ByRef_Dataset.tableName, CStr(false), true, function (_P1) {  }); // Suppress Messages, doRefresh
...
async function JSB_MDL_DROPGRIDCOLUMNS(ByRef_Columns, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "dropgridcolumns", "JSB_MDL_DROPGRIDCOLUMNS");
    // local variables
...
async function JSB_MDL_EDC_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "edc", "JSB_MDL_EDC_Pgm");
    // local variables
    var Projectname, Viewname, Frompage, Iniframe, Columnid, Closewindow;
    ...
    if (await JSB_ODB_OPEN('', CStr(Projectname), me._activeProcess.At_File, function (_At_File) { me._activeProcess.At_File = _At_File })); else return Stop(At(-1), 'File/Project ', Projectname, ' not found');
...
async function JSB_MDL_EDITPAGEMODEL_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "editpagemodel", "JSB_MDL_EDITPAGEMODEL_Pgm");
    // local variables
    ...
    if (CBool(Frompage)) return At_Response.Redirect(Frompage);
    if (JSB_BF_HASTCLPARENT()) return Stop();
    At_Server.End(); 
    return; 
}
// </EDITPAGEMODEL_Pgm>
...
async function JSB_MDL_EDITPAGEMODEL_Sub(ByRef_Projectname, ByRef_Pagename, Showbackbtn, Quickrtn, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "EDITPAGEMODEL", "JSB_MDL_EDITPAGEMODEL_Sub");
    // local variables
    var Originaldb, Pagetype, Isnew, Pagedataset, Ans, Viewname;
    var Objectmodel, Html, Toolbar, Parentrefresh, Cmd, Clonepagename;
    ...
    do {
       ...
       Objectmodel = await JSB_MDL_MODELFORPAGE(ByRef_Projectname, function (_ByRef_Projectname) { ByRef_Projectname = _ByRef_Projectname }); 
       await JSB_MDL_GETEXTRAPAGECOLUMNS_Sub(Objectmodel.columns, Pagedataset.templateName, function (_P1, _P2) {  }); 

       Html = await JSB_MDL_FORMVIEWNCOLUMNS(CStr(ByRef_Projectname), Objectmodel, CStr(false), Pagedataset, CStr(Viewname)); 
       ...
       Print(ClearScreen(), JSB_HTML_ROWS2('%', CStr(Html), '66px', CStr(Toolbar))); 
       ...
       break; 
          ...
          case 'CC': 
             ...
             if (CBool(Isnew)) {
                if (await JSB_ODB_DELETEITEM(await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Pagename))); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;
                if (await JSB_ODB_DELETEITEM(await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Pagename) + '.view')); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;
                if (await JSB_ODB_DELETEITEM(await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Pagename) + '_west.view')); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);;
                if (await JSB_ODB_DELETEITEM(await JSB_BF_FHANDLE('dict', ByRef_Projectname), CStr(ByRef_Pagename) + '.page')); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors);
             }
             if (await JSB_ODB_ATTACHDB(CStr(Originaldb))); else null;
             return exit(undefined); 
       ...
       break; 
          ...
          case 'C': 
             ...
             if (CBool(Clonepagename)) { await JSB_MDL_RESPONSEREDIRECT(JSB_BF_JSBROOTEXECUTE(CStr(Clonepagename)), function (_P1) {  }); }
       ...
       break; 
          ...
          case 'S': case 'G': 
             Print(ClearScreen()); 
             if (await JSB_ODB_WRITEJSON(Pagedataset, await JSB_BF_FHANDLE('dict', ByRef_Projectname), CStr(ByRef_Pagename) + '.page')); else return Stop(At(-1), 'Edp-', System(28, me), ': ', me._activeProcess.At_Errors);
             ...
             if (Cmd=='S') {
                ...
                return At_Response.Redirect(JSB_BF_JSBROOTEXECUTE(CStr(ByRef_Pagename)));
             }
       ...
       break; 
          ...
          case 'R': 
             ...
             Newpagename = Trim(await JSB_BF_INPUTBOX('Rename', 'Enter a new name', '', '80%', '', function (_P1) {  })); 
...
async function JSB_MDL_EDITREPORTURL(ByRef_Tabid, ByRef_Reportid, ByRef_Databasename, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "editreporturl", "JSB_MDL_EDITREPORTURL");
    // local variables
    var Url;
    ...
    Url = JSB_BF_JSBROOTACCOUNT() + '?edp dbBuilderReports'; 
    if (CBool(ByRef_Reportid)) Url = CStr(Url) + ' ' + CStr(ByRef_Reportid);
    Url = (CStr(Url) + ' (' + '&tabID=' + urlEncode(CStr(ByRef_Tabid)) + '&databaseName=' + urlEncode(CStr(ByRef_Databasename))); 
    return exit(Url); 
}
// </EDITREPORTURL>
...
async function JSB_MDL_EDITTABLE_Sub(ByRef_Tablename, ByRef_Showtablelist, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "edittable", "JSB_MDL_EDITTABLE_Sub");
    // local variables
    var Fdict, Metadata, Row, Options, Mydata, Header, Jqgrid;
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                ...
                Options = {"HeadForeColor":'blue',"HeadBackColor":'yellow',"allowUpdates":true,"allowDeletes":true,"allowInserts":true,"width100percent":true}; 
            
                
            case "STARTOVER": 
                
                Mydata = await JSB_BF_GETTABLECOLUMNDEFS(ByRef_Tablename, CStr(false), true, function (_ByRef_Tablename) { ByRef_Tablename = _ByRef_Tablename }); 
                ...
                Header = JSB_HTML_CENTER('Editing Meta Data for table ' + JSB_HTML_BOLD(CStr(ByRef_Tablename))); 
                Header = CStr(Header) + crlf; 
                Header = CStr(Header) + JSB_HTML_CENTER(JSB_HTML_SUBMITBTN('btn', 'Save') + ' ' + JSB_HTML_SUBMITBTN('btn', 'Cancel')); 
            
                
            case "TOP": 
                
                Jqgrid = JSB_HTML_JQGRID('myGrid', Mydata, Metadata, Options); 
                ...
                for (Cmd of iterateOver(Ops)) {
                   ...
                   if (Cmd.op=='delete') {
                      if (Left(Cmd.id,1)!='-') {
                         if (await JSB_ODB_DELETEITEM(Fdict, '!' + CStr(Cmd.row[Pkname]))); else await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
                      }
                   } else {
                      ...
                      if (Cmd.op=='new') {
                         if (await JSB_ODB_WRITE(CStr(Row), Fdict, CStr(Pkid))); else await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
                      } else if (Cmd.op=='update') {
                         if (Left(Cmd.id,1)=='-') {
                            if (await JSB_ODB_WRITE(CStr(Row), Fdict, CStr(Pkid))); else await JSB_BF_MSGBOX(me._activeProcess.At_Errors);
                         } else {
                            if (Originalpkid!=Pkid && !isEmpty(Originalpkid)) { if (await JSB_ODB_DELETEITEM(Fdict, '!' + CStr(Originalpkid))); else await JSB_BF_MSGBOX(me._activeProcess.At_Errors); }
...
async function JSB_MDL_EDITVIEWBTN(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Btntext, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "editviewbtn", "JSB_MDL_EDITVIEWBTN");
    // local variables
...
async function JSB_MDL_EDITVIEWCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, ByRef_Iniframe, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "editviewcolumn", "JSB_MDL_EDITVIEWCOLUMN_Sub");
    // local variables
    var Viewdataset, Tablename, Isgrid, Canmodify, Changed, Viewrow;
    var Columnidx, Viewmodel, Callname, Subname, Html, Toolbar;
    ...
    if (Not(Canmodify)) {
       ...
       if (CBool(Changed)) { if (await JSB_ODB_WRITEJSON(Viewdataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname))); else return Stop(me._activeProcess.At_Errors); }
    }
    ...
    do {
       ...
       // Display the record
       Html = await JSB_MDL_FORMVIEWNCOLUMNS(CStr(ByRef_Projectname), Viewmodel, CStr(false), Viewrow, CStr(ByRef_Viewname)); 
       ...
       if (CBool(ByRef_Iniframe)) {
          ...
          } else if (JSB_BF_HASPARENTPROCESS()) {
       ...
       Html = JSB_HTML_ROWS2('%', CStr(Html), '33px', CStr(Toolbar)); 
       ...
       break; 
          ...
          case 'SV': 
             ...
             if (CBool(Tablename)) {
                ...
                await JSB_MDL_STANDARDUNLOAD(JSB_BF_TABLECOLUMNS(), Tablerow);
                if (await JSB_ODB_WRITE(CStr(Tablerow), await JSB_BF_FHANDLE('DICT', Tablename), '!' + CStr(ByRef_Columnid))); else return Stop(At(-1), 'edc-', System(28, me), ': ', me._activeProcess.At_Errors);
             }

             // Save view column
             if (await JSB_ODB_WRITEJSON(Viewdataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname))); else return Stop(At(-1), 'edc-', System(28, me), ': ', me._activeProcess.At_Errors);
...
async function JSB_MDL_EDITVIEWCOLUMN_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "EDITVIEWCOLUMN", "JSB_MDL_EDITVIEWCOLUMN_Pgm");
    // local variables
    var Viewname, Columnid, Projectname, Iniframe, Frompage, Closewindow;
    var Pagename;
...
async function JSB_MDL_EDITVIEWMODEL_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Showbackbtn, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "editviewmodel", "JSB_MDL_EDITVIEWMODEL_Sub");
    // local variables
    ...
    do {
       if (System(1, me)=='aspx') {
          if (await JSB_ODB_ATTACHDB(CStr(Dataset.attachdb))); else { Println(Alert('edv-' + CStr(System(28, me)) + ': Unable to attach to ' + CStr(Dataset.attachdb) + ' in view ' + CStr(ByRef_Viewname))); debugger; }
       }
       ...
       if (Null0(Dataset.tableName)!=Tablename) {
          if (await JSB_ODB_WRITEJSON(Dataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname))); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
          Tablename = Dataset.tableName;
       }
       ...
       break; 
          ...
          case '#': 
             Niceobjectname = nicename(dropIfRight(CStr(ByRef_Viewname), '.view', true)); 
       ...
       break; 
          ...
          case 'S': 
             Print(ClearScreen()); 
             if (await JSB_ODB_WRITE(CStr(Dataset), await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname))); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
             await JSB_MDL_REGENPAGEMODEL_Sub(ByRef_Projectname, dropIfRight(CStr(ByRef_Viewname), '.view', true), false); // Was GenerateView

             if (JSB_BF_HASTCLPARENT() || JSB_BF_HASPARENTPROCESS()) return exit(undefined);
       ...
       break; 
          ...
          case 'R': 
             if (await JSB_MDL_RENAMEVIEW(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, '', function (_ByRef_Projectname, _ByRef_Pagename, _ByRef_Viewname, _P4) { ByRef_Projectname = _ByRef_Projectname; ByRef_Pagename = _ByRef_Pagename; ByRef_Viewname = _ByRef_Viewname })) {
                if (await JSB_ODB_READJSON(Dataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname), function (_Dataset) { Dataset = _Dataset })); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
                Parentrefresh = 1;
             }
...
async function JSB_MDL_EDITVIEWMODEL_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "EDITVIEWMODEL", "JSB_MDL_EDITVIEWMODEL_Pgm");
    // local variables
    var Viewname, Projectname, Frompage, Closewindow, Pagename;
...
async function JSB_MDL_EDITVIEWMODELCOLUMNS_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Showbackbtn, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "editviewmodelcolumns", "JSB_MDL_EDITVIEWMODELCOLUMNS_Sub");
    // local variables
    ...
    Url = (JSB_BF_JSBRESTCALL('editViewColumn?viewname=' + urlEncode(CStr(ByRef_Viewname)) + '&id={id}&projectname=' + urlEncode(CStr(ByRef_Projectname)) + '&inIFrame=1')); 
    Containername = 'newDiv' + CStr(Rnd(9999)); 
    Onselect = JSB_BF_GENEVENTHANDLER('Select', CStr(Url), 4, CStr(Containername)); // 4 Open In An Iframe Whose Id Is containerName
    ...
    Jqgridrestcall = (JSB_BF_JSBRESTCALL('fetchViewColumns?viewname=' + urlEncode(CStr(ByRef_Viewname)) + '&projectname=' + urlEncode(CStr(ByRef_Projectname)))); 
    Jq = JSB_HTML_JQGRID('jqGrid001', Jqgridrestcall, Viewmodel.columns, {}) + CStr(Onselect); 
    ...
    do {
       ...
       Th = JSB_HTML_ROWS2('%', CStr(Jq), '6em', JSB_HTML_CENTER(CStr(Toolbar))); 
       Print(ClearScreen(), JSB_HTML_ROWS2('50%', CStr(Th), '50%', JSB_HTML_IFRAME(CStr(Containername), ''))); 
...
async function JSB_MDL_EDP_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "edp", "JSB_MDL_EDP_Pgm");
    // local variables
    ...
    if (await JSB_ODB_OPEN('', CStr(Projectname), me._activeProcess.At_File, function (_At_File) { me._activeProcess.At_File = _At_File })); else {
       ...
       if (await JSB_ODB_OPEN('', CStr(Projectname), me._activeProcess.At_File, function (_At_File) { me._activeProcess.At_File = _At_File })); else return Stop('I\'m sorry, I wasn\'t able to create that project');
    }
    ...
    if (JSB_BF_HASTCLPARENT()) return Stop('Done');
    At_Server.End(); 
    return; 
}
// </EDP_Pgm>
...
async function JSB_MDL_EDP_PICK_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "edp_pick", "JSB_MDL_EDP_PICK_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                Fromurl = paramVar('FROMURL', 1); Fromvalue = paramVar('FROMVALUE', 2); 
                // %options aspxC-
                Projectname = paramVar('projectName'); 
                ...
                if (CBool(Newfromvalue)) Newfromvalue = CStr(Newfromvalue) + '.page'; else Newfromvalue = Chr(27);
...
async function JSB_MDL_EDT_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "edt", "JSB_MDL_EDT_Pgm");
    // local variables
    var Paramvars, Tablename, Frompage, Closewindow, Sentence;
...
async function JSB_MDL_EDTBCODE(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Btntext, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "edtbcode", "JSB_MDL_EDTBCODE");
    // local variables
    var Objectmodel, Niceviewname, Btni, Btn, Code;
    ...
    if (await JSB_ODB_READJSON(Objectmodel, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname), function (_Objectmodel) { Objectmodel = _Objectmodel })); else { Println(Alert('Unable to find view ' + CStr(ByRef_Viewname))); debugger; return exit(false); }
    Niceviewname = nicename(dropIfRight(CStr(ByRef_Viewname), '.view', true)); 
    ...
    if (isEmpty(Btn.customcall)) {
       ...
       Btn.customcall = CStr(Niceviewname) + '_' + nicename(CStr(ByRef_Btntext)) + '_button'; 
       if (await JSB_ODB_WRITEJSON(Objectmodel, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname))); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
       await JSB_MDL_REGENPAGEMODEL_Sub(ByRef_Projectname, ByRef_Pagename, false);
    }

    if (await JSB_ODB_READ(Code, await JSB_BF_FHANDLE('', ByRef_Projectname), dropIfRight(CStr(ByRef_Pagename), '.page', true), function (_Code) { Code = _Code })); else return exit(false);
    if (InStr1(1,LCase(Code),'function ' + LCase(Btn.customcall) + '(')==0) {
       Code = Replace(Code,-1,0,0,''); 
       Code = Replace(Code,-1,0,0,'function ' + CStr(Btn.customcall) + '(viewVars, rtnErrors)'); 
       Code = Replace(Code,-1,0,0,'   return false ;* stops any transfers'); 
       Code = Replace(Code,-1,0,0,'end function'); 
       if (await JSB_ODB_WRITE(CStr(Code), await JSB_BF_FHANDLE('', ByRef_Projectname), dropIfRight(CStr(ByRef_Pagename), '.page', true))); else { Println(Alert(me._activeProcess.At_Errors)); debugger; }
    }

    await JSB_BF_POPOUTWINDOW(CStr(ByRef_Viewname) + ' Custom Code', JSB_BF_JSBROOTEXECUTE('ed ' + CStr(ByRef_Projectname) + ' \'' + dropIfRight(CStr(ByRef_Pagename), '.page', true) + '\''), true, '80%', '80%');
    return exit(true); 
}
// </EDTBCODE>
...
async function JSB_MDL_EDV_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "edv", "JSB_MDL_EDV_Pgm");
    // local variables
    var Projectname, Viewname, Frompage, Closewindow, Pagename;
    ...
    if (isEmpty(Projectname) && isEmpty(Viewname) && InStr1(1,me._activeProcess.At_Sentence,'=')==0) {
       Sentence = Field(me._activeProcess.At_Sentence,'(',1); 
       Projectname = Field(Sentence,' ',2); 
    ...
    if (await JSB_ODB_READJSON(Dataset, await JSB_BF_FHANDLE('DICT', Projectname), CStr(Viewname), function (_Dataset) { Dataset = _Dataset })); else {
       ...
       if (await JSB_ODB_WRITEJSON(Dataset, await JSB_BF_FHANDLE('DICT', Projectname), CStr(Viewname))); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
...
async function JSB_MDL_EDV_PICK_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "edv_pick", "JSB_MDL_EDV_PICK_Pgm");
    // local variables
...
async function JSB_MDL_ENABLEDHTMLEDITING(ByRef_Backdrophtml, ByRef_Niceditid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "enabledhtmlediting", "JSB_MDL_ENABLEDHTMLEDITING");
    // local variables
    var S;
    ...
    ByRef_Backdrophtml = CStr(ByRef_Backdrophtml) + JSB_BF_HTML('\<input id=\'' + CStr(ByRef_Niceditid) + '_ViewProps\' name=\'' + CStr(ByRef_Niceditid) + '_formBtn\' type=\'submit\' value=\'ViewProps\' style=\'display:none\'/\>' + crlf); 
    ByRef_Backdrophtml = CStr(ByRef_Backdrophtml) + JSB_BF_HTML('\<input id=\'' + CStr(ByRef_Niceditid) + '_PageProps\' name=\'' + CStr(ByRef_Niceditid) + '_formBtn\' type=\'submit\' value=\'PageProps\' style=\'display:none\'/\>' + crlf); 
    ByRef_Backdrophtml = CStr(ByRef_Backdrophtml) + JSB_BF_HTML('\<input id=\'' + CStr(ByRef_Niceditid) + '_RstBak\' name=\'' + CStr(ByRef_Niceditid) + '_formBtn\' type=\'submit\' value=\'RstBak\' style=\'display:none\'/\>' + crlf); 
    ByRef_Backdrophtml = CStr(ByRef_Backdrophtml) + JSB_BF_HTML('\<input id=\'' + CStr(ByRef_Niceditid) + '_SavBak\' name=\'' + CStr(ByRef_Niceditid) + '_formBtn\' type=\'submit\' value=\'SavBak\' style=\'display:none\'/\>' + crlf); 
...
async function JSB_MDL_EXCELUPLOAD_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "excelupload", "JSB_MDL_EXCELUPLOAD_Pgm");
    var Frompage = queryVar('fromPage'); 
    var Rolemsg = (JSB_BF_ISAUTHENTICATED()?'':'You must be logged in to view this page. ' + JSB_HTML_ANCHOR(JSB_BF_LOGINURL(), 'Click here to login')); 
    if (!(await JSB_BF_ISEMPLOYEE())) return Stop((Rolemsg?Rolemsg:'You must be at least an employee to view this page.'));
    ...
    Print(await JSB_THEMES_THEME_IFWIS('mdl', 'ExcelUpload', await JSB_MDL_DISPLAY_EXCELUPLOAD(Centerviewvars), function (_P1, _P2, _P3) {  })); 
...
async function JSB_MDL_EXCELUPLOAD_CUSTOMMENU_Sub(ByRef_Menubar, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "excelupload_custommenu", "JSB_MDL_EXCELUPLOAD_CUSTOMMENU_Sub");
    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Menubar)
        return v
    }
    // Placeholder
    return exit(); 
...
async function JSB_MDL_FETCHVIEWCOLUMNS_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "fetchviewcolumns", "JSB_MDL_FETCHVIEWCOLUMNS_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                Projectname = paramVar('PROJECTNAME', 1); Viewname = paramVar('VIEWNAME', 2); _Search = paramVar('_SEARCH', 3); Nd = paramVar('ND', 4); Rows = paramVar('ROWS', 5); Page = paramVar('PAGE', 6); Sidx = paramVar('SIDX', 7); Sord = paramVar('SORD', 8); Searchfield = paramVar('SEARCHFIELD', 9); Searchoper = paramVar('SEARCHOPER', 10); Searchstring = paramVar('SEARCHSTRING', 11); 
                // %options aspxC-
                Preventtimeout = System(56, me); 
                Viewname = dropIfRight(CStr(Viewname), '.view', true) + '.view'; 
                if (await JSB_ODB_READJSON(Dataset, await JSB_BF_FHANDLE('DICT', Projectname), CStr(Viewname), function (_Dataset) { Dataset = _Dataset })); else { me.Restful_Result = {"errors":me._activeProcess.At_Errors}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
                me.Restful_Result = {"page":1,"total":1,"records":Len(Dataset.columns),"rows":Dataset.columns}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; 
...
async function JSB_MDL_FINDBTNI(ByRef_Objectmodel, ByRef_Btntext, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "findbtni", "JSB_MDL_FINDBTNI");
    // local variables
    var I;
    ...
    for (I = 1; I<=_ForEndI_1; I++) {
       if (Null0(ByRef_Objectmodel.custombtns[I].buttontxt)==ByRef_Btntext) return exit(I);
    } 
...
async function JSB_MDL_FINDVIEWCOLUMN(ByRef_Dataset, Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "findviewcolumn", "JSB_MDL_FINDVIEWCOLUMN");
    // local variables
    ...
    Nicecolumnid = nicename(CStr(Columnid)); 
    ...
    for (Columnidx = 1; Columnidx<=_ForEndI_2; Columnidx++) {
       ...
       if (Vname==Columnid || nicename(CStr(Vname))==Nicecolumnid) return exit(Columnidx);
    } 
    return exit(0); 
}
// </FINDVIEWCOLUMN>
...
async function JSB_MDL_FORMATCD(ByRef_Cd, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "formatcd", "JSB_MDL_FORMATCD");
    function exit(v) {
...
async function JSB_MDL_FORMURL(Url, Addoutputs, Outputfields, Passparams, Addnewrec, Addfrompage) { 
    var me = new jsbRoutine("JSB_MDL", "formurl", "JSB_MDL_FORMURL");
    // local variables
    ...
    if (CBool(Addoutputs) && CBool(Outputfields)) {
       if (Not(Insidequotes)) { Url = CStr(Url) + ':\''; Insidequotes = true; }
       Url = CStr(Url) + CStr(Sep) + Mid1(await JSB_MDL_URLOUTPUTPARAMS(Outputfields, function (_Outputfields) { Outputfields = _Outputfields }),2); 
       Sep = ('&');
    }

    if (CBool(Addnewrec)) {
       if (Not(Insidequotes)) { Url = CStr(Url) + ':\''; Insidequotes = true; }
       Url = CStr(Url) + CStr(Sep) + 'newRecord=1';
    }

    if (CBool(Addfrompage)) {
       if (Not(Insidequotes)) { Url = CStr(Url) + ':\''; Insidequotes = true; }
       Url = CStr(Url) + CStr(Sep) + 'fromPage=\':@Url'; 
       Insidequotes = false; 
       Sep = ('&');
    }

    if (CBool(Passparams)) {
       if (CBool(Insidequotes)) { Url = CStr(Url) + '\':'; Insidequotes = false; }
       Url = (CStr(Url) + 'iif(@QueryString, \'' + CStr(Sep) + '\':Replace(Replace(@QueryString, "newRecord=1", ""), "&&", "&"), \'\')');
    }

    if (CBool(Insidequotes)) Url = CStr(Url) + '\'';
    return Url; 
}
// </FORMURL>
...
async function JSB_MDL_FORMVIEWNCOLUMNS(Projectname, Objectmodel, Dokobinding, Userow, Viewname) { 
    var me = new jsbRoutine("JSB_MDL", "formviewncolumns", "JSB_MDL_FORMVIEWNCOLUMNS");
    // local variables
    ...
    var COLUMN_LastI10 = UBound(Modelcolumns); 
    for (Ci = LBound(Modelcolumns); Ci<=COLUMN_LastI10; Ci++) {
       ...
       if (Column.display!='hidden') {
          if (CBool(Column.collaspestart)) {
             if (Startedformgrouprow) Html[Html.length] = JSB_BF_HTML('\</div\>');

             Html[Html.length] = (JSB_BF_HTML('\<div class="collase_block"\>\<div class="collase_header" onclick="$(this).next().slideToggle();"\>&#8681;&nbsp;' + htmlEscape(Column.collaspestart) + '&nbsp;&#8681;\</div\>')); 
             if (CBool(Column.collaspopen)) {
                Html[Html.length] = JSB_BF_HTML('\<div class="collase_content"\>');
             } else {
                Html[Html.length] = JSB_BF_HTML('\<div class="collase_content" style="display: none"\>');
             }
          ...
          if (!Startedformgrouprow) {
             Html[Html.length] = JSB_BF_HTML('\<div class="form-group row"' + Style + '\>'); 
          ...
          var Nicecolumnname = nicename(CStr(Column.name)); 
          ...
          if (CBool(Column.pickfunction)) {
             ...
             Ctlhtml = JSB_HTML_ADDPICK(CStr(Ctlhtml), Nicecolumnname, CStr(Pickurl), '80%', '90%', CStr(Pickurl), CStr(Column.autopostback));
          }
          ...
          if (Callname=='ctl_json_inline') {
             ...
             if (CBool(Column.suppresslabel)) {
                Html[Html.length] = JSB_BF_HTML('\<mdlctl id=\'ctl_' + Nicecolumnname + '\' style=\'width: 100%\'\>') + CStr(Ctlhtml) + JSB_BF_HTML('\</mdlctl\>');
             } else {
                Html[Html.length] = JSB_BF_LBLCTLSET(CStr(Ctllabel), CStr(Ctlhtml), Nicecolumnname, CNum(Column.fullline), CNum(Column.suppresslabel), CStr(Column.lblstyle));
             }
          } else {
             Html[Html.length] = JSB_BF_LBLCTLSET(CStr(Ctllabel), CStr(Ctlhtml), Nicecolumnname, CNum(Column.fullline), CNum(Column.suppresslabel), CStr(Column.lblstyle));
          }
          ...
          if (CBool(Column.collaspeend) && Startedcollaspe) {
             if (Startedformgrouprow) Html[Html.length] = JSB_BF_HTML('\</div\>');
             Html[Html.length] = JSB_BF_HTML('\</div\>\</div\>'); // Collase_block, Collase_content
             Startedformgrouprow = false; 
    ...
    if (Startedformgrouprow) {
       Html[Html.length] = JSB_BF_HTML('\</div\>'); 
       Startedformgrouprow = false;
    }
...
async function JSB_MDL_FULLLINECOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "fulllinecolumn", "JSB_MDL_FULLLINECOLUMN_Sub");
    // local variables
    var Dataset, Columnidx;
...
async function JSB_MDL_GENERATECODEFROMTEMPLATE(ByRef_Projectname, Templatefilename, ByRef_Parentobjectname, ByRef_Parentobject, ByRef_Objectname, ByRef_Results, ByRef_Compileresults, Nomodelingstuff, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "generatecodefromtemplate", "JSB_MDL_GENERATECODEFROMTEMPLATE");
    // local variables
    var Objectmodel, Templatename, Template, Templatejson, Templatefield;
    var Code, Codename, Errors;
    ...
    if (await JSB_ODB_READ(Template, await JSB_BF_FHANDLE(CStr(Templatefilename)), CStr(Templatename), function (_Template) { Template = _Template })); else {
       ...
       if (CBool(ByRef_Parentobject)) ByRef_Compileresults = CStr(ByRef_Compileresults) + 'This is coming from an embedded template ' + CStr(ByRef_Parentobject.templateName);
       return exit(false);
    }

    Templatejson = Field(Template,'******************************************************************************',1, true); 
    Template = Mid1(Template,me._activeProcess.Col2); 
    ...
    if (CBool(ByRef_Parentobject)) {
       // process view
       Code = [undefined, 'Function genCodeStubView(ProjectName, pageName, pageModel, viewName, viewModel, TemplateFileName, Errors, noModelingStuff)' + am]; 
       Codename = 'genCodeStubView';
    } else {
       // process page
       Code = [undefined, 'Function genCodeStubPage(ProjectName, pageName, pageModel, TemplateFileName, Errors, noModelingStuff)' + am]; 
       Codename = 'genCodeStubPage';
    }

    Code[Code.length] = 'Gen = []'; 

    if (Templatefilename=='jsb_viewtemplates') {
       if (CBool(ByRef_Parentobject)) Code[Code.length] = 'pageTemplateName = "' + LCase(Objectmodel.templateName) + '"';
       Code[Code.length] = 'viewTemplateName = "' + CStr(Templatename) + '"'; ;
    } else if (Templatefilename=='jsb_pagetemplates') {
       Code[Code.length] = 'pageTemplateName = "' + CStr(Templatename) + '"'; ;
    } else {
       Code[Code.length] = 'TemplateName = "' + CStr(Templatename) + '"';
    }

    await JSB_MDL_PROCESSTEMPLATE_Sub(Code, Templatefilename, Templatename, Template, function (_Code) { Code = _Code }); 

    Code[Code.length] = 'Gen = Join(Gen, \'\')'; 

    Code[Code.length] = 'Return Gen'; 
    Code[Code.length] = 'End Function'; 
    Code = Join(Code,am); 

    if (await JSB_ODB_WRITE(CStr(Code), await JSB_BF_FHANDLE('TMP'), CStr(Codename))); else return Stop(At(-1), 'Modeler-', System(28, me), ': ', me._activeProcess.At_Errors);
    ...
    if (InStr1(1,ByRef_Compileresults,'^') || InStr1(1,ByRef_Compileresults,'You must be an administrator')) {
       ...
       ByRef_Compileresults = CStr(ByRef_Compileresults) + JSB_BF_ANCHOREDIT(CStr(Templatefilename), CStr(Templatename)); 
       return exit(false);
    }

    ByRef_Compileresults = CStr(ByRef_Compileresults) + JSB_BF_ANCHOREDIT(CStr(ByRef_Projectname), dropIfRight(CStr(ByRef_Objectname), '.page')); 
...
async function JSB_MDL_GETCOLUMNDEFINITIONS(Tblid_Or_Sql, ByRef_Columnnames, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "getcolumndefinitions", "JSB_MDL_GETCOLUMNDEFINITIONS");
    // local variables
    ...
    if (CBool(Isselect)) {
       ...
       Tablecolumns = await JSB_BF_GETTABLECOLUMNDEFS(Tblid_Or_Sql, CStr(false), false, function (_Tblid_Or_Sql) { Tblid_Or_Sql = _Tblid_Or_Sql });
    }

    if (HasTag(Tablecolumns[LBound(Tablecolumns)],'Oridinal')) Tablecolumns = Sort(CStr(Tablecolumns), 'Oridinal');
    if (HasTag(Tablecolumns[LBound(Tablecolumns)],'oridinal')) Tablecolumns = Sort(CStr(Tablecolumns), 'oridinal');
...
async function JSB_MDL_GETDISTINCTVALUES(ByRef_Tblid_Or_Sql, ByRef_Field, ByRef_Limitno, ByRef_Datatype, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "getdistinctvalues", "JSB_MDL_GETDISTINCTVALUES");
    // local variables
    ...
    if (CBool(Isselect)) {
       ...
       if (await asyncDNOSqlSelect(Sqlselect, _selectList => Ss = _selectList)); else { Alert(me._activeProcess.At_Errors);return exit([undefined, ]); }
    } else {
       if (await JSB_ODB_OPEN('', CStr(ByRef_Tblid_Or_Sql), F, function (_F) { F = _F })); else { Alert(me._activeProcess.At_Errors);return exit([undefined, ]); }

       if (JSB_BF_ISSQLSERVER(F)) {
          Usename = ByRef_Tblid_Or_Sql; 
          if (await JSB_BF_DBVIEWEXISTS(Usename, undefined, function (_Usename) { Usename = _Usename })) {
             Tmptblname = '##dview_' + CStr(ByRef_Tblid_Or_Sql) + '_' + nicename(JSB_BF_USERNAME()); 
          ...
          if (await JSB_ODB_SELECTTO('', F, '', Ss, function (_Ss) { Ss = _Ss })); else { Alert(me._activeProcess.At_Errors);return exit([undefined, ]); }
          Keys = getList(Ss); 
          return exit(Split(Keys,am)); ;
       } else {
          if (await JSB_ODB_SELECTTO('distinct ' + CStr(Top) + ' [' + CStr(ByRef_Field) + ']', F, '', Ss, function (_Ss) { Ss = _Ss })); else { Alert(me._activeProcess.At_Errors);return exit([undefined, ]); }
       }
    }
...
async function JSB_MDL_GETDISTINCTYEARS(ByRef_Tblid_Or_Sql, ByRef_Field, ByRef_Limitno, ByRef_Datatype, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "getdistinctyears", "JSB_MDL_GETDISTINCTYEARS");
    // local variables
    ...
    if (CBool(Isselect)) {
       ...
       if (JSB_BF_ISSQLSERVER(F)) {
          ...
          if (await JSB_ODB_OPEN('', CStr(ByRef_Tblid_Or_Sql), F, function (_F) { F = _F })); else { Alert(me._activeProcess.At_Errors);return exit([undefined, ]); }
          if (await JSB_ODB_SELECTTO('distinct ' + CStr(Top) + ' ' + CStr(Fnc) + ' As [' + CStr(ByRef_Field) + ']', F, '', Ss, function (_Ss) { Ss = _Ss })); else { Alert(me._activeProcess.At_Errors);return exit([undefined, ]); }
       }
    }
...
async function JSB_MDL_GETEXTRAPAGECOLUMNS_Sub(ByRef_Modelcolumns, ByRef_Pagetemplatename, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "getextrapagecolumns", "JSB_MDL_GETEXTRAPAGECOLUMNS_Sub");
    // local variables
    var Template, Columns, Xrow;
...
async function JSB_MDL_GETJSONDEFDEFAULTS(ByRef_Jsondef_Id, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "getjsondefdefaults", "JSB_MDL_GETJSONDEFDEFAULTS");
    // local variables
    var Jsondefs;
...
async function JSB_MDL_GETROW(ByRef_Settings, ByRef_Rows, ByRef_Rowi, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "getrow", "JSB_MDL_GETROW");
    // local variables
    ...
    for (C = CNum(ByRef_Settings.FirstC); (C<=_ForEndI_2) && C<=UBound(Srcrow); C++) {
       Cellvalue = Srcrow[C]; 
       if (CBool(Cellvalue)) {
          if (Null0(ByRef_Settings.State)==STATE_LOOKINGFORHEADER) {
...
async function JSB_MDL_GETSELECTIONCOLUMNS(ByRef_Viewmodel, ByRef_Cnames, Orderallcolumns, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "getselectioncolumns", "JSB_MDL_GETSELECTIONCOLUMNS");
    // local variables
...
async function JSB_MDL_GETTABLEPRIMARYKEYCOL(ByRef_Tablecolumns, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "gettableprimarykeycol", "JSB_MDL_GETTABLEPRIMARYKEYCOL");
    // local variables
...
async function JSB_MDL_HASSERVERSIDEPARAMS(ByRef_Viewmodel, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "hasserversideparams", "JSB_MDL_HASSERVERSIDEPARAMS");
    // local variables
    var Row;
...
async function JSB_MDL_HIDEVIEWCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "hideviewcolumn", "JSB_MDL_HIDEVIEWCOLUMN_Sub");
    // local variables
...
async function JSB_MDL_HIGHESTVIEWINDEX(ByRef_Dataset, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "highestviewindex", "JSB_MDL_HIGHESTVIEWINDEX");
    // local variables
    ...
    for (Columnidx = 1; Columnidx<=_ForEndI_1; Columnidx++) {
       Viewrow = Vcolumns[Columnidx]; 
       if (Null0(Viewrow.index)>I) I = Viewrow.index;
    } 
    return exit( +I+1); 
}
// </HIGHESTVIEWINDEX>
...
async function JSB_MDL_ISROWEMPTY(ByRef_Row, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "isrowempty", "JSB_MDL_ISROWEMPTY");
    function exit(v) {
...
async function JSB_MDL_LABELCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "labelcolumn", "JSB_MDL_LABELCOLUMN_Sub");
    // local variables
    var Dataset, Columnidx, Newvalue;
    ...
    Columnidx = await JSB_MDL_FINDVIEWCOLUMN(Dataset, ByRef_Columnid, function (_Dataset) { Dataset = _Dataset }); 
    if (Columnidx=='0') return exit(undefined);
    Newvalue = Trim(await JSB_BF_INPUTBOX('Column Property ' + CStr(ByRef_Columnid), 'Label', CStr(Dataset.columns[Columnidx].label), '80%', 'auto', function (_P1) {  })); 
    if (Newvalue==Chr(27)) return exit(undefined);
...
// <LBLCTLPAIR>
async function JSB_MDL_LBLCTLPAIR(ByRef_Label, ByRef_Ctlhtml, ByRef_Nicecolumnname, ByRef_Fullline, ByRef_Suppresslabel, ByRef_Lblstyle, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "lblctlpair", "JSB_MDL_LBLCTLPAIR");
    // local variables
    var C1, Lbl, Ctl;

    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Label, ByRef_Ctlhtml, ByRef_Nicecolumnname, ByRef_Fullline, ByRef_Suppresslabel, ByRef_Lblstyle)
        return v
    }
    // %options aspxC-
    ...
    if (CBool(ByRef_Fullline)) {
       if (CBool(ByRef_Suppresslabel)) {
          C1[C1.length] = JSB_BF_HTML('\<div class="lblctlpair_FullLine lblctlpair_FullLine_suppressLabel"\>'); 
          C1[C1.length] = JSB_BF_HTML('   \<mdlctl id=\'ctl_' + CStr(ByRef_Nicecolumnname) + '\'\>') + CStr(ByRef_Ctlhtml) + JSB_BF_HTML('\</mdlctl\>'); 
          C1[C1.length] = JSB_BF_HTML('\</div\>');
       } else {
          if (true) {
             Lbl = (JSB_BF_HTML('\<label id=\'LBL_' + CStr(ByRef_Nicecolumnname) + '\' xfor=\'' + CStr(ByRef_Nicecolumnname) + '\' style=\'float: left; overflow:hidden; text-align:right; width: 116px;' + CStr(ByRef_Lblstyle) + '\'\>') + Trim(ByRef_Label) + JSB_BF_HTML('&nbsp;\</label\>')); 
             Ctl = JSB_BF_HTML('\<mdlctl id=\'ctl_' + CStr(ByRef_Nicecolumnname) + '\'\>') + CStr(ByRef_Ctlhtml) + JSB_BF_HTML('\</mdlctl\>'); 
             C1[C1.length] = JSB_HTML_COLS2('120px', CStr(Lbl), '%', CStr(Ctl), '', 'overflow: hidden', 'overflow: auto'); // Cols2(C1Width, C1Content, C2Width, C2Content, Height, C1Style, C2Style);
          } else {
             C1[C1.length] = JSB_BF_HTML('\<div class="lblctlpair_FullLine lblctlpair_FullLine_withLabel" \>'); 
             Lbl = (JSB_BF_HTML('\<label id=\'LBL_' + CStr(ByRef_Nicecolumnname) + '\' xfor=\'' + CStr(ByRef_Nicecolumnname) + '\' style=\'' + CStr(ByRef_Lblstyle) + '\'\>') + Trim(ByRef_Label) + JSB_BF_HTML('&nbsp;\</label\>')); 
             Ctl = JSB_BF_HTML('\<mdlctl id=\'ctl_' + CStr(ByRef_Nicecolumnname) + '\'\>') + CStr(ByRef_Ctlhtml) + JSB_BF_HTML('\</mdlctl\>'); 
       ...
       if (CBool(ByRef_Suppresslabel)) {
          C1[C1.length] = JSB_BF_HTML('\<div class="lblctlpair lblctlpair_suppressLabel"\>'); 
          C1[C1.length] = JSB_BF_HTML('   \<mdlctl id=\'ctl_' + CStr(ByRef_Nicecolumnname) + '\' \>') + CStr(ByRef_Ctlhtml) + JSB_BF_HTML('\</mdlctl\>'); 
          C1[C1.length] = JSB_BF_HTML('\</div\>');
       } else {
          C1[C1.length] = JSB_BF_HTML('\<div class="lblctlpair lblctlpair_withLabel"\>'); 
          C1[C1.length] = (JSB_BF_HTML('\<label id=\'LBL_' + CStr(ByRef_Nicecolumnname) + '\' xfor=\'' + CStr(ByRef_Nicecolumnname) + '\' style=\'' + CStr(ByRef_Lblstyle) + '\'\>') + Trim(ByRef_Label) + JSB_BF_HTML('&nbsp;\</label\>')); 
          C1[C1.length] = JSB_BF_HTML('   \<mdlctl id=\'ctl_' + CStr(ByRef_Nicecolumnname) + '\'\>') + CStr(ByRef_Ctlhtml) + JSB_BF_HTML('\</mdlctl\>'); 
          C1[C1.length] = JSB_BF_HTML('\</div\>');
       }
    }
...
async function JSB_MDL_MAKECLONENAME(Projectname, Oldpagename, Oldviewname, Newpagename) { 
    var me = new jsbRoutine("JSB_MDL", "makeclonename", "JSB_MDL_MAKECLONENAME");
    // local variables
    ...
    if (Newviewname!=Oldpagename) return CStr(Newviewname) + '.view';
...
async function JSB_MDL_MAKENEWREPORT_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "makenewreport", "JSB_MDL_MAKENEWREPORT_Pgm");
    // local variables
    ...
    Databasename = paramVar('databaseName'); 
    if (Databasename === undefined) return Stop('databaseName is a required parameter');
    if (Not(await JSB_ODB_ATTACHDBEXTENDED(Databasename, function (_Databasename) { Databasename = _Databasename }))) return Stop(me._activeProcess.At_Errors);
    ...
    if (Reportname!=Chr(27) && !isEmpty(Reportname)) {
       ...
       await JSB_BF_MSGBOX('Redirecting to ' + JSB_BF_JSBROOTACT() + CStr(Reportname));
       return At_Response.Redirect(JSB_BF_JSBROOTACT() + CStr(Reportname));
       return Stop();
    }
...
async function JSB_MDL_MAKENEWREPORT(Tabid, Databasename, ByRef_Reportname, Tblid_Or_Sql, Visiblecolumns, Defaultdevxgridlayout, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "MAKENEWREPORT", "JSB_MDL_MAKENEWREPORT");
    // local variables
    ...
    if (Not(Databasename)) { me._activeProcess.At_Errors = 'A databaseName is a required URL parameter'; return exit(false); }
    ...
    if (Searchtypes==false) return exit(false);
    ...
    Prefix = nicename(Databasename) + '_'; 
    ...
    if (CBool(Editinputs) && CBool(Editcolumns)) {
       if (await JSB_ODB_READJSON(Griddataset, await JSB_BF_FHANDLE('dict', 'dbBuilderReports'), CStr(ByRef_Reportname) + '_Grid.view', function (_Griddataset) { Griddataset = _Griddataset })) {
          ...
          if (await JSB_ODB_WRITEJSON(Griddataset, await JSB_BF_FHANDLE('dict', 'dbBuilderReports'), CStr(ByRef_Reportname) + '_Grid.view')); else return Stop(me._activeProcess.At_Errors);
       }
       ...
       if (CBool(Oldview.columns)) {
          ...
          if (await JSB_ODB_WRITEJSON(Oldview, await JSB_BF_FHANDLE('dict', 'dbBuilderReports'), CStr(ByRef_Reportname) + '_editRecord.view')); else return Stop(me._activeProcess.At_Errors);
       } else {
          ...
          if (await JSB_ODB_WRITEJSON(Newview, await JSB_BF_FHANDLE('dict', 'dbBuilderReports'), CStr(ByRef_Reportname) + '_editRecord.view')); else return Stop(me._activeProcess.At_Errors);
       }
    }

    ByRef_Reportname = dropIfRight(CStr(ByRef_Reportname), '.page', true); 
    Url = JSB_BF_JSBROOTACCOUNT() + urlEncode(CStr(ByRef_Reportname)) + '?tabID=' + urlEncode(Tabid); 
    ...
    if (Tabid) {
       ...
       Print(At(-1), JSB_HTML_SCRIPT('\r\n\
            if (window.parent.reloadDbTree) {\r\n\
                ...
                \r\n\
                if (true || isMobile()) {\r\n\
                    window.parent.openInTab("' + Tabid + '", ' + JSB_BF_JSESCAPESTRING(CStr(ByRef_Reportname)) + ', ' + JSB_BF_JSESCAPESTRING(CStr(Tabtitle)) + ', ' + JSB_BF_JSESCAPESTRING(CStr(Url)) + ');\r\n\
                    var tabID = dropLeft(window.frameElement.id, "_");\r\n\
                    window.parent.$(\'a[href="#tab_' + Tabid + '"]\').click()\r\n\
                } else {\r\n\
                    windowOpen(' + JSB_BF_JSESCAPESTRING(CStr(Url)) + ', \'_blank\');\r\n\
                }\r\n\
                \r\n\
...
async function JSB_MDL_MDLGETREFFIELDS_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "mdlgetreffields", "JSB_MDL_MDLGETREFFIELDS_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                Databasename = paramVar('DATABASENAME', 1); Tablename = paramVar('TABLENAME', 2); Arg = paramVar('ARG', 3); 
                // %options aspxC-
                ...
                if (!(await JSB_BF_ISMANAGER())) { me.Restful_Result = {"err":'You must be at least a data manager to use this command.'}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
                if (Not(Databasename)) { me.Restful_Result = {"err":'A databaseName is a required URL parameter'}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
                if (Not(Arg)) { me.Restful_Result = {"err":'A arg value is a required URL parameter'}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
            
                if (Not(await JSB_ODB_ATTACHDBEXTENDED(Databasename, function (_Databasename) { Databasename = _Databasename }))) { me.Restful_Result = false; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; }
            
                Tbldefs = await JSB_BF_GETTABLECOLUMNDEFS(Arg, CStr(false), false, function (_Arg) { Arg = _Arg }); 
                Columns = [undefined, '']; 
                for (Column of iterateOver(Tbldefs)) {
                   Columns[Columns.length] = Column.name; 
                } 
                me.Restful_Result = {"results":Columns}; me._state = "RESTFUL_SERVEREXIT"; continue atgoto; 
...
async function JSB_MDL_MDLGETREFVALUES(ByRef_Projectname, ByRef_Column, ByRef_Crow, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "mdlgetrefvalues", "JSB_MDL_MDLGETREFVALUES");
    // local variables
    ...
    if (CBool(ByRef_Column.reflist)) {
       Values = JSB_BF_MAKEARRAY(ByRef_Column.reflist); ;
       ...
       if (Left(Reffile,2)=='{!') {
          ...
          if (!isEmpty(Tablename)) {
             Columns = await JSB_BF_GETTABLECOLUMNDEFS(Tablename, CStr(false), true, function (_Tablename) { Tablename = _Tablename }); 
          ...
          if (await JSB_ODB_OPEN('dict', CStr(ByRef_Projectname), F, function (_F) { F = _F })) {
             if (await JSB_ODB_SELECTTO('', F, 'ItemId Like \'%.view\'', Y, function (_Y) { Y = _Y })) {
                ...
                Values = [undefined, 'unable to select file ' + CStr(ByRef_Projectname) + ' ' + 'Modeler-' + CStr(System(28, me)) + ': ' + me._activeProcess.At_Errors];
             }
          } else {
             Values = [undefined, 'unable to open file ' + CStr(ByRef_Projectname) + ' ' + 'Modeler-' + CStr(System(28, me)) + ': ' + me._activeProcess.At_Errors];
          }
       } else {
...
async function JSB_MDL_MDLSESSIONSETUP_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Viewdataset, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "mdlsessionsetup", "JSB_MDL_MDLSESSIONSETUP_Sub");
    // local variables
...
async function JSB_MDL_MODELER_Sub() { 
    var me = new jsbRoutine("JSB_MDL", "modeler", "JSB_MDL_MODELER_Sub");
}
// </MODELER_Sub>

// <MODELFORALLCOLUMNS>
async function JSB_MDL_MODELFORALLCOLUMNS() { 
    var me = new jsbRoutine("JSB_MDL", "modelforallcolumns", "JSB_MDL_MODELFORALLCOLUMNS");
    // local variables
...
async function JSB_MDL_MODELFORPAGE(ByRef_Projectname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "modelforpage", "JSB_MDL_MODELFORPAGE");
    // local variables
...
async function JSB_MDL_MODELFORTABLECOLUMNS() { 
    var me = new jsbRoutine("JSB_MDL", "modelfortablecolumns", "JSB_MDL_MODELFORTABLECOLUMNS");
    // local variables
    var Columns, Model;

    // %options aspxC-

    Columns = JSB_BF_TABLECOLUMNS(); 
    Model = {}; 
    Model.columns = Columns; 
    return Model; 
}
...
async function JSB_MDL_MODELFORVIEW() { 
    var me = new jsbRoutine("JSB_MDL", "modelforview", "JSB_MDL_MODELFORVIEW");
    // local variables
    var Columns, Model;
...
async function JSB_MDL_MODELFORVIEWCOLUMNS() { 
    var me = new jsbRoutine("JSB_MDL", "modelforviewcolumns", "JSB_MDL_MODELFORVIEWCOLUMNS");
    // local variables
    var Model;
    ...
    Model = {}; 
    Model.columns = JSB_BF_VIEWCOLUMNS(); 
    return Model; 
}
// </MODELFORVIEWCOLUMNS>
...
async function JSB_MDL_NEWPAGE(Projectname, Pagenameprefix, ByRef_Pagename, Predefinedcolumns, Predefinedinputs, Predefinedoutputs, ByRef_Pagetype, ByRef_Tblid_Or_Sql, Visiblecolumns, Devxgridlayout, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "newpage", "JSB_MDL_NEWPAGE");
    // local variables
    ...
    var Nicepagename, Oururl, Searchoutputs, Cname, Label, Basepagename;
    ...
    if (await JSB_ODB_OPEN('dict', Projectname, Fdictproject, function (_Fdictproject) { Fdictproject = _Fdictproject })); else return exit(undefined);
    Isselect = LCase(Left(ByRef_Tblid_Or_Sql,7))=='select '; 
    ...
    if (!ByRef_Pagename) {
       if (!(InStr1(1,ByRef_Tblid_Or_Sql,' ')) && Desc=='report') {
          Suggestedname = nicename(CStr(ByRef_Tblid_Or_Sql) + ' Report');
       }
       ...
       while (true) {
          ...
          ByRef_Pagename = nicename(ByRef_Pagename); 
          if (await JSB_ODB_READJSON(Pagedataset, Fdictproject, Pagenameprefix + ByRef_Pagename + '.page', function (_Pagedataset) { Pagedataset = _Pagedataset })); else break;
       ...
       ByRef_Pagename = Pagenameprefix + ByRef_Pagename;
    }
    ...
    if (ByRef_Pagetype) {
       ...
       Xtratypes = await JSB_BF_FSELECT('vt', 'ItemID \> "1"', undefined, undefined, function (_P1) {  }); 
    ...
    if (LCase(ByRef_Pagetype)=='twoviews') {
       ...
       Masterviewdataset.attachdb = JSB_BF_ACCOUNT(); 
       ...
       // pick columns
       Schemacolumns = await JSB_BF_GETTABLECOLUMNDEFS(Masterviewdataset.tableName, CStr(true), true, function (_P1) {  }); 
       ...
       if (Not(await JSB_MDL_CHOOSECOLUMNS_SS(Projectname, CStr(Viewname), Masterviewdataset, Schemacolumns))) { ByRef_Pagename = ''; return exit(undefined); }

       Gotpk = false; 
       for (Column of iterateOver(Masterviewdataset.columns)) {
          if (Null0(Column.name)==Primarykeyname) { Gotpk = true; break; }
       } 
       if (Not(Gotpk)) {
       ...
       if (await JSB_ODB_WRITEJSON(Masterviewdataset, Fdictproject, CStr(Viewname))); else return Stop(At(-1), 'Edp-', System(28, me), ': ', me._activeProcess.At_Errors);
    ...
    Viewdataset.attachdb = JSB_BF_ACCOUNT(); 
    ...
    if (Usetype!='Menu') {
       if (Not(ByRef_Tblid_Or_Sql)) {
          ...
          if (Not(ByRef_Tblid_Or_Sql)) {
             if (LCase(ByRef_Pagetype)=='twoviews') { if (await JSB_ODB_DELETEITEM(Fdictproject, CStr(Viewname))); else if (me._activeProcess.At_Errors) Alert(me._activeProcess.At_Errors); }
    ...
    if (CBool(Isselect)) {
       ...
       Schemacolumns = await JSB_BF_GETTABLECOLUMNDEFS(ByRef_Tblid_Or_Sql, CStr(false), false, function (_ByRef_Tblid_Or_Sql) { ByRef_Tblid_Or_Sql = _ByRef_Tblid_Or_Sql });
    }
    ...
    if (Not(Predefinedcolumns)) {
       ...
       if (Not(await JSB_MDL_CHOOSECOLUMNS_SS(Projectname, CStr(Viewname), Viewdataset, Schemacolumns, CStr(D)))) { ByRef_Pagename = ''; return exit(undefined); }
    }
    ...
    Nicepagename = nicename(ByRef_Pagename); 
    ...
    break; 
       ...
       case 'Search Form': case 'Report': 
          ...
          Viewdataset.custombtns[Viewdataset.custombtns.length] = {"buttontxt":'Query',"opento":10,"url":nicename(ByRef_Pagename) + '_grid',"verifyform":true,"addoutputs":true}; 

          if (CBool(isAdmin())) {
             Viewdataset.custombtns[Viewdataset.custombtns.length] = {"buttontxt":'Pivot',"opento":10,"url":nicename(ByRef_Pagename) + '_pivotsetup',"verifyform":true,"addoutputs":true}
          }
          ...
          if (Not(Predefinedoutputs)) {
             ...
             for (Column of iterateOver(Viewdataset.columns)) {
                Cname = Column.name; 
                if (Null0(Column.name)!=Null0(Column.label) && !isEmpty(Column.label)) Label = Column.label; else Label = '';
                Searchoutputs[Searchoutputs.length] = {"field":Column.name,"name":Label,"scope":'UrlParam'}; 
             } 
       ...
       case 'Report(Grid)': 
          ...
          if (Len(Predefinedinputs)==0) {
             ...
             for (Column of iterateOver(Viewdataset.columns)) {
                ...
                if (Null0(Column.name)!=Null0(Column.label) && !isEmpty(Column.label)) Label = Column.label; else Label = '';
                Predefinedinputs[Predefinedinputs.length] = {"and":'1',"lparen":false,"field":Column.name,"name":Label,"scope":'UrlParam',"datatype":Column.datatype,"operator":'=',"required":false,"rparen":false,"defaultvalue":''}; 
             }
          }

          Viewdataset.inputs = Predefinedinputs; 
          Viewdataset.custombtns[Viewdataset.custombtns.length] = {"buttontxt":'Back',"opento":10,"url":nicename(CStr(Basepagename)),"confirmop":2,"passparams":true}; 
    ...
    break; 
       ...
       case 'Report(Pivot Setup)': 
          ...
          Viewdataset.custombtns[Viewdataset.custombtns.length] = {"buttontxt":'Next',"opento":11,"url":nicename(CStr(Basepagename)) + '_pivot',"onParentExtra":'if (!saveRowsAndCols()) return false; ',"customcall":'view_' + CStr(Nicepagename) + '_Next',"verifyform":true,"passparams":true,"addoutputs":true}; 
    ...
    break; 
       ...
       case 'Report(Pivot)': 
          ...
          Viewdataset.inputs = Predefinedinputs; 
          Viewdataset.custombtns[Viewdataset.custombtns.length] = {"buttontxt":'Back',"opento":10,"url":nicename(CStr(Basepagename)) + '_pivotsetup',"confirmop":2,"passparams":true}; 
    ...
    if (await JSB_ODB_WRITEJSON(Viewdataset, Fdictproject, CStr(Viewname))); else return Stop(At(-1), 'Edp-', System(28, me), ': ', me._activeProcess.At_Errors);

    if (await JSB_ODB_WRITEJSON(Pagedataset, Fdictproject, ByRef_Pagename + '.page')); else return Stop(At(-1), 'Edp-', System(28, me), ': ', me._activeProcess.At_Errors);
...
async function JSB_MDL_NEWROW(ByRef_Objectname, ByRef_Columns, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "newrow", "JSB_MDL_NEWROW");
    // local variables
    var Newrow;
...
async function JSB_MDL_NEWSEARCHTYPE(Projectname, Databasename, Tblid_Or_Sql, Visiblecolumns) { 
    var me = new jsbRoutine("JSB_MDL", "newsearchtype", "JSB_MDL_NEWSEARCHTYPE");
    // local variables
    ...
    var Priorrow, Priorfield, Reflist, Datatype, Rf, V, Html, Jsscript;
    var Btns, Btn, Collatitude, Collongitude, Sresults, Isautopostback;
    var Passed, Fdicthandle, J;
        ...
        switch (me._state) {
            case "":
                ...
                if (CBool(Projectname)) {
                   Rptid = 'rptdef_' + nicename(CStr(Projectname)) + '_' + nicename(CStr(Tblid_Or_Sql)) + '.rpt';
                } else {
                   Projectname = 'css'; 
                   Rptid = 'rptdef_' + nicename(CStr(Databasename)) + '_' + nicename(CStr(Tblid_Or_Sql)) + '.rpt';
                }
            ...
            {"name":'reflist',"index":26,"label":'Ref List',"width":23,"control":'textbox',"canedit":true,"required":false,"notblank":false,"jsconditionals":Jscondreflist,"tooltip":'Lbl,Val;.. Or Lbl,Lbl,..'}
                ...
                var COLUMN_LastI3 = UBound(Schemacolumns); 
                for (Columni = LBound(Schemacolumns); Columni<=COLUMN_LastI3; Columni++) {
                   Column = Extract(Schemacolumns, Columni, 0, 0); 
                   Cname = Column.name; 
                ...
                do {
                   ...
                   Html = [undefined, ]; 
                   Html[Html.length] = JSB_BF_HTML('\<input id=\'dataset\' name=\'dataset\' type="hidden" value=\'\' /\>'); 
            
                   Html[Html.length] = JSB_HTML_H1('Select columns for the Selection Screen, the display grid, and the edit screen for the ' + CStr(Tblid_Or_Sql) + ' report') + crlf; 
            
                   Html[Html.length] = JSB_HTML_REPEATERLOAD(CStr(Projectname), undefined, Dataset); 
                   Html[Html.length] = await JSB_CTLS_REPEATERGRIDBACKGROUND(CStr(Projectname), CStr(Projectname), {"columns":Fielddefinitions,"attachdb":Databasename,"tableName":Tblid_Or_Sql}, Dataset, '', false, '', ''); 
            
                   Jsscript = await JSB_BF_JSCONDITIONALSSCRIPT(CStr(Projectname), Fielddefinitions, true); 
                   if (CBool(Jsscript)) Html[Html.length] = JSB_HTML_SCRIPT(CStr(Jsscript));
            
                   Html[Html.length] = JSB_BF_HTML('\<br\>GPS Coordinate columns:\<br\>'); 
                   Html[Html.length] = JSB_HTML_LABEL('Longitude: ', 'colLongitude') + JSB_HTML_DROPDOWNBOX('colLongitude', Columnnames, CStr(Default_Lngname), true); 
                   Html[Html.length] = JSB_HTML_LABEL('Latitude: ', 'colLatitude') + JSB_HTML_DROPDOWNBOX('colLatitude', Columnnames, CStr(Default_Latname), true); 
                   Html[Html.length] = JSB_BF_HTML('\<br\>'); 
                   ...
                   Btns[Btns.length] = JSB_HTML_BUTTON('BtnSelectAll', 'Select None', {
            "onclick":'\r\n\
            ...
            '}); 
                   ...
                   Btns[Btns.length] = JSB_HTML_BUTTON('BtnDisplayAll', 'Display None', {
            "onclick":'\r\n\
            ...
            '}); 
                   ...
                   Btns[Btns.length] = JSB_HTML_BUTTON('BtnEditAll', 'Edit None', {
            "onclick":'\r\n\
            ...
            '}); 
                   ...
                   Html[Html.length] = JSB_HTML_CENTER(Join(Btns,'')); 
                   ...
                   if (CBool(Collatitude) || CBool(Collongitude)) {
                      for (Sf of iterateOver(Dataset)) {
                         if (Null0(Sf.name)==Collatitude) Sf.isLatitude = true;
                         if (Null0(Sf.name)==Collongitude) Sf.isLongitude = true;
                      }
                   }
                   ...
                   if (Btn=='Reset') {
                      if (await JSB_BF_MSGBOX('Are you sure you want to reset the grid?', 'Yes,*No')=='Yes') {
                         ...
                         me._state = "STARTOVER"; continue atgoto;
                      }
                   }
...
async function JSB_MDL_NEWWEBSITEMODEL_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "newwebsitemodel", "JSB_MDL_NEWWEBSITEMODEL_Pgm");
    // local variables
    ...
    while (true) {
       ...
       Actname = nicename(CStr(Actname)); 
       if (await JSB_BF_CREATEACCOUNT(CStr(Actname))) break; 
       await JSB_BF_MSGBOX(me._activeProcess.At_Errors)
    } 

    // Get them to create a basic page
    return At_Response.Redirect(JSB_BF_HTMLROOT() + CStr(Actname) + '/edp%20BP%20' + CStr(Actname));
}
// </NEWWEBSITEMODEL_Pgm>

// <NEXTROWISDIFFERENT>
async function JSB_MDL_NEXTROWISDIFFERENT(ByRef_Settings, ByRef_Rows, ByRef_Nxtrowi, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "nextrowisdifferent", "JSB_MDL_NEXTROWISDIFFERENT");
    // local variables
    ...
    if (Nxtrowcc>UBound(ByRef_Settings.HdrRow)) return exit(true);

    if (CBool(ByRef_Settings.SideBySideTables)) {
       if (!Join(Nxtrow,'')) return exit(true);
    }

    if (ByRef_Nxtrowi<100) {
       Minimumcolumns = UBound(ByRef_Settings.HdrRow)*0.40; 
       return exit(Nxtrowcc<Minimumcolumns);
    }
...
async function JSB_MDL_OKAYTOOUTPUTTBL(ByRef_Settings, ByRef_Tbl, Sheetname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "okaytooutputtbl", "JSB_MDL_OKAYTOOUTPUTTBL");
    // local variables
    var Rowcnt, Colcnt;
    ...
    if (CBool(ByRef_Settings.ignoreTablesWithNoRows) && Rowcnt<=1) {
       me._activeProcess.At_Errors = 'Ignored... not enough rows'; 
       return exit(false);
    }

    ByRef_Settings.HdrRow = ByRef_Tbl[1]; 
    Colcnt = UBound(ByRef_Settings.HdrRow); 
    if (Colcnt<Null0(ByRef_Settings.ignoreTablesWithLessThanNColumns)) {
       me._activeProcess.At_Errors = 'Ignored... not enough columns'; 
       return exit(false);
    }
...
async function JSB_MDL_OPENTABLEANDCHECKID(ByRef_Objectmodel, ByRef_Viewname, ByRef_Ftable, ByRef_Pkid, ByRef_Rtnerrors, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "opentableandcheckid", "JSB_MDL_OPENTABLEANDCHECKID");
    // local variables
    var Tblname;
...
async function JSB_MDL_ORDERBUTTONS_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "orderbuttons", "JSB_MDL_ORDERBUTTONS_Sub");
    // local variables
    ...
    for (Btnname of iterateOver(Btnnames)) {
       for (Btn of iterateOver(Objectmodel.custombtns)) {
          if (Null0(Btn.buttontxt)==Btnname) Neworder[Neworder.length] = Btn;
       } 
    } 
...
async function JSB_MDL_ORDERCOLUMNS_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "ordercolumns", "JSB_MDL_ORDERCOLUMNS_Sub");
    // local variables
    ...
    for (Columnname of iterateOver(Selectedcols)) {
       for (Column of iterateOver(Dataset.columns)) {
          if (Null0(Column.name)==Columnname) Newcolumns[Newcolumns.length] = Column;
       } 
    } 
...
async function JSB_MDL_ORDERDISTINCTVALUES(ByRef_Ss, ByRef_Field, ByRef_Limitno, ByRef_Sortorder, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "orderdistinctvalues", "JSB_MDL_ORDERDISTINCTVALUES");
    // local variables
    ...
    if (CBool(ByRef_Limitno) && Ub>ByRef_Limitno) return exit([undefined, ]);
    ...
    Reflist = [undefined, ]; 
    var ROW_LastI4 = UBound(Dfields); 
    for (Rowi = LBound(Dfields); Rowi<=ROW_LastI4; Rowi++) {
       Row = Extract(Dfields, Rowi, 0, 0); 
       V = Row[ByRef_Field]; 
...
async function JSB_MDL_OUTPUTTABLE(ByRef_Settings, ByRef_Tbl, Sheetname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "outputtable", "JSB_MDL_OUTPUTTABLE");
    // local variables
        ...
        switch (me._state) {
            case "":
                ...
                for (Rowi = 2; ( +Stepsize>=0)?(Rowi<=_ForEndI_2):(Rowi>=_ForEndI_2); Rowi += +Stepsize) {
                   Row = ByRef_Tbl[Rowi]; 
                   Rec = {"RowNumber":Rowi}; 
                   var NAME_LastI3 = UBound(ByRef_Settings.HdrRow); 
                   for (Coli = LBound(ByRef_Settings.HdrRow); Coli<=NAME_LastI3; Coli++) {
                      Name = Extract(ByRef_Settings.HdrRow, Coli, 0, 0); 
            ...
            case "REDO": 
                ...
                await JSB_MDL_STATUSPRINT_Sub('Sheet \'' + CStr(Dbtblname) + '\' of ' + CStr(Rowcnt) + ' Rows and ' + CStr(UBound(ByRef_Settings.HdrRow)) + ' columns', function (_P1) {  }); 
                ...
                var _ForEndI_12 = UBound(ByRef_Tbl); 
                for (Rowi = 2; Rowi<=_ForEndI_12; Rowi++) {
                   Row = ByRef_Tbl[Rowi]; 
                   Rec = {"RowNumber":Rowi}; 
                   var NAME_LastI13 = UBound(ByRef_Settings.HdrRow); 
                   for (Coli = LBound(ByRef_Settings.HdrRow); Coli<=NAME_LastI13; Coli++) {
                      Name = Extract(ByRef_Settings.HdrRow, Coli, 0, 0); 
                      if (CBool(Name)) {
                         Rec[Name] = Row[Coli];
                      }
                   ...
                   if (await JSB_ODB_WRITEJSON(Rec, Sqlhandle, CStr(Rec[Primarykeyname]))); else {
                      if (CBool(ByRef_Settings.deleteAndRecreateTables) && InStr1(1,me._activeProcess.At_Errors,'truncated')) {
                         if (Rowi>Lastfailedrowi) {
                            Lastfailedrowi = Rowi; 
            
                            var _ForEndI_18 =  +Rowi+100; 
                            for (Rowi =  +Rowi; (Rowi<=_ForEndI_18) && Rowi<=UBound(ByRef_Tbl); Rowi++) {
                               Row = ByRef_Tbl[Rowi]; 
                               Rec = {"RowNumber":Rowi}; 
                               var NAME_LastI19 = UBound(ByRef_Settings.HdrRow); 
                               for (Coli = LBound(ByRef_Settings.HdrRow); Coli<=NAME_LastI19; Coli++) {
                                  Name = Extract(ByRef_Settings.HdrRow, Coli, 0, 0); 
                                  if (CBool(Name)) Rec[Name] = Row[Coli];
                               } 
                            ...
                            await JSB_MDL_STATUSPRINT_Sub('Redoing analysis of metadata', function (_P1) {  }); 
                            me._state = "REDO"; continue atgoto;
                         }
                      }
            
                      me._activeProcess.At_Errors += crlf + CStr(Rec); 
                      return exit(false);
                   }
            
                   if (Rowi % 1000==0) { await JSB_MDL_STATUSPRINT_Sub('Writing row ' + CStr(Rowi) + ' of ' + CStr(UBound(ByRef_Tbl)), function (_P1) {  }); }
                } 
            
                // record column names
                Hdrs = {}; 
                Cnu = ByRef_Settings.columnNamesUsed; 
                var NAME_LastI22 = UBound(ByRef_Settings.HdrRow); 
                for (I = LBound(ByRef_Settings.HdrRow); I<=NAME_LastI22; I++) {
                   Name = Extract(ByRef_Settings.HdrRow, I, 0, 0); 
                   if (CBool(Name)) {
                      Hdrs[Name] = I; 
...
async function JSB_MDL_PICKATABLE(Defaulttablename, Title) { 
    var me = new jsbRoutine("JSB_MDL", "pickatable", "JSB_MDL_PICKATABLE");
    // local variables
    ...
    Newtablename = await JSB_BF_INPUTBOX('Create Table', 'New Table Name', '', '80%', '', function (_P1) {  }); 
...
async function JSB_MDL_PICKTABLE_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "picktable", "JSB_MDL_PICKTABLE_Pgm");
    // local variables
...
async function JSB_MDL_PICKVIEWINPUTS_Sub(ByRef_Projectname, Pagename, Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "pickviewinputs", "JSB_MDL_PICKVIEWINPUTS_Sub");
    // local variables
    ...
    if (await JSB_ODB_READJSON(Pagedataset, await JSB_BF_FHANDLE('dict', ByRef_Projectname), CStr(Pagename), function (_Pagedataset) { Pagedataset = _Pagedataset })); else {
       Alert('Unable to find page ' + CStr(Pagename) + ' ' + me._activeProcess.At_Errors);
       return exit(undefined);
    }

    if (await JSB_ODB_READJSON(Viewdataset, await JSB_BF_FHANDLE('dict', ByRef_Projectname), CStr(Viewname), function (_Viewdataset) { Viewdataset = _Viewdataset })); else {
       Alert('Unable to find view ' + CStr(Pagename) + ' ' + CStr(Viewname) + ' ' + me._activeProcess.At_Errors);
       return exit(undefined);
    }
    ...
    if (Len(Ans)) {
       ...
       if (await JSB_ODB_WRITEJSON(Viewdataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(Viewname))); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
...
async function JSB_MDL_PICKVIEWOUTPUTS_Sub(ByRef_Projectname, Pagename, Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "pickviewoutputs", "JSB_MDL_PICKVIEWOUTPUTS_Sub");
    // local variables
    var Pagedataset, Viewdataset, Ans;
    ...
    At_Session.Item('SQLCOLUMNS', await JSB_BF_GETTABLECOLUMNDEFS(Viewdataset.tableName, CStr(true), true, function (_P1) {  })); 
    At_Session.Item('VIEWCOLUMNS', Viewdataset.columns); 
...
async function JSB_MDL_PKCOLUMNID(Viewmodel, Projectname, Viewname) { 
    var me = new jsbRoutine("JSB_MDL", "pkcolumnid", "JSB_MDL_PKCOLUMNID");
    // local variables
    ...
    if (CBool(Projectname) && CBool(Viewname)) {
       ...
       if (await JSB_ODB_READJSON(Model, Phandle, CStr(Viewname), function (_Model) { Model = _Model })) {
          ...
          if (await JSB_ODB_WRITEJSON(Model, Phandle, CStr(Viewname))); else return Stop(me._activeProcess.At_Errors);
       }
    }
...
async function JSB_MDL_POSSIBLEINPUTCOLUMNS(ByRef_Projectname, Pagename, Currentviewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "possibleinputcolumns", "JSB_MDL_POSSIBLEINPUTCOLUMNS");
    // local variables
    ...
    if (await JSB_ODB_READJSON(Viewdataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(Currentviewname), function (_Viewdataset) { Viewdataset = _Viewdataset })) {
       Columns = await JSB_BF_GETTABLECOLUMNDEFS(Viewdataset.tableName, CStr(true), true, function (_P1) {  });
    } else {
       // No table?
       Columns = [undefined, ];
    }
...
async function JSB_MDL_PROCESSEXCELFILE(ByRef_Settings, ByRef_Basedirectory, ByRef_Xlsfilename, ByRef_Sqldbname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "processexcelfile", "JSB_MDL_PROCESSEXCELFILE");
    // local variables
    ...
    if (Not(Success)) {
       me._activeProcess.At_Errors = 'Could not open ' + CStr(ByRef_Xlsfilename); 
       return exit(false);
    }
...
async function JSB_MDL_PROCESSNEWDATAROW(ByRef_Settings, ByRef_Tbl, ByRef_Row, ByRef_Rowi, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "processnewdatarow", "JSB_MDL_PROCESSNEWDATAROW");
    // local variables
    ...
    if (CBool(ByRef_Settings.DataMustHaveOneNumericRow)) {
       ...
       if (Not(Validdata)) {
          if (await JSB_MDL_ISROWEMPTY(ByRef_Tbl[UBound(ByRef_Tbl)], function (_P1) {  })) {
    ...
    if (CBool(ByRef_Settings.DefaultEmptyValuesFromPreviousRow) && UBound(ByRef_Tbl)>1) {
       ...
       var TERM_LastI6 = UBound(ByRef_Row); 
       for (Termi = LBound(ByRef_Row); Termi<=TERM_LastI6; Termi++) {
          Term = Extract(ByRef_Row, Termi, 0, 0); 
          if (Not(Term)) {
...
async function JSB_MDL_PROCESSNEWHEADER(ByRef_Settings, ByRef_Tbl, ByRef_Rows, ByRef_Rowi, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "processnewheader", "JSB_MDL_PROCESSNEWHEADER");
    // local variables
    ...
    if (await JSB_MDL_ISROWEMPTY(ByRef_Rows( +ByRef_Rowi+1), function (_P1) {  })) {
    ...
    if (Not(ByRef_Settings.SideBySideTables)) {
       ...
       while (true) {
          ...
          for (I = 1; I<=_ForEndI_4; I++) {
             Cell = Nxtrow[I]; 
             if (CBool(Cell)) {
                if (I<Null0(ByRef_Settings.FirstC)) ByRef_Settings.FirstC = I;
                if (I>Null0(ByRef_Settings.LastC)) ByRef_Settings.LastC = I;
             }
          } 
    ...
    var TERM_LastI18 = UBound(Hdrrow); 
    for (Termi = LBound(Hdrrow); Termi<=TERM_LastI18; Termi++) {
       Term = Extract(Hdrrow, Termi, 0, 0); 
       Term = Trim(Term); 
    ...
    if (UBound(Hdrrow) && Noofemptycells>(UBound(Hdrrow)/2)) return exit(false);

    // Does the header contain numbers?  Problably not really a header then
    if (Noofnumbers>1 && UBound(Hdrrow)==1) {
       // Maybe they forgot to make a header?
       H = [undefined, ]; 
       var TERM_LastI23 = UBound(Hdrrow); 
       for (I = LBound(Hdrrow); I<=TERM_LastI23; I++) {
          Term = Extract(Hdrrow, I, 0, 0); 
...
async function JSB_MDL_PROCESSSHEET(ByRef_Settings, ByRef_Sheetname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "processsheet", "JSB_MDL_PROCESSSHEET");
    // local variables
    ...
    while (true) {
       ...
       for (Rowi =  +Rowi+1; Rowi<=_ForEndI_3; Rowi++) {
          if (Rowi % 1000==0) { await JSB_MDL_STATUSPRINT_Sub('Extrcting row ' + CStr(Rowi) + ' of ' + CStr(Rowcount), function (_P1) {  }); }
       ...
       if (Not(Rowi<Rowcount)) break; 
       // clear range 
       var _ForEndI_15 =  +Rowi; 
       for (Ri = CNum(ByRef_Settings.firstR); Ri<=_ForEndI_15; Ri++) {
          Srcrow = Rows[Ri]; 
          var _ForEndI_16 = UBound(Srcrow); 
          for (Ci = CNum(ByRef_Settings.FirstC); (Ci<=_ForEndI_16) && Ci<=Null0(ByRef_Settings.LastC); Ci++) {
             Srcrow[Ci] = ''; 
          } 
       } 
    } 
...
async function JSB_MDL_PROCESSTEMPLATE_Sub(ByRef_Code, Templatefilename, Templatename, Template, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "processtemplate", "JSB_MDL_PROCESSTEMPLATE_Sub");
    // local variables
    ...
    if (CBool(S) || CBool(E)) {
       ...
       for (I =  +Lno; I<=_ForEndI_14; I++) {
          if (I>'0') Println(I, ' ', Change(Change(Extract(Originaltemplate,I,0,0),'\<%',JSB_HTML_BOLD('\<%')),'%\>',JSB_HTML_BOLD('%\>')));
       } 
       ...
       Println(); 
       Println(JSB_BF_HTML('\<a href=' + JSB_BF_JSBROOTEXECUTE('ED ' + CStr(Templatefilename) + ' ' + CStr(Templatename) + ' (' + CStr(Lno2), CStr(true)) + ' target="_blank"\>' + Chr(29) + 'ED ' + CStr(Templatefilename) + ' ' + CStr(Templatename) + Chr(28) + '\</a\>')); 
       Println(); 
...
// <PROJECT_MENUS_Sub>
async function JSB_MDL_PROJECT_MENUS_Sub(ByRef_Menubar, Edpname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "project_menus", "JSB_MDL_PROJECT_MENUS_Sub");
    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Menubar)
        return v
    }
    ...
    if (CBool(isAdmin())) {
       var Frompage = ''; 
       if (InStr1(1,JSB_BF_URL(),'\\' + Edpname)) Frompage = JSB_BF_URL(); else Frompage = Edpname;

       ByRef_Menubar['Web Page'] = {}; 
       ByRef_Menubar['Web Page']['View Page Props'] = {}; 
       ByRef_Menubar['Web Page']['View Page Props'].id = ('?edp&projectName=' + urlEncode(CStr(System(27, me))) + '&pageName=' + urlEncode(Edpname) + '&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Web Page']['ReGen Page'] = {}; 
       ByRef_Menubar['Web Page']['ReGen Page'].id = ('?ReGenPageModel&projectName=' + urlEncode(CStr(System(27, me))) + '&pageName=' + urlEncode(Edpname) + '&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Web Page'].seperator1 = true; 
       ByRef_Menubar['Web Page']['Add Page'] = {}; 
       ByRef_Menubar['Web Page']['Add Page'].id = ('?edp&projectName=' + urlEncode(CStr(System(27, me))) + '&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Web Page']['Clone Page As'] = {}; 
       ByRef_Menubar['Web Page']['Clone Page As'].id = ('?clonePageModel&projectName=' + urlEncode(CStr(System(27, me))) + '&pageName=' + urlEncode(Edpname) + '&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Web Page'].seperator2 = true; 
       ByRef_Menubar['Web Page']['Rename Page'] = {}; 
       ByRef_Menubar['Web Page']['Rename Page'].id = ('?renamePageModel&projectName=' + urlEncode(CStr(System(27, me))) + '&pageName=' + urlEncode(Edpname) + '&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Web Page'].seperator3 = true; 
       ByRef_Menubar['Web Page']['Delete Page'] = {}; 
       ByRef_Menubar['Web Page']['Delete Page'].id = ('?deletePageModel&projectName=' + urlEncode(CStr(System(27, me))) + '&pageName=' + urlEncode(Edpname) + '&fromPage=Menu'); 

       // Can collaspe here after Any HTML
       ByRef_Menubar.collaspable = true; 

       ByRef_Menubar['Menus'] = {}; 
       ByRef_Menubar['Menus']['Main Menu'] = {}; 
       ByRef_Menubar['Menus']['Main Menu'].id = '?Menu'; 
       ByRef_Menubar['Menus']['Add Page To Menu'] = {}; 
       ByRef_Menubar['Menus']['Add Page To Menu'].id = ('?addPageToMenu&projectName=' + urlEncode(CStr(System(27, me))) + '&pageName=' + urlEncode(Edpname) + '&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Menus']['Add Page To Reference Menu'] = {}; 
       ByRef_Menubar['Menus']['Add Page To Reference Menu'].id = ('?addPageToReferenceMenu&projectName=' + urlEncode(CStr(System(27, me))) + '&pageName=' + urlEncode(Edpname) + '&fromPage=' + urlEncode(Frompage)); 

       ByRef_Menubar['TCL'] = {}; 
       ByRef_Menubar['TCL'].id = 'tcl'; 

       ByRef_Menubar['Web Security'] = {}; 
       ByRef_Menubar['Web Security']['Security'] = {}; 
       ByRef_Menubar['Web Security']['Security'].id = ('?WebSiteSecurity&projectName=' + urlEncode(CStr(System(27, me))) + '&fromPage=' + urlEncode(Frompage)); 

       ByRef_Menubar['Websites'] = {}; 
       ByRef_Menubar['Websites']['ReGen MenuBar'] = {}; 
       ByRef_Menubar['Websites']['ReGen MenuBar'].id = ('?updateMenus&projectName=' + urlEncode(CStr(System(27, me))) + '&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Websites'].seperator1 = true; 
       ByRef_Menubar['Websites']['ReGen Website'] = {}; 
       ByRef_Menubar['Websites']['ReGen Website'].id = ('?RegenWebSiteModel&projectName=' + urlEncode(CStr(System(27, me))) + '&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Websites'].seperator2 = true; 
       ByRef_Menubar['Websites']['New Website'] = {}; 
       ByRef_Menubar['Websites']['New Website'].id = ('?newWebSiteModel&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Websites']['Clone Website'] = {}; 
       ByRef_Menubar['Websites']['Clone Website'].id = ('?cloneWebSite&fromPage=' + urlEncode(Frompage)); 
       ByRef_Menubar['Websites'].seperator3 = true; 
       ByRef_Menubar['Websites']['Delete Website'] = {}; 
       ByRef_Menubar['Websites']['Delete Website'].id = ('?deleteWebSiteModel&projectName=' + urlEncode(CStr(System(27, me))) + '');
    }

    var Custommenu = 'mdl.' + nicename(Edpname) + '_customMenu'; 
    await asyncCallByName(Custommenu, me, 1 /*ignore if missing */, ByRef_Menubar); 
    return exit(); 
}
// </PROJECT_MENUS_Sub>
...
async function JSB_MDL_PUTROWDATAINTOHTML(ByRef_Projectname, ByRef_Objectmodel, ByRef_Row, ByRef_Htmlbackdrop, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "putrowdataintohtml", "JSB_MDL_PUTROWDATAINTOHTML");
    // local variables
    ...
    for (Column of iterateOver(ByRef_Objectmodel.columns)) {
       ...
       if (isEmpty(Column.name)) Column.name = 'no id';
       if (Column.display!='hidden') {
          Nicecolumnname = nicename(CStr(Column.name)); 
          ...
          if (CBool(Column.pickfunction)) {
             if (InStr1(1,Column.pickfunction,'.page')) Pickurl = dropRight(CStr(Column.pickfunction), '.page'); else Pickurl = Column.pickfunction;
             Pickurl = Change(Pickurl,'{projectname}',urlEncode(CStr(ByRef_Projectname))); 
             Newctl = JSB_HTML_ADDPICK(CStr(Newctl), CStr(Nicecolumnname), 'Pick a ' + CStr(Ctllabel), '80%', '90%', CStr(Pickurl), CStr(Column.autopostback));
          }
          ...
          if (I=='0') I = InStr1(1,Backdrophtml,'\<mdlctl id=\'ctl_' + CStr(Nicecolumnname) + '\'');
          if (CBool(I)) {
          ...
          if (CBool(I) && CBool(J)) {
             ...
             if (Oldctl!=Newctl) {
                ...
                if (Oldstylescnt==Oldstylescnt && Left(Callname,9)!='ctl_json_') {
             ...
             // Place a new control
             C1 = JSB_BF_LBLCTLSET(CStr(Column.label), CStr(Newctl), CStr(Nicecolumnname), CNum(Column.fullline), CNum(Column.suppresslabel), CStr(Column.lblstyle)); 
...
async function JSB_MDL_READONLYCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "readonlycolumn", "JSB_MDL_READONLYCOLUMN_Sub");
    // local variables
    var Dataset, Columnidx;
...
async function JSB_MDL_REBUILD_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "rebuild", "JSB_MDL_REBUILD_Pgm");
    // %options aspxC-
    return Chain('Regen ' + dropLeft(me._activeProcess.At_Sentence, ' ', CStr(1))); 
}
// </REBUILD_Pgm>

// <REFRESHVIEWMODEL>
async function JSB_MDL_REFRESHVIEWMODEL(Viewname, Viewmodel, Mergedict, Getalldictcols) { 
    var me = new jsbRoutine("JSB_MDL", "refreshviewmodel", "JSB_MDL_REFRESHVIEWMODEL");
    // local variables
    ...
    Dicttablemeta.columns = await JSB_BF_GETTABLECOLUMNDEFS(Viewmodel.tableName, CStr(false), true, function (_P1) {  }); // Suppress Messages, doRefresh
    ...
    Viewcolumns = Sort(CStr(Viewcolumns), '#index'); 

    if (CBool(Updated)) Viewmodel.columns = Viewcolumns;

    if (!Mergedict && CBool(Updated)) {
       if (await JSB_ODB_OPEN('DICT', CStr(Viewmodel.tableName), Fdicthandle, function (_Fdicthandle) { Fdicthandle = _Fdicthandle })); else return Stop(At(-1), 'refreshViewModel: edv: ', me._activeProcess.At_Errors);

       if (await JSB_ODB_WRITEJSON(Viewmodel, Fdicthandle, Viewname)); else null;
    }
...
async function JSB_MDL_REGEN_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "regen", "JSB_MDL_REGEN_Pgm");
    // local variables
    ...
    if (await JSB_BF_PARSESELECT(me._activeProcess.At_Sentence, Defaulttop, Top, Columns, Dictdata, Tablename, Filterby, Orderby, Options, Allitems, F_File, false, Ignorefilenotfound, undefined, function (_Top, _Columns, _Dictdata, _Tablename, _Filterby, _Orderby, _Options, _Allitems, _F_File) { Top = _Top; Columns = _Columns; Dictdata = _Dictdata; Tablename = _Tablename; Filterby = _Filterby; Orderby = _Orderby; Options = _Options; Allitems = _Allitems; F_File = _F_File })) {
       Fname = LTrim(Dictdata + ' ' + Tablename); 

       if (!Fname) {
          if (!Ignorefilenotfound) {
             if (Not(Help)) Errors[Errors.length] = Field(me._activeProcess.At_Sentence,' ',1) + ' TableName ItemID(s) (Options'; else Errors = Help;
          }
       } else {
          if (Allitems || Filterby=='*' || (!Filterby && Dftallitems && Not(System(11, me)))) {
             Filterby = ''; 
             clearSelect(odbActiveSelectList); 
             if (await JSB_ODB_SELECT('', F_File, '', '')); else return Stop(me._activeProcess.At_Errors);
          } else {
             if (Filterby) {
                Filterby = LTrim(RTrim(Filterby + Orderby)); 
                if (await JSB_ODB_SELECT(LTrim(Top + ' ') + Join(Columns,' '), F_File, Filterby, '')); else return Stop(me._activeProcess.At_Errors);
             }
          }
       }
    } else {
       if ((!Tablename || Tablename=='?') && CBool(Help)) Errors = Help; else Errors[Errors.length] = me._activeProcess.At_Errors;
       Fname = LTrim(Dictdata + ' ' + Tablename);
    }
    ...
    Nomodelingstuff = InStr1(1,Options,'N'); 
    Fdict = await JSB_BF_FHANDLE('dict', Fname); 

    Errors = [undefined, ]; 
    Lctime = At_Session.Item('LastMenuCompile_' + Fname); 

    var INAME_LastI16 = UBound(Inames); 
    for (Itemi = LBound(Inames); Itemi<=INAME_LastI16; Itemi++) {
       Iname = Extract(Inames, Itemi, 0, 0); 
    ...
    if (CBool(Errors)) Println(JSB_HTML_MONOSPACE(Join(Errors,crlf)));
...
async function JSB_MDL_REGENPAGEMODEL_Sub(Projectname, Ipagename, Showresults, Nostopping, Nomodelingstuff) { 
    var me = new jsbRoutine("JSB_MDL", "regenpagemodel", "JSB_MDL_REGENPAGEMODEL_Sub");
    // local variables
    ...
    if (await JSB_ODB_READJSON(Pagedataset, await JSB_BF_FHANDLE('dict', Projectname), CStr(Pagename), function (_Pagedataset) { Pagedataset = _Pagedataset })); else {
       Println('Unable to find page ', Pagename, ' ', me._activeProcess.At_Errors); 
       return;
    }
...
async function JSB_MDL_REGENPAGEMODEL_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "REGENPAGEMODEL", "JSB_MDL_REGENPAGEMODEL_Pgm");
    // local variables
    ...
    Print(At(-1), 'Complete..'); 
    if (CBool(Frompage)) return At_Response.Redirect(Frompage);
    if (Not(System(22, me))) { await JSB_MDL_RESPONSEREDIRECT(JSB_BF_JSBROOTEXECUTE(CStr(Pagename)), function (_P1) {  }); }
    return; 
}
// </REGENPAGEMODEL_Pgm>
...
async function JSB_MDL_REGENWEBSITEMODEL_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "regenwebsitemodel", "JSB_MDL_REGENWEBSITEMODEL_Pgm");
    // local variables
    ...
    atgoto: while (true) {
        switch (me._state) {
            case "":
                ...
                if (await JSB_ODB_OPEN('', CStr(Projectname), Scp, function (_Scp) { Scp = _Scp })); else { me._state = "DONE"; continue atgoto; }
                if (await JSB_ODB_OPEN('dict', CStr(Projectname), Dscp, function (_Dscp) { Dscp = _Dscp })); else { me._state = "DONE"; continue atgoto; }
            
                Print(At(-1)); 
                if (await JSB_ODB_SELECTTO('', Dscp, 'ItemID Like \'%.page\'', Sl, function (_Sl) { Sl = _Sl })); else return Stop(me._activeProcess.At_Errors);
                L = getList(Sl); 
            
                L = await JSB_BF_INPUTMULTISELECT('Select pages to regenerate', 'Pages', CStr(L), CStr(L)); 
                if (Len(L)==0 || L==Chr(27)) { me._state = "DONE"; continue atgoto; }
            ...
            case "DONE": 
                ...
                if (CBool(Frompage)) return At_Response.Redirect(Frompage);
                if (JSB_BF_HASTCLPARENT() || JSB_BF_HASPARENTPROCESS()) return Stop();
                return; 
...
async function JSB_MDL_RENAMEPAGEMODEL_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "renamepagemodel", "JSB_MDL_RENAMEPAGEMODEL_Pgm");
    // local variables
    var Projectname, Oldpagename, Frompage, Closewindow, Sentence;
    var Newpagename;
    ...
    if (Newpagename==Chr(27) || isEmpty(Newpagename)) {
       if (CBool(Frompage)) { await JSB_MDL_RESPONSEREDIRECT(Frompage, function (_Frompage) { Frompage = _Frompage }); }
       if (CBool(Closewindow) || !JSB_BF_HASPARENTPROCESS()) At_Server.End();
       await JSB_MDL_RESPONSEREDIRECT(JSB_BF_JSBROOTEXECUTE(CStr(Oldpagename)), function (_P1) {  });
    }
    ...
    if (CBool(Frompage) || JSB_BF_HASTCLPARENT()) {
       if (await JSB_BF_MSGBOX('Where to?', 'Go to new page?', '*Yes,No')!='Yes') { await JSB_MDL_RESPONSEREDIRECT(Frompage, function (_Frompage) { Frompage = _Frompage }); }
    } else {
       if (CBool(Closewindow) || !JSB_BF_HASPARENTPROCESS()) At_Server.End();
    }
    ...
    await JSB_MDL_RESPONSEREDIRECT(JSB_BF_JSBROOTEXECUTE(CStr(Newpagename)), function (_P1) {  });
    return; 
}
// </RENAMEPAGEMODEL_Pgm>
...
async function JSB_MDL_RENAMEVIEW(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Newviewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "renameview", "JSB_MDL_RENAMEVIEW");
    // local variables
    ...
    if (await JSB_ODB_READJSON(Dataset, Fdict, CStr(ByRef_Viewname) + '.view', function (_Dataset) { Dataset = _Dataset })); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);

    if (Not(ByRef_Newviewname)) {
       ByRef_Newviewname = Trim(await JSB_BF_INPUTBOX('Rename view', 'Enter a new name', CStr(ByRef_Viewname), '80%', '', function (_P1) {  })); 
...
async function JSB_MDL_REQUIRECOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "requirecolumn", "JSB_MDL_REQUIRECOLUMN_Sub");
    // local variables
    var Dataset, Columnidx;
...
async function JSB_MDL_REQUIREDCOLUMNS_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "requiredcolumns", "JSB_MDL_REQUIREDCOLUMNS_Sub");
    // local variables
...
async function JSB_MDL_RESETBACKGROUND(Projectname, Objectmodel, ByRef_Htmlbackdrop, Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "resetbackground", "JSB_MDL_RESETBACKGROUND");
    // local variables
    var Ans;
...
async function JSB_MDL_RESPONSEREDIRECT(ByRef_Topage, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "responseredirect", "JSB_MDL_RESPONSEREDIRECT");
    function exit(v) {
    ...
    Print(At(-1), JSB_HTML_SCRIPT('window.open(' + JSB_BF_JSESCAPESTRING(CStr(ByRef_Topage)) + ')')); 
    FlushHTML();
    return Stop(); 
}
// </RESPONSEREDIRECT>
...
async function JSB_MDL_RESTAPIFETCH_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "restApiFetch", "JSB_MDL_RESTAPIFETCH_Pgm");
    // local variables
    var Grpi, Innsersql, Grpi2, Rec;

    var Notimeout = System(56, me); 
    var Clause = undefined; 
    var Databasename = paramVar('databaseName'); 
    var Tablename = paramVar('tableName'); 

    var Result = undefined; 

    if (await JSB_ODB_ATTACHDB(Databasename)); else { Result = {"errors":me._activeProcess.At_Errors}; await JSB_MDL_RTNRESULT(Result, function (_Result) { Result = _Result });return; }
    var Ftable = undefined; 
    if (await JSB_ODB_OPEN('', Tablename, Ftable, function (_Ftable) { Ftable = _Ftable })); else { Result = {"errors":me._activeProcess.At_Errors}; await JSB_MDL_RTNRESULT(Result, function (_Result) { Result = _Result });return; }

    if (!JSB_BF_ISSQLSERVER(Ftable)) { Result = {"errors":'not a arySQL table'}; await JSB_MDL_RTNRESULT(Result, function (_Result) { Result = _Result });return; }

    var Skip = paramVar('skip'); 
    var Take = paramVar('take'); 
    var Requiretotalcount = paramVar('requireTotalCount'); 
    var Requiregroupcount = paramVar('requireGroupCount'); 
    var Order = paramVar('sort'); 
    var Filter = paramVar('filter'); 
    var Totalsummary = paramVar('totalSummary'); 
    var Group = paramVar('group'); 
    var Groupsummary = paramVar('groupSummary'); 
    ...
    if (Not(Group)) {
       ...
       if (await asyncDNOSqlSelect(Sqlcmd, _selectList => Sl = _selectList)); else { Result = {"errors":me._activeProcess.At_Errors + '; ' + Databasename + ': ' + CStr(Arysql)}; await JSB_MDL_RTNRESULT(Result, function (_Result) { Result = _Result });return; }
    ...
    for (Grpi = UBound(Arygroup)+1; Grpi>=1; Grpi--) {
       ...
       if (Grpi>UBound(Arygroup)) {
          Newsql[Newsql.length] = '(select * from [' + Tablename + '] As F' + CStr(Grpi);
       } else {
          Clause = Arygroup[Grpi]; 
          if (CBool(Clause.isExpanded)) {
             Newsql[Newsql.length] = CStr((Grpi>1?'(':'')) + 'select [' + CStr(Clause.selector) + '] As "key", count(*) As "count", items = '; 
             for (Innsersql of iterateOver(Arysql)) {
                Newsql[Newsql.length] = '   ' + CStr(Innsersql); 
             }
          } else {
             Newsql[Newsql.length] = CStr((Grpi>1?'(':'')) + 'select [' + CStr(Clause.selector) + '] As "key", count(*) As "count", items = null';
          }
       }
    ...
    if (await asyncDNOSqlSelect(Ssql, _selectList => Sl = _selectList)); else { Result = {"errors":me._activeProcess.At_Errors + '; ' + Databasename + ': ' + Ssql}; await JSB_MDL_RTNRESULT(Result, function (_Result) { Result = _Result });return; }
...
async function JSB_MDL_RTNRESULT(ByRef_Result, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "RESTAPIFETCH", "JSB_MDL_RTNRESULT");
    function exit(v) {
...
async function JSB_MDL_RESTAPIBUILDWHERE(Filter) { 
    var me = new jsbRoutine("JSB_MDL", "RESTAPIFETCH", "JSB_MDL_RESTAPIBUILDWHERE");
    // local variables
    var Filterby, Clause;
...
async function JSB_MDL_ROWHASNUMBERS(ByRef_Row, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "rowhasnumbers", "JSB_MDL_ROWHASNUMBERS");
    // local variables
...
async function JSB_MDL_SETBUTTONURL_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "setbuttonurl", "JSB_MDL_SETBUTTONURL_Sub");
    // local variables
...
async function JSB_MDL_SETCOLUMNDEFAULTS_Sub(ByRef_Datatype, ByRef_Viewcolumn, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "setcolumndefaults", "JSB_MDL_SETCOLUMNDEFAULTS_Sub");
    function exit(v) {
...
async function JSB_MDL_SETDEFAULTS(ByRef_Objectname, ByRef_Columns, ByRef_Row, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "setdefaults", "JSB_MDL_SETDEFAULTS");
    // local variables
    ...
    for (Column of iterateOver(ByRef_Columns)) {
       ...
       if (CBool(Cname)) {
          if (isNothing(ByRef_Row[Cname])) {
             Dvalue = JSB_BF_GETDEFAULTVALUE(CStr(ByRef_Row[Cname]), CStr(Column.defaultvalue), CStr(Cname), CStr(ByRef_Objectname)); 
             if (!isNothing(Dvalue)) {
...
async function JSB_MDL_SETVIEWHEADER_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "setviewheader", "JSB_MDL_SETVIEWHEADER_Sub");
    // local variables
    ...
    if (await JSB_ODB_READJSON(Dataset, await JSB_BF_FHANDLE('DICT', ByRef_Projectname), CStr(ByRef_Viewname), function (_Dataset) { Dataset = _Dataset })); else return exit(undefined);
    Newvalue = Trim(await JSB_BF_INPUTBOX('View Header for' + CStr(ByRef_Viewname), 'Header', CStr(Dataset.header), '80%', '', function (_P1) {  })); 
...
async function JSB_MDL_SETVISIBLECOLUMNS_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "setvisiblecolumns", "JSB_MDL_SETVISIBLECOLUMNS_Sub");
    // local variables
    var Dataset, Allcolumns, Selcolumns, Column, Cname, Selectedcols;
...
function show1000ContextMenu(Gridid, Columnname) { 
    var me = new jsbRoutine("JSB_MDL", "show1000contextmenu", "show1000ContextMenu");
    // local variables
    var niceColumnName, niceGridName, CtlID, Cm;

    // %options aspxC-

    niceColumnName = nicename(CStr(Columnname)); 
    niceGridName = nicename(CStr(Gridid)); 
...
"Show Column":{"cmd":'ShowCol',"val":CtlID}
    ...
    return JSB_HTML_CONTEXTMENU('[id=\'' + CStr(CtlID) + '\']', Cm, true, CStr(CtlID) + '_contextMenu'); 
}
// </SHOW1000CONTEXTMENU>
...
async function JSB_MDL_SHOWEXCELFORM(ByRef_Centerviewvars, ByRef_Basedirectory, ByRef_Xlsfilename, ByRef_Sqldbname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "showexcelform", "JSB_MDL_SHOWEXCELFORM");
    // local variables
    var Form, Settings, Dstdb, Options;
    ...
    if (!(await JSB_BF_DBDATABASEEXISTS(CStr(Dstdb)))) {
       if (await JSB_ODB_ATTACHDB('')); else null;
       if (!(await JSB_BF_DBCREATESQLDATABASE(CStr(Dstdb)))) { await JSB_MDL_STATUSPRINT_Sub(me._activeProcess.At_Errors, function (_At_Errors) { me._activeProcess.At_Errors = _At_Errors }); return exit(false); }
    }

    if (Not(await JSB_ODB_ATTACHDBEXTENDED(Dstdb, 'databaseBuilder_rw', function (_Dstdb, _P2) { Dstdb = _Dstdb }))) { await JSB_MDL_STATUSPRINT_Sub(me._activeProcess.At_Errors, function (_At_Errors) { me._activeProcess.At_Errors = _At_Errors }); return exit(false); }

    Options = 'Importing Excel file: ' + CStr(ByRef_Xlsfilename) + ' into SQL Database ' + CStr(Dstdb) + crlf; 
    Options = CStr(Options) + '   Option: Data rows must have at least one numeric cell ' + CStr(Settings.DataMustHaveOneNumericRow) + crlf; 
    Options = CStr(Options) + '   Option: Allow Side By Side Tables ' + CStr(Settings.SideBySideTables) + crlf; 
    Options = CStr(Options) + '   Option: Ignore Tables With No Rows ' + CStr(Settings.ignoreTablesWithNoSettingss) + crlf; 
    Options = CStr(Options) + '   Option: Ignore Tables With Less Than ' + CStr(Settings.ignoreTablesWithLessThanNColumns) + ' Columns ' + crlf; 
    Options = CStr(Options) + '   Option: One Table Per Sheet ' + CStr(Settings.OneTablePerSheet) + crlf; 
    Options = CStr(Options) + '   Option: Allow Data to have Blank Rows ' + CStr(Settings.AllowBlankRows) + crlf; 
    Options = CStr(Options) + '   Option: Default Empty Values From Previous Row ' + CStr(Settings.DefaultEmptyValuesFromPreviousRow) + crlf; 
    Options = CStr(Options) + '   Option: Delete And Recreate Sql Table ' + CStr(Settings.deleteAndRecreateTables) + crlf; 
    Options = CStr(Options) + '   Option: Show Imported Data ' + CStr(Settings.showTables) + crlf; 
    await JSB_MDL_STATUSPRINT_Sub(Options, function (_Options) { Options = _Options }); 

    if (Not(await JSB_MDL_PROCESSEXCELFILE(Settings, ByRef_Basedirectory, ByRef_Xlsfilename, Dstdb, function (_Settings, _ByRef_Basedirectory, _ByRef_Xlsfilename, _Dstdb) { Settings = _Settings; ByRef_Basedirectory = _ByRef_Basedirectory; ByRef_Xlsfilename = _ByRef_Xlsfilename; Dstdb = _Dstdb }))) { await JSB_MDL_STATUSPRINT_Sub(me._activeProcess.At_Errors, function (_At_Errors) { me._activeProcess.At_Errors = _At_Errors }); return exit(false); }

    await JSB_MDL_STATUSPRINT_Sub('Done', function (_P1) {  }); 
    return exit(true); 
}
// </SHOWEXCELFORM>
...
async function JSB_MDL_SHOWTABLE(ByRef_Settings, ByRef_Tbl, ByRef_Sheetname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "showtable", "JSB_MDL_SHOWTABLE");
    // local variables
    var Html, Col, Coli, Rc, Rowi, Row;

    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Settings, ByRef_Tbl, ByRef_Sheetname)
        return v
    }
    Html = JSB_BF_HTML('\<style\>\r\n\
        table, th, td {\r\n\
    ...
    Html = CStr(Html) + JSB_BF_HTML('\<table\>'); 

    ByRef_Settings.HdrRow = ByRef_Tbl[1]; 
    Html = CStr(Html) + JSB_BF_HTML('\<tr\>'); 
    var COL_LastI1 = UBound(ByRef_Settings.HdrRow); 
    for (Coli = LBound(ByRef_Settings.HdrRow); Coli<=COL_LastI1; Coli++) {
       Col = Extract(ByRef_Settings.HdrRow, Coli, 0, 0); 
       Html = CStr(Html) + JSB_BF_HTML('\<th\>') + CStr(Col) + JSB_BF_HTML('\</th\>'); 
    } 
    Html = CStr(Html) + JSB_BF_HTML('\</tr\>'); 

    Rc = UBound(ByRef_Tbl); 
    var _ForEndI_2 =  +Rc; 
    for (Rowi = 2; (Rowi<=_ForEndI_2) && Rowi<10; Rowi++) {
       Row = ByRef_Tbl[Rowi]; 

       Html = CStr(Html) + JSB_BF_HTML('\<tr\>'); 
       var COL_LastI3 = UBound(Row); 
       for (Coli = LBound(Row); Coli<=COL_LastI3; Coli++) {
          Col = Extract(Row, Coli, 0, 0); 
          Html = CStr(Html) + JSB_BF_HTML('\<td\>') + CStr(Col) + JSB_BF_HTML('\</td\>'); 
       } 
       Html = CStr(Html) + JSB_BF_HTML('\</tr\>'); 
    } 
    Html = CStr(Html) + JSB_BF_HTML('\</table\>'); 
    if (Rowi>10) {
       Html = CStr(Html) + '.......................' + CStr( +Rc-10) + ' more Rows';
    }
    ...
    if (await JSB_MDL_OKAYTOOUTPUTTBL(ByRef_Settings, ByRef_Tbl, ByRef_Sheetname, function (_ByRef_Settings, _ByRef_Tbl) { ByRef_Settings = _ByRef_Settings; ByRef_Tbl = _ByRef_Tbl })) {
       return exit(JSB_HTML_STYLE('.IFrame { background-color: aliceblue; }') + JSB_HTML_SCRIPT('openInTab("Excel2SQL", "' + CStr(ByRef_Sheetname) + '", "' + CStr(ByRef_Sheetname) + '", ' + JSB_BF_JSESCAPESTRING(CStr(Html)) + ')'));
    } else {
       Html = CStr(Html) + JSB_HTML_COLOR('lightgray', CStr(Html)); 
       Html = CStr(Html) + JSB_HTML_COLOR('Red', me._activeProcess.At_Errors); 

       Print(JSB_HTML_SCRIPT('\r\n\
            $("#xlsStatus").append(' + JSB_BF_JSESCAPESTRING(CStr(Html)) + '); \r\n\
            $("#Excel2SQL").tabs("option", "active", 0);\r\n\
        ')); 

       await JSB_MDL_STATUSPRINT_Sub(crlf, function (_P1) {  }); 
       Html = '';
    }
...
async function JSB_MDL_SHOWTOP1000_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "showtop1000", "JSB_MDL_SHOWTOP1000_Pgm");
    // local variables
    var Rolemsg;
    ...
    // Did we run from TCL? setup something for testing
    if (Not(Tableid)) {
       var S = RTrim(Field(me._activeProcess.At_Sentence,'(',1)); 
       var Cmd = Field(S,' ',1); 
       if (LCase(Cmd)=='run') Tableid = Field(S,' ',4); else Tableid = Field(S,' ',2);
    ...
    if (Not(await JSB_MDL_SHOWTOP1000(Tabid, Databasename, Tableid, Reportname, Devxgridlayout))) return Stop(me._activeProcess.At_Errors);
    if (JSB_BF_HASTCLPARENT() || JSB_BF_HASPARENTPROCESS()) return Stop(); else At_Server.End();
    return; 
}
// </SHOWTOP1000_Pgm>
...
async function JSB_MDL_SHOWTOP1000(Tabid, Databasename, Tablename, Reportname, Devxgridlayout) { 
    var me = new jsbRoutine("JSB_MDL", "SHOWTOP1000", "JSB_MDL_SHOWTOP1000");
    // local variables
    ...
    var Prescripts, Postscripts, Html, Ctltext, Ctlid, Contextmenutext;
    ...
    var Gridid = 'showGrid'; 
    Reportname = Databasename + '_gridViewGridDefs_'; 
    ...
    if (Databasename) {
       if (Not(await asyncRpcRequest('ATTACHDBEXTENDED', Databasename, function (_Databasename) {  }))) return false;
       if (CBool(await asyncRpcRequest('DBVIEWEXISTS', Tablename, function (_Tablename) {  }))) {
          var Tmptblname = '##dview_' + Tablename + '_' + nicename(JSB_BF_USERNAME()); 
          if (await JSB_ODB_OPEN('', Tmptblname, Ftable, function (_Ftable) { Ftable = _Ftable })); else {
             Ftable = await asyncRpcRequest('DBMAKETMPTABLE', Tablename, function (_Tablename) {  });
          }
       }
    }

    if (Not(Ftable)) {
       if (await JSB_ODB_OPEN('', Tablename, Ftable, function (_Ftable) { Ftable = _Ftable })); else return false;
    }

    var Columnnames = '*'; 
    var Hideexportbtns = false; 
    var Urlbase = (JSB_BF_JSBROOTACCOUNT() + '?showTop1000&databaseName=' + urlEncode(Databasename) + '&tableID=' + urlEncode(Tablename)); 
    ...
    Tablecolumns = await asyncRpcRequest('GETTABLECOLUMNDEFS', Tablename, true, true, function (_Tablename, _P2, _P3) {  }); 
    ...
    do {
       ...
       if (await JSB_BF_ISMANAGER()) Toolbar = CStr(Toolbar) + JSB_HTML_SUBMITBTN('Btn', 'Make Report');

       Toolbar = CStr(Toolbar) + JSB_HTML_SUBMITBTN('Btn', 'Reset Layout', {"class":'btnReset'}); 
       Toolbar = CStr(Toolbar) + JSB_HTML_SUBMITBTN('Btn', 'Fit to Screen'); 
       Toolbar = CStr(Toolbar) + JSB_HTML_SUBMITBTN('Btn', 'Fit to Data'); 

       if (!Hideexportbtns) {
          Toolbar = CStr(Toolbar) + JSB_HTML_SUBMITBTN('Btn', 'Export to CSV');
       }

       if (Tabid || JSB_BF_HASTCLPARENT()) Toolbar = CStr(Toolbar) + JSB_HTML_SUBMITBTN('Btn', 'Quit');
    ...
    \r\n\
       ...
       Html = CStr(Prescripts) + JSB_HTML_ROWS2('%', CStr(Htmlgrid), '66px', CStr(Toolbar)) + CStr(Postscripts); 

       // Theme screen
       if (Not(Tabid) && CBool(Theme)) {
          Html = await asyncCallByName(Theme, me, 0/*ignore if missing */, System(27, me), System(28, me), Html);
       }
       ...
       Devxgridlayout = JSB_BF_CJSON(formVar(Gridid + '_devXGridLayout')); 
       ...
       if (Cmd=='Fit to Data') {
          ...
          if (await JSB_ODB_SELECTTO('top 100 *', Ftable, '', Selectlist, function (_Selectlist) { Selectlist = _Selectlist })); else return Stop(me._activeProcess.At_Errors);
          ...
          for (Row of iterateOver(Dt)) {
             for (Colname of iterateOver(Row)) {
                V = RTrim(Row[Colname]); 
                Lenofv = Len(V); 
                if (Lenofv>Null0(Colwidths[Colname])) Colwidths[Colname] = Lenofv;
             } 
          } 
          ...
          for (Dc of iterateOver(Dxcolumns)) {
             ...
             if (Len(Caption)+2>Cw) Cw = Len(Caption)+2;
             if (Cw>60) Cw = 60;
          ...
          if (paramVar('filter') || paramVar('layout')) {
             if (await JSB_ODB_DELETEITEM(Ftmp, Reportname)); else return Stop(me._activeProcess.At_Errors);
             if (Tabid) {
                Print(JSB_HTML_SCRIPT('windowOpen(' + JSB_BF_JSESCAPESTRING(Urlbase) + ', \'_blank\'); doJsbSubmit();')); 
                FlushHTML();
                Print(At(-1));
             } else {
       ...
       if (Cmd=='Export KML') {
          ...
          if (Devxgridlayout) Url = (CStr(Url) + '&layout=' + urlEncode(Devxgridlayout));

          await JSB_MDL_RUNSCRIPT('\r\n\
                if (window.self !== window.parent) { \r\n\
                    w = window.parent.open(' + JSB_BF_JSESCAPESTRING(CStr(Url)) + ', \'_blank\'); \r\n\
                    setTimeout(function () { window.location = "close_html"; }, 500);\r\n\
                } else { \r\n\
                    window.open(' + JSB_BF_JSESCAPESTRING(CStr(Url)) + ', \'_self\'); \r\n\
                } \r\n\
            ', function (_P1) {  });
       } else if (Cmd=='Make Report') {
          debugger; 

          Url = (JSB_BF_JSBROOTACCOUNT() + '?makenewreport&databaseName=' + urlEncode(Databasename) + '&tableID=' + urlEncode(Tablename)); 
          await JSB_MDL_RUNSCRIPT('debugger; window.parent.open(' + JSB_BF_JSESCAPESTRING(CStr(Url)) + ', \'_blank\'); jsb.submit();', function (_P1) {  });
       }
...
async function JSB_MDL_RUNSCRIPT(ByRef_S, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "SHOWTOP1000", "JSB_MDL_RUNSCRIPT");
    // local variables
    var Redirectpage;
...
async function JSB_MDL_SHOWTOP1000_MYGRID(Gridid) { 
    var me = new jsbRoutine("JSB_MDL", "showtop1000_mygrid", "JSB_MDL_SHOWTOP1000_MYGRID");
    return System(29, me).getControl(Gridid); 
}
// </SHOWTOP1000_MYGRID>

// <SHOWVIEWCOLUMN_Sub>
async function JSB_MDL_SHOWVIEWCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "showviewcolumn", "JSB_MDL_SHOWVIEWCOLUMN_Sub");
    // local variables
...
async function JSB_MDL_SQLQUERYBUILDER_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "sqlquerybuilder", "JSB_MDL_SQLQUERYBUILDER_Pgm");
    // local variables
    ...
    var Username, Passwrd, Initialcat, B, Mylastsql, Cmd, Ctltext;
    ...
    Btnsave = CStr(Btnsave) + CStr(Hideothersave); 
    Btnsave = ''; 
    ...
    Btndelete = CStr(Btndelete) + CStr(Hideotherdelete); 
    ...
    while (true) {
       Print(At(-1), JSB_HTML_ROWS2('%', CStr(await JSB_MDL_DEVXSQLBUILDER(Querybuilderid, Databasename, Lastxmlquery, CStr(Btnsave) + CStr(Btnquit) + CStr(Btndelete) + CStr(Btnmakereport), Username, Passwrd, Initialcat, function (_Querybuilderid, _Databasename, _Lastxmlquery, _P4, _Username, _Passwrd, _Initialcat) { Querybuilderid = _Querybuilderid; Databasename = _Databasename; Lastxmlquery = _Lastxmlquery; Username = _Username; Passwrd = _Passwrd; Initialcat = _Initialcat })), '66px', CStr(Toolbar))); 
       ...
       Lastxmlquery = System(29, me).lastXmlQuery; 
       Mylastsql = System(29, me).myLastSql; 
       ...
       if (Cmd=='Delete') {
          if (Sqlid!='*NEW*') {
             if (await JSB_ODB_DELETEITEM(Fsql, CStr(Sqlid) + '.devX')); else Alert(me._activeProcess.At_Errors);
             if (await JSB_ODB_DELETEITEM(Fsql, CStr(Sqlid) + '.SQL')); else Alert(me._activeProcess.At_Errors);
             Print(At(-1), JSB_HTML_SCRIPT('\r\n\
                            if (window.parent.reloadDbTree) window.parent.reloadDbTree();\r\n\
                        ')); 
             FlushHTML();
       ...
       if (!isEmpty(Lastxmlquery)) {
          if (Sqlid=='*NEW*') {
             ...
             while (true) {
                ...
                Prefix = nicename(CStr(Databasename)) + '_'; 
                ...
                if (await JSB_ODB_READ(Xx, Fsql, CStr(Sqlid) + '.devX', function (_Xx) { Xx = _Xx })) {
                   ...
                   if (await JSB_ODB_WRITE(CStr(Lastxmlquery), Fsql, CStr(Sqlid) + '.devX')); else Alert(me._activeProcess.At_Errors);
                   if (await JSB_ODB_WRITE(CStr(Mylastsql), Fsql, CStr(Sqlid) + '.SQL')); else Alert(me._activeProcess.At_Errors);
                   ...
                   Print(At(-1), JSB_HTML_SCRIPT('\r\n\
                            if (window.parent.reloadDbTree) {\r\n\
                                window.parent.reloadDbTree();\r\n\
                                window.parent.openInTab("' + CStr(Tabid) + '", ' + JSB_BF_JSESCAPESTRING(CStr(Sqlid)) + ', ' + JSB_BF_JSESCAPESTRING(CStr(Lbl)) + ', ' + JSB_BF_JSESCAPESTRING(CStr(Url)) + ')\r\n\
                                window.close();\r\n\
                            }\r\n\
                        ')); 
                   FlushHTML();
             ...
             if (await JSB_ODB_WRITE(CStr(Lastxmlquery), Fsql, CStr(Sqlid) + '.devX')); else return Stop(me._activeProcess.At_Errors);
             if (await JSB_ODB_WRITE(CStr(Mylastsql), Fsql, CStr(Sqlid) + '.SQL')); else return Stop(me._activeProcess.At_Errors);
          }
       }
       ...
       if (Cmd=='Make Report') {
          Print(At(-1)); 
          if (Sqlid=='*NEW*') {
             if (Not(await JSB_MDL_SHOWTOP1000(CStr(Tabid), CStr(Databasename), CStr(Mylastsql)))) Msg = me._activeProcess.At_Errors;
          } else {
             if (Not(await JSB_MDL_SHOWTOP1000(CStr(Tabid), CStr(Databasename), CStr(Mylastsql), CStr(Sqlid)))) Msg = me._activeProcess.At_Errors;
          }
       }
    } 
...
async function JSB_MDL_STANDARDFORMVIEW(ByRef_Projectname, ByRef_Objectmodel, ByRef_Absolutepositioning, ByRef_Viewname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "standardformview", "JSB_MDL_STANDARDFORMVIEW");
    // local variables
    var Columns, Html, Column, Callname, Ctllabel, Nicecolumnname;
    ...
    Columns = await JSB_MDL_DROPGRIDCOLUMNS(ByRef_Objectmodel.columns, function (_P1) {  }); 
    ...
    for (Column of iterateOver(Columns)) {
       if (Column.display!='hidden') {
          ...
          if (CBool(Column.pickfunction)) {
             if (InStr1(1,Column.pickfunction,'.page')) Pickurl = dropRight(CStr(Column.pickfunction), '.page'); else Pickurl = Column.pickfunction;
             Pickurl = Change(Pickurl,'{projectname}',urlEncode(CStr(ByRef_Projectname))); 
             Ctlhtml = JSB_HTML_ADDPICK(Ctlhtml, CStr(Nicecolumnname), CStr(Pickurl), '80%', '90%', CStr(Pickurl), CStr(Column.autopostback));
          }

          I = InStr1(1,Ctlhtml,'class=\''); 
          if (I=='0') I = InStr1(1,Ctlhtml,'class="');
          ...
          Html[Html.length] = JSB_HTML_LABEL(CStr(Column.label), Nicecolumnname) + Ctlhtml;
       }
    } 
    return exit(Join(Html,'')); 
}
// </STANDARDFORMVIEW>

// <STANDARDUNLOAD>
async function JSB_MDL_STANDARDUNLOAD(Columns, Row, Restricttomodel) { 
    var me = new jsbRoutine("JSB_MDL", "standardunload", "JSB_MDL_STANDARDUNLOAD");
    // local variables
    var Newvaluen;
    ...
    for (Column of iterateOver(Columns)) {
       Cn = Column.name; 
       if (Cn) {
          var V = formVar(nicename(CStr(Column.name))); 
    ...
    for (Column of iterateOver(Columns)) {
       ...
       if (CBool(Column.canedit) && Column.display!='hidden') {
          ...
          break; 
             ...
             case 'datetime': 
                if (CBool(Newvalue)) {
                   Newvaluen = JSB_BF_DATETIME(JSB_BF_DATETIME(Newvalue)); 
                   if (CBool(Newvaluen)) Newvalue = Newvaluen;
                }
          break; 
...
async function JSB_MDL_STATUSPRINT_Sub(ByRef_Text, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "statusprint", "JSB_MDL_STATUSPRINT_Sub");
    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Text)
        return v
    }
    Print(JSB_HTML_SCRIPT('\r\n\
        $("#xlsStatus").append(convertPrint2Html(null, ' + JSB_BF_JSESCAPESTRING(CStr(ByRef_Text) + crlf) + ')); \r\n\
        $("#Excel2SQL").tabs("option", "active", 0);\r\n\
    ')); 
    FlushHTML();
    return exit(); 
...
async function JSB_MDL_SUBTREEURL(ByRef_Tabid, ByRef_Databasename, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "subtreeurl", "JSB_MDL_SUBTREEURL");
    // local variables
    var Url;
    ...
    Url = (JSB_BF_JSBRESTCALL('getDBSubTree') + '?tabID=' + urlEncode(CStr(ByRef_Tabid)) + '&noCache=' + CStr(Timer())); 
    if (CBool(ByRef_Databasename)) Url = (CStr(Url) + '&databaseName=' + urlEncode(CStr(ByRef_Databasename)));
    return exit(Url); 
}
// </SUBTREEURL>
...
async function JSB_MDL_SUPPRESSLABELCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "suppresslabelcolumn", "JSB_MDL_SUPPRESSLABELCOLUMN_Sub");
    // local variables
    var Dataset, Columnidx;
...
async function JSB_MDL_TOOLTIPCOLUMN_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Columnid, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "tooltipcolumn", "JSB_MDL_TOOLTIPCOLUMN_Sub");
    // local variables
    ...
    Newvalue = Trim(await JSB_BF_INPUTBOX('Column Property ' + CStr(ByRef_Columnid), 'Tool Tip', CStr(Tooltip), '80%', 'auto', function (_P1) {  })); 
    if (Newvalue==Chr(27)) return exit(undefined);
    if (Newvalue==Null0(Dataset.columns[Columnidx].label)) Newvalue = '';
...
async function JSB_MDL_UPDATECODE(Genedsrc, Projectname, Itemid) { 
    var me = new jsbRoutine("JSB_MDL", "updatecode", "JSB_MDL_UPDATECODE");
    // local variables
    var Usrmodifiedsrc, Blockname, Genedblock, Priorblock, Results;

    // %options aspxC-

    var Fproject = await JSB_BF_FHANDLE(Projectname); 
    ...
    if (await JSB_ODB_READ(Usrmodifiedsrc, await JSB_BF_FHANDLE(Projectname), Itemid, function (_Usrmodifiedsrc) { Usrmodifiedsrc = _Usrmodifiedsrc })); else Usrmodifiedsrc = '';
    if (Usrmodifiedsrc!=Null0(Genedsrc)) {
       ...
       if (InStr1(1,Lprior,'* \<insert\>') || InStr1(1,Lprior,'*\<insert\>') || InStr1(1,Lprior,'* \<delete\>') || InStr1(1,Lprior,'*\<delete\>')) {
          ...
          await JSB_BF_TRASHIT(Projectname, Itemid);
       }

       if (await JSB_ODB_WRITE(Genedsrc, Fproject, Itemid)); else return Stop('Form-', System(28, me), ': ', me._activeProcess.At_Errors);

       await asyncTclExecute('Basic ' + Projectname + ' ' + Itemid, _capturedData => Results = _capturedData)
       Results = Change(Results,am,crlf); 
       if (InStr1(1,Results,'^')) {
          Results = 'Compiler error: ' + 'Basic ' + Projectname + ' ' + Itemid + crlf + CStr(Results) + crlf;
       }
    }

    return CStr(Results) + JSB_BF_ANCHOREDIT(Projectname, Itemid) + crlf + crlf; 
}
// </UPDATECODE>

// <UPDATEMENU_Sub>
async function JSB_MDL_UPDATEMENU_Sub(Projectname, Forceit) { 
    var me = new jsbRoutine("JSB_MDL", "updatemenu", "JSB_MDL_UPDATEMENU_Sub");
    // local variables
    ...
    if (await JSB_ODB_READ(Oldmenu, await JSB_BF_FHANDLE(Projectname), 'Project_Menus', function (_Oldmenu) { Oldmenu = _Oldmenu })) {
       var Lastmenucompile = At_Session.Item('LastMenuCompile_' + Projectname); 
       if (r83Time()-Lastmenucompile<15) return;
    }

    var Html = [undefined, ]; 
    Html[Html.length] = 'Subroutine Project_Menus(ByRef MenuBar As JSON, ByVal edpName As String)'; 
    Html[Html.length] = '* EDITOR-OPTION: AUTOFORMAT") '; 
    Html[Html.length] = '   MenuBar = {}'; 
    ...
    if (await JSB_ODB_OPEN('dict', Projectname, Fproject, function (_Fproject) { Fproject = _Fproject })); else return Stop(At(-1), 'Edp-', System(28, me), ': ', me._activeProcess.At_Errors);
    if (await JSB_ODB_SELECTTO(CStr(undefined), Fproject, 'ItemID Like \'%.page\'', Fs, function (_Fs) { Fs = _Fs })); else return Stop(At(-1), 'Edp-', System(28, me), ': ', me._activeProcess.At_Errors);
    Projectpages = getList(Fs); 

    Menutree = {}; 
    var PAGEID_LastI5 = UBound(Projectpages); 
    for (Pno = LBound(Projectpages); Pno<=PAGEID_LastI5; Pno++) {
       Pageid = Extract(Projectpages, Pno, 0, 0); 
       if (await JSB_ODB_READJSON(Item, Fproject, CStr(Pageid), function (_Item) { Item = _Item })); else return Stop(At(-1), 'Edp-', System(28, me), ': ', me._activeProcess.At_Errors);
    ...
    for (Role of iterateOver(Roles)) {
       ...
       if (Len(Menus)) {
          Html[Html.length] = ''; 
          if (Role!='Anonymous') Html[Html.length] = '   If @Is' + CStr(Role) + ' Then';

          // Convert to HTML - do ICONs first
          for (Menuname of iterateOver(Menus)) {
             So = Menus[Menuname]; 
             if (isJSON(So)) {
                if (nicename(CStr(Menuname))==Menuname) To = '.' + CStr(Menuname); else To = '[\'' + CStr(Menuname) + '\']';
                Html[Html.length] = '   MenuBar' + CStr(To) + ' = ' + CStr(So);
          ...
          for (Menuname of iterateOver(Menus)) {
             ...
             if (CBool(isArray(So))) {
                if (nicename(CStr(Menuname))==Menuname) To = '.' + CStr(Menuname); else To = '[\'' + CStr(Menuname) + '\']';
                Html[Html.length] = '   If !isJson(MenuBar' + CStr(To) + ') Then MenuBar' + CStr(To) + ' = {}'; 
                for (Submenuname of iterateOver(So)) {
                   if (nicename(CStr(Submenuname))==Submenuname) To2 = '.' + CStr(Submenuname); else To2 = '[\'' + CStr(Submenuname) + '\']';
                   Item = So[Submenuname]; 
                   if (isJSON(Item)) {
                      Html[Html.length] = '   MenuBar' + CStr(To) + CStr(To2) + ' = ' + CStr(Item);
                   } else {
                      Html[Html.length] = '   MenuBar' + CStr(To) + CStr(To2) + ' = { id: "' + CStr(Item) + '" }';
                   }
                }
             }
          } 

          if (Role!='Anonymous') Html[Html.length] = '   End If';
       }
    } 

    // Tag on web master stuff
    Html[Html.length] = '   If @IsAdmin Then'; 
    Html[Html.length] = '      Dim fromPage As String'; 
    Html[Html.length] = '      If InStr(@Url, "\\\\":edpName) Then fromPage = @Url Else fromPage = edpName'; 
    Html[Html.length] = ''; 
    Html[Html.length] = '      MenuBar["Web Page"] = {}'; 
    Html[Html.length] = '      MenuBar["Web Page"]["View Page Props"] = {}'; 
    Html[Html.length] = ('      MenuBar["Web Page"]["View Page Props"].id = "?edp&projectName=":@UrlEncode(System(27)):"&pageName=":@UrlEncode(edpName):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Web Page"]["ReGen Page"] = {}'; 
    Html[Html.length] = ('      MenuBar["Web Page"]["ReGen Page"].id = "?ReGenPageModel&projectName=":@UrlEncode(System(27)):"&pageName=":@UrlEncode(edpName):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Web Page"].seperator1 = true'; 
    Html[Html.length] = '      MenuBar["Web Page"]["Add Page"] = {}'; 
    Html[Html.length] = ('      MenuBar["Web Page"]["Add Page"].id = "?edp&projectName=":@UrlEncode(System(27)):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Web Page"]["Clone Page As"] = {}'; 
    Html[Html.length] = ('      MenuBar["Web Page"]["Clone Page As"].id = "?clonePageModel&projectName=":@UrlEncode(System(27)):"&pageName=":@UrlEncode(edpName):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Web Page"].seperator2 = true'; 
    Html[Html.length] = '      MenuBar["Web Page"]["Rename Page"] = {}'; 
    Html[Html.length] = ('      MenuBar["Web Page"]["Rename Page"].id = "?renamePageModel&projectName=":@UrlEncode(System(27)):"&pageName=":@UrlEncode(edpName):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Web Page"].seperator3 = true'; 
    Html[Html.length] = '      MenuBar["Web Page"]["Delete Page"] = {}'; 
    Html[Html.length] = ('      MenuBar["Web Page"]["Delete Page"].id = "?deletePageModel&projectName=":@UrlEncode(System(27)):"&pageName=":@UrlEncode(edpName):"&fromPage=Menu"'); 
    Html[Html.length] = ''; 
    Html[Html.length] = '      // Can collaspe here after Any HTML '; 
    Html[Html.length] = '      MenuBar.collaspable = True'; 
    Html[Html.length] = ''; 
    Html[Html.length] = '      MenuBar["Menus"] = {}'; 
    Html[Html.length] = '      MenuBar["Menus"]["Main Menu"] = {}'; 
    Html[Html.length] = '      MenuBar["Menus"]["Main Menu"].id = "?Menu"'; 
    Html[Html.length] = '      MenuBar["Menus"]["Add Page To Menu"] = {}'; 
    Html[Html.length] = ('      MenuBar["Menus"]["Add Page To Menu"].id = "?addPageToMenu&projectName=":@UrlEncode(System(27)):"&pageName=":@UrlEncode(edpName):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Menus"]["Add Page To Reference Menu"] = {}'; 
    Html[Html.length] = ('      MenuBar["Menus"]["Add Page To Reference Menu"].id = "?addPageToReferenceMenu&projectName=":@UrlEncode(System(27)):"&pageName=":@UrlEncode(edpName):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = ''; 
    Html[Html.length] = '      MenuBar["TCL"] = {}'; 
    Html[Html.length] = '      MenuBar["TCL"].id = "tcl"'; 
    Html[Html.length] = ''; 
    Html[Html.length] = '      MenuBar["Web Security"] = {}'; 
    Html[Html.length] = '      MenuBar["Web Security"]["Security"] = {}'; 
    Html[Html.length] = ('      MenuBar["Web Security"]["Security"].id = "?WebSiteSecurity&projectName=":@UrlEncode(System(27)):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = ''; 
    Html[Html.length] = '      MenuBar["Websites"] = {}'; 
    Html[Html.length] = '      MenuBar["Websites"]["ReGen MenuBar"] = {}'; 
    Html[Html.length] = ('      MenuBar["Websites"]["ReGen MenuBar"].id = "?updateMenus&projectName=":@UrlEncode(System(27)):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Websites"].seperator1 = true'; 
    Html[Html.length] = '      MenuBar["Websites"]["ReGen Website"] = {}'; 
    Html[Html.length] = ('      MenuBar["Websites"]["ReGen Website"].id = "?RegenWebSiteModel&projectName=":@UrlEncode(System(27)):"&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Websites"].seperator2 = true'; 
    Html[Html.length] = '      MenuBar["Websites"]["New Website"] = {}'; 
    Html[Html.length] = ('      MenuBar["Websites"]["New Website"].id = "?newWebSiteModel&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Websites"]["Clone Website"] = {}'; 
    Html[Html.length] = ('      MenuBar["Websites"]["Clone Website"].id = "?cloneWebSite&fromPage=":@UrlEncode(fromPage)'); 
    Html[Html.length] = '      MenuBar["Websites"].seperator3 = true'; 
    Html[Html.length] = '      MenuBar["Websites"]["Delete Website"] = {}'; 
    Html[Html.length] = ('      MenuBar["Websites"]["Delete Website"].id = "?deleteWebSiteModel&projectName=":@UrlEncode(System(27)):""'); 

    Html[Html.length] = '   End If'; 

    Html[Html.length] = ''; 
    Html[Html.length] = '    Dim customMenu As String = \'' + Projectname + '.\':NiceName(edpName):\'_customMenu\''; 
    Html[Html.length] = '    Call @?customMenu(MenuBar)'; 
    Html[Html.length] = 'End Sub'; 

    var Newmenu = Join(Html,crlf); 
    Oldmenu = Change(Oldmenu,am,crlf); 

    if (Oldmenu!=Null0(Newmenu) || Forceit) {
       Println(await JSB_MDL_UPDATECODE(CStr(Html), Projectname, 'Project_Menus'));
    }

    At_Session.Item('LastMenuCompile_' + Projectname, r83Time()); 
}
// </UPDATEMENU_Sub>

// <UPDATEMENUS_Pgm>
async function JSB_MDL_UPDATEMENUS_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "updatemenus", "JSB_MDL_UPDATEMENUS_Pgm");
    // local variables
...
async function JSB_MDL_URLOUTPUTPARAMS(ByRef_Outputfields, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "urloutputparams", "JSB_MDL_URLOUTPUTPARAMS");
    // local variables
    var Urlparams, Row;
...
async function JSB_MDL_URLPASSTHROUGH(ByRef_Viewmodel, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "urlpassthrough", "JSB_MDL_URLPASSTHROUGH");
    // local variables
    var Html, Inputs, Row, Varname;
    ...
    Html = [undefined, '   UrlParams = ""']; // Returns UrlParams
    Inputs = [undefined, ]; 
    ...
    for (Row of iterateOver(Inputs)) {
       ...
       if (CBool(Row.name)) {
          ...
          switch (Row.scope) {
             case 'UrlParam': 
                Html[Html.length] = '   ' + CStr(Varname) + ' = @ParamVar("' + CStr(Row.name) + '")'; 
                Html[Html.length] = '   if !IsNothing(' + CStr(Varname) + ') Then'; 
                Html[Html.length] = ('      If UrlParams Then UrlParams := "&" Else UrlParams = "?"'); 
                Html[Html.length] = '      UrlParams := "' + CStr(Row.name) + '=":@UrlEncode(' + CStr(Varname) + ')'; 
                Html[Html.length] = '   End If';           
       }
       }
    } 
...
async function JSB_MDL_VIEWDEFINITIONS(ByRef_Modelcolumns, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "viewdefinitions", "JSB_MDL_VIEWDEFINITIONS");
    // local variables
...
async function JSB_MDL_VIEWSINPAGE(ByRef_Projectname, Pagename, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "viewsinpage", "JSB_MDL_VIEWSINPAGE");
    // local variables
...
async function JSB_MDL_VIEWWIZARD_Sub(ByRef_Projectname, ByRef_Pagename, ByRef_Viewname, ByRef_Dataset, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "viewwizard", "JSB_MDL_VIEWWIZARD_Sub");
    // local variables
    var Fdict, Totype;
    ...
    if (await JSB_ODB_WRITEJSON(ByRef_Dataset, Fdict, CStr(ByRef_Viewname))); else return Stop(At(-1), 'edv-', System(28, me), ': ', me._activeProcess.At_Errors);
...
async function JSB_MDL_VIEW_EXCELUPLOAD_CANCELEDITS(Pkid, ByRef_Rtnerrors, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_canceledits", "JSB_MDL_VIEW_EXCELUPLOAD_CANCELEDITS");
    function exit(v) {
...
async function JSB_MDL_VIEW_EXCELUPLOAD_CHECKCOMMANDS_Sub(Viewvars, Pagebtn, Columnid) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_checkcommands", "JSB_MDL_VIEW_EXCELUPLOAD_CHECKCOMMANDS_Sub");
    // What button did the user click?
    var Btn = Pagebtn + formVar('formBtn_ExcelUpload') + formVar('ne_ExcelUpload_formBtn') + formVar('contextMenuCmd'); 
    var Rtnerrors = ''; var Pkid = ''; var Transferhtml = ''; var Urlparams = ''; var Buff = '', Tag = ''; 

    await JSB_BF_CHECKSTANDARDCOMMANDS_Sub(Btn, 'mdl', 'ExcelUpload.view', Columnid, 'ne_ExcelUpload', false, CStr(Viewvars.fromParentPage), CNum(Viewvars.parentMultiView), CStr(Viewvars.htmlBackDrop), undefined, function (_Btn) { Btn = _Btn }); 
    ...
    switch (Btn) {
       case 'C': case 'B': case 'Q': 
          ...
          if (await JSB_MDL_VIEW_EXCELUPLOAD_ISDIRTY(Viewvars)) {
             Ans = await JSB_BF_MSGBOX('Warning!', JSB_BF_LANGUAGE('You are about to lose any edits you have made. Continue?'), 'Yes,No');
          } else {
             Ans = 'Yes';
          }
          if (Ans=='Yes') {
    ...
    break; 
       ...
       case 'Save': 
          ...
          if (Successop) {
             Urlparams = ''; 
             Transferhtml = JSB_BF_GENEVENTHANDLER('Transfer', 'ExcelUpload' + Urlparams, 10, '', 0, false); 
             if (CBool(Viewvars.ParentRefresh)) { Transferhtml = CStr(Transferhtml) + JSB_HTML_SCRIPT('if (parent.refreshData) parent.refreshData();'); }
             Transferhtml = CStr(Transferhtml) + JSB_HTML_SCRIPT('window.eventHandler_Transfer(jsbFormVars(), "ItemID");'); 

             // Push all our values into a hidden tag to be used by the transfer
             for (Tag of iterateOver(Viewvars.Row)) {
                Transferhtml = JSB_HTML_HIDDEN(Tag, CStr(Viewvars.Row[Tag])) + Transferhtml; 
             } 
...
async function JSB_MDL_VIEW_EXCELUPLOAD_DELETEROW(Pkid, ByRef_Rtnerrors, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_deleterow", "JSB_MDL_VIEW_EXCELUPLOAD_DELETEROW");
    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Rtnerrors)
        return v
    }
    ...
    if (await JSB_ODB_DELETEITEM(Ftable, Pkid)); else {
       ByRef_Rtnerrors = me._activeProcess.At_Errors; 
...
async function JSB_MDL_VIEW_EXCELUPLOAD_DISPLAY_CC_Sub(Viewvars, ByRef_Html, ByRef_Toolbar, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_display_cc", "JSB_MDL_VIEW_EXCELUPLOAD_DISPLAY_CC_Sub");
    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Html, ByRef_Toolbar)
        return v
    }
...
async function JSB_MDL_VIEW_EXCELUPLOAD_GETPK(Sqlfilter, ByRef_Pkid, ByRef_Rtnerrors, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_getpk", "JSB_MDL_VIEW_EXCELUPLOAD_GETPK");
    function exit(v) {
    ...
    if (await JSB_ODB_SELECTTO('', Ftable, Sqlfilter, Selecthandle, function (_Selecthandle) { Selecthandle = _Selecthandle })); else {
       ByRef_Rtnerrors = 'Form-' + CStr(System(28, me)) + '-Select-' + Sqlfilter + ': ' + me._activeProcess.At_Errors; 
       await JSB_BF_LOGERR(ByRef_Rtnerrors);
       return exit(false);
    }

    ByRef_Pkid = readNext(Selecthandle).itemid; 
    if (ByRef_Pkid); else ByRef_Rtnerrors = 'No items found in table \'tmp\' Where ' + Sqlfilter;
...
async function JSB_MDL_VIEW_EXCELUPLOAD_ISDIRTY(Viewvars) { 
    var me = new jsbRoutine("JSB_MDL", "VIEW_EXCELUPLOAD_ISDIRTY", "JSB_MDL_VIEW_EXCELUPLOAD_ISDIRTY");
...
async function JSB_MDL_VIEW_EXCELUPLOAD_NEWROW(ByRef_Rtnerrors, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_newrow", "JSB_MDL_VIEW_EXCELUPLOAD_NEWROW");
    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Rtnerrors)
        return v
    }
...
async function JSB_MDL_VIEW_EXCELUPLOAD_READROW(Pkid, ByRef_Rtnrow, ByRef_Rtnerrors, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_readrow", "JSB_MDL_VIEW_EXCELUPLOAD_READROW");
    function exit(v) {
        if (typeof setByRefValues == 'function') setByRefValues(ByRef_Rtnrow, ByRef_Rtnerrors)
        return v
    }
    if (await JSB_ODB_ATTACHDB('sysprog')); else { ByRef_Rtnerrors = me._activeProcess.At_Errors; return exit(undefined); }
    ...
    if (await JSB_ODB_READJSON(ByRef_Rtnrow, Ftable, Pkid, function (_ByRef_Rtnrow) { ByRef_Rtnrow = _ByRef_Rtnrow })); else {
       ByRef_Rtnerrors = me._activeProcess.At_Errors; 
       if (!ByRef_Rtnerrors) ByRef_Rtnerrors = 'Item \'' + Pkid + '\' in \'ExcelUpload.view\' not found.';
       await JSB_BF_LOGERR(ByRef_Rtnerrors);
       return exit(false);
    }
...
async function JSB_MDL_VIEW_EXCELUPLOAD_SETDEFAULTS(Rtnrow) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_setdefaults", "JSB_MDL_VIEW_EXCELUPLOAD_SETDEFAULTS");
...
async function JSB_MDL_VIEW_EXCELUPLOAD_SETUP_Sub(ByRef_Viewvars, Basedirectory, Xlsfilename, Sqldbname, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_setup", "JSB_MDL_VIEW_EXCELUPLOAD_SETUP_Sub");
    function exit(v) {
    ...
    if (InStr1(1,JSB_BF_URL(),'newRecord=1')) {
       ...
       if (Rtnerrors && !JSB_BF_ISPOSTBACK()) Alert(Rtnerrors, true);
    } else {
       Row = {}; 
       Sqlfilter = ''; 
       ...
       if (!(await JSB_MDL_VIEW_EXCELUPLOAD_GETPK(Sqlfilter, Currentid, Rtnerrors, function (_Currentid, _Rtnerrors) { Currentid = _Currentid; Rtnerrors = _Rtnerrors })) && !JSB_BF_ISPOSTBACK()) Alert(Rtnerrors, true);
       ...
       if (!Currentid) {
          ...
          // Didn't find record
          if (Not(Row)) { Row = {} }
          if (Rtnerrors && !JSB_BF_ISPOSTBACK()) Alert(Rtnerrors, false);
       }
    }
...
async function JSB_MDL_VIEW_EXCELUPLOAD_UNLOAD_Sub(Viewvars) { 
    var me = new jsbRoutine("JSB_MDL", "VIEW_EXCELUPLOAD_UNLOAD", "JSB_MDL_VIEW_EXCELUPLOAD_UNLOAD_Sub");
...
async function JSB_MDL_VIEW_EXCELUPLOAD_WRITEROW(Row, ByRef_Pkid, ByRef_Rtnerrors, setByRefValues) { 
    var me = new jsbRoutine("JSB_MDL", "view_excelupload_writerow", "JSB_MDL_VIEW_EXCELUPLOAD_WRITEROW");
    atgoto: while (true) {
        switch (me._state) {
            case "":
                function exit(v) {
                ...
                if (Newrecord) {
                   ...
                   if (await JSB_ODB_WRITEJSON(Row, Ftable, ByRef_Pkid)); else { me._state = "ERRWRITE"; continue atgoto; }
                } else {
                   if (HasTag(Row,'isA_CrlF_TextFile')) {
                      ...
                      if (await JSB_ODB_WRITE(Supdatedrow, Ftable, ByRef_Pkid)); else { me._state = "ERRWRITE"; continue atgoto; }
                   } else {
                      ...
                      if (await JSB_ODB_WRITEJSON(Updatedrow, Ftable, ByRef_Pkid)); else { me._state = "ERRWRITE"; continue atgoto; }
                   }
                }
...
async function JSB_MDL_WEBSITEMODEL_Sub() { 
    var me = new jsbRoutine("JSB_MDL", "websitemodel", "JSB_MDL_WEBSITEMODEL_Sub");
    // %options aspxC-
}
// </WEBSITEMODEL_Sub>
...
async function JSB_MDL_WEBSITESECURITY_Pgm() {  // PROGRAM
    var me = new jsbRoutine("JSB_MDL", "websitesecurity", "JSB_MDL_WEBSITESECURITY_Pgm");
    // local variables
    ...
    if (await JSB_ODB_OPEN('dict', CStr(Projectname), F, function (_F) { F = _F })); else return Stop(me._activeProcess.At_Errors);
    if (await JSB_ODB_SELECTTO(CStr(undefined), F, 'ItemID like \'%.page\'', Sl, function (_Sl) { Sl = _Sl })); else return Stop(me._activeProcess.At_Errors);
    Pages = getList(Sl); 
    ...
    Println(); 
    Print(JSB_BF_HTML('\<div class="container"\>')); 
    for (Pagename of iterateOver(Pages)) {
       if (await JSB_ODB_READJSON(Page, F, CStr(Pagename), function (_Page) { Page = _Page })); else return Stop(me._activeProcess.At_Errors);
       R = Page.minimumRole; 
       if (Locate(R, Choices, 0, 0, 0, "", position => { })); else R = 'Anonymous';
       Pn = dropIfRight(CStr(Pagename), '.page', true); 
       Print(JSB_BF_LBLCTLSET(CStr(Pn), JSB_HTML_RADIOBTNS(CStr(Pn), Choices, CStr(R)), CStr(Pn), true)); 
    } 
    Println(JSB_BF_HTML('\</div\>')); // Container
    Println(); 
    ...
    if (B=='Save') {
       ...
       for (Pagename of iterateOver(Pages)) {
          ...
          if (CBool(V)) {
             if (V=='\<delete\>') {
                if (Not(await JSB_MDL_DELETEPAGEMODEL(Projectname, Pagename, function (_Projectname, _Pagename) { Projectname = _Projectname; Pagename = _Pagename }))) Alert(me._activeProcess.At_Errors);
                Updatethemenus = true; ;
             } else {
                if (await JSB_ODB_READJSON(P, F, CStr(Pagename), function (_P) { P = _P })); else return Stop(me._activeProcess.At_Errors);
                R = P.minimumRole; 
                if (Locate(R, Choices, 0, 0, 0, "", position => { })); else R = 'Anonymous';
                if (R!=V) {
                   P.minimumRole = V; 
                   if (await JSB_ODB_WRITEJSON(P, F, CStr(Pagename))); else return Stop(At(-1), 'WebSiteSecurity-', System(28, me), ': ', me._activeProcess.At_Errors);
