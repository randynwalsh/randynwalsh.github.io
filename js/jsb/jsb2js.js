    ...
    atgoto: while (true) {
        switch (gotoLabel) {
            case "":
                ...
                window.onBatteryStatus = function (status) { me.Status = status; 
    ...
    atgoto: while (true) {
        switch (gotoLabel) {
            case "":
                ...
                window.onBatteryLevel = function (status) { me.Status = status; 
    ...
    atgoto: while (true) {
        switch (gotoLabel) {
            case "":
                ...
                me.handleCompassChange = function (event) { if (me.nested) return;
        me.nested += 1;
...
async function JSB_BF_DEBUGGER_Sub(Fromobj, Ilineno) { 
    ...
    window.dbgAddVar = (function (Name) { window.dbgAddVar = function(Name) {
            if (!Name) return;
    ...
    // =======================================================================================
    window.dbgGetVariable = (function (Name,forDisplay) { var fromObj = window.dbgEditingObj;
        var vo = null;
    ...
    window.dbgSetVariable = (function (Name,newVal) { var oldVal = window.dbgGetVariable(Name) ;
    ...
    window.varSelect = function (sel) { if ($(sel).prop('nodeName') == "DIV") {
    ...
    window.dbgPostCommand = function (Txt) { if (window.debuggerProcess) {
    ...
    // =======================================================================================
    window.debuggerKeyUp = (function (e) { if (e.which === 116) { // F5 - run
         dbgPostCommand("R")
         return false;
      }
    ...
    $('word').hover(function () { var myJSB = $('#jsb')
      var pos = $(this).offset();
      ...
      if (window.dbgPopupTimer) clearTimeout(dbgPopupTimer); },function () { if (window.dbgPopupTimer) clearTimeout(dbgPopupTimer);
    ...
    $('#popupvar').hover(function () { if (window.dbgPopupTimer) clearTimeout(dbgPopupTimer); },function () { if (window.dbgPopupTimer) clearTimeout(dbgPopupTimer);
      dbgPopupTimer = setTimeout(function () { $("#popupvar").hide(); dbgPopupTimer = null; }, 500) });
    ...
    atgoto: while (true) {
        switch (gotoLabel) {
            case "":
                ...
                me.handleOrientationChange = function (event) { if (me.nested) return;
        me.nested += 1;
        me.myEvent = event;
...
"tap":false
       ...
       window.leaflet_clearTileCache = function () { Commons_JSB_BF.Tilelist = [undefined]
       ...
       window.leaflet_onZoomStart = function () { window.limitToLevel = 0; // Allow all tiles to load until ZoomEnd
       ...
       window.leaflet_onZoomEnd = function () { window.limitToLevel = map.getZoom(); // Zoom is done, only load tiles are current level
         }; 
       map.on('zoomend',window.leaflet_onZoomEnd);

       map.on('mousedown touchstart',function (Event) { var msg = CStr(map.getZoom()) + ":(" + Left(Event.latlng.lat, 10) + ", " + Left(Event.latlng.lng, 10) + ")"
            leaflet_Msg(msg) });

       if (($('.ui-dialog').length)) {
          window.leaflet_onresize = function () { window.minTileNumber = window.tileNumber
                window.limitToLevel = 0
                Commons_JSB_BF.Tilelist = [undefined]
       ...
       window.leaflet_Msg = function (Msg) { if (!Msg) Msg = CStr(map.getZoom()) + ":(" + Left(map.getCenter().lat, 10) + ", " + Left(map.getCenter().lng, 10) + ")"
    ...
    atgoto: while (true) {
        switch (gotoLabel) {
            case "":
                ...
                window.navigator.geolocation.getCurrentPosition(function (position) { if (window.waitingForGPS) { 
        ...
        } },function (err) { if (window.waitingForGPS) { 
...
function JSB_BF_THEME(Usetheme) { 
    ...
    if (Usetheme) {
       ...
       window.setTimeout(function () { $("link[href*='bootstrap_'][href*='.css']").remove();
            $("link[href*='jsb_mobile.css']").remove();
            $('head').append(Newtheme); },1);
    }
